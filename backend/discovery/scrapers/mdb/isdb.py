"""
Islamic Development Bank (IsDB) Scraper
======================================

Scrapes procurement opportunities from Islamic Development Bank.

URL: https://www.isdb.org/procurement

Notes:
- IsDB is a multilateral development bank for Muslim-majority countries
...
class IsDBScraper(BaseScraper):
	"""Scraper for IsDB procurement opportunities."""

	source_id = "isdb"
	source_name = "Islamic Development Bank"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape IsDB procurement opportunities."""
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching IsDB from {path}")
	...
	def _parse_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single procurement item from IsDB."""
