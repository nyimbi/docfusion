"""
IFC (International Finance Corporation) Scraper
================================================

Scrapes procurement opportunities from IFC, a member of the World Bank Group.

URL: https://www.ifc.org/en/what-we-do/procurement

Notes:
- IFC is the private sector arm of the World Bank Group
- Focuses on private sector development projects
...
class IFCScraper(BaseScraper):
	"""Scraper for IFC procurement opportunities."""

	source_id = "ifc"
	source_name = "International Finance Corporation"
	base_url = "https://www.ifc.org"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape IFC procurement opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# IFC procurement pages
		search_paths = [
			"/en/what-we-do/procurement",
			"/en/projects-and-operations/procurement",
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching IFC from {path}")
				html = await self.fetch_page(path)
				soup = self.parse_html(html)

				# Find opportunity items
	...
	def _parse_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single procurement item from IFC."""
		# Extract title
