"""
Asian Infrastructure Investment Bank (AIIB) Scraper
=================================================

Scrapes procurement opportunities from Asian Infrastructure Investment Bank.

URL: https://www.aiib.org/en/opportunities/procurement

Notes:
- AIIB is a multilateral development bank focused on infrastructure
- Procurement notices for consulting, goods, and works
- Focuses on sustainable infrastructure projects
...
class AIIBScraper(BaseScraper):
	"""Scraper for AIIB procurement opportunities."""

	source_id = "aiib"
	source_name = "Asian Infrastructure Investment Bank"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape AIIB procurement opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		search_paths = [
			"/en/opportunities/procurement",
			"/opportunities/procurement",
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching AIIB from {path}")
	...
	def _parse_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single procurement item from AIIB."""
