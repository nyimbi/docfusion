"""
Inter-American Development Bank (IDB) Scraper
============================================

Scrapes procurement opportunities from Inter-American Development Bank.

URL: https://www.iadb.org/en/procurement

Notes:
- IDB is a regional development bank for Latin America and Caribbean
- Procurement notices for consulting, goods, and works
- Focuses on development projects in member countries
- IDB Invest for private sector operations
...
class IDBScraper(BaseScraper):
	"""Scraper for IDB procurement opportunities."""

	source_id = "idb"
	source_name = "Inter-American Development Bank"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape IDB procurement opportunities."""
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching IDB from {path}")
	...
	def _parse_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single procurement item from IDB."""
