...
Scrapes procurement opportunities from EBRD.

URL: https://www.ebrd.com/work-with-us/procurement.html

Notes:
- EBRD is a multilateral development bank focused on transition economies
- Procurement notices for consulting, goods, and works
- Focuses on private sector development
- Covers Central Asia, Eastern Europe, and Mediterranean
...
class EBRDScraper(BaseScraper):
	"""Scraper for EBRD procurement opportunities."""

	source_id = "ebrd"
	source_name = "European Bank for Reconstruction and Development"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape EBRD procurement opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		search_paths = [
			"/work-with-us/procurement.html",
			"/procurement",
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching EBRD from {path}")
	...
	def _parse_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single procurement item from EBRD."""
