"""
Gabon DGMP (Direction Générale des Marchés Publics) Scraper
============================================================

Scrapes tender opportunities from Gabon's Public Procurement Agency
(DGMP - Direction Générale des Marchés Publics).

URL: http://www.dgmp.ga

Notes:
- Official portal managed by DGMP
- Ministry of Budget and Public Accounts oversight
- French language portal
- Limited e-procurement (ePublishing/Notification)
...
class GabonScraper(PlaywrightScraper):
	"""Scraper for Gabon DGMP Portal.

	Gabon's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "gabon_dgmp"
	source_name = "Gabon DGMP"
	base_url = "http://www.dgmp.ga"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Gabon DGMP for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Gabon DGMP pages - French language
		search_paths = [
			"/",  # Main portal
		]
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Gabon DGMP from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Gabon DGMP."""
		# Extract title (French: Objet)
