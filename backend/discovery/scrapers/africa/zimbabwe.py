"""
Zimbabwe PRAZ e-GP (Electronic Government Procurement) Scraper
===============================================================

Scrapes tender opportunities from Zimbabwe's e-GP system.

URL: https://egp.praz.org.zw

Notes:
- Managed by Procurement Regulatory Authority of Zimbabwe (PRAZ)
- Launched October 2023
- 10,000+ tenders published
- 17,000+ registered suppliers
...
class ZimbabweScraper(PlaywrightScraper):
	"""Scraper for Zimbabwe PRAZ e-GP Portal.

	Zimbabwe's e-GP requires JavaScript rendering.
	"""

	source_id = "zimbabwe_praz"
	source_name = "Zimbabwe PRAZ"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Zimbabwe PRAZ e-GP for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Zimbabwe PRAZ e-GP pages - requires public access
		# Note: Main tender listings require login, but public reports are available
		search_paths = [
			"/egp-Q21zSG9tZVBhZ2VzL29wZW5fZGF0YV9ncmlk",  # Public reports
			"/egp-SW5kZXhlcy9pbmRleA==",  # Bulletin board
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching Zimbabwe PRAZ from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Zimbabwe PRAZ e-GP."""
		# Extract title
