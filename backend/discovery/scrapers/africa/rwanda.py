"""
Rwanda Umucyo e-Procurement Scraper
===================================

Scrapes tender opportunities from Rwanda's UMUCYO e-procurement portal.
...
class RwandaScraper(PlaywrightScraper):
	"""Scraper for Rwanda Umucyo e-Procurement Portal.

	Rwanda's UMUCYO portal uses JavaScript for dynamic content.
	"""

	source_id = "rwanda_umucyo"
	source_name = "Rwanda Umucyo"
	base_url = "https://www.umucyo.gov.rw"
	...
	default_wait_for = "table tbody tr, .tender-item, .tender-row, .opportunity-item"

	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Rwanda Umucyo for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Try multiple paths for tender listings
		search_paths = [
			"/eb/bpp/selectPageProcurementPlan.do?menuId=EB01010100",
			"/pt/pcm/moveMobileIntroductionDetail.do?menuId=PT01010900",
			"/",
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching Rwanda Umucyo from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		...
		title_elem = item.select_one(
			"td:first-child a, .title a, a[href*='tender'], td a, .tender-title"
		)
		if not title_elem:
			return None
