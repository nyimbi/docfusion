"""
Morocco Marchés Publics Scraper
===============================

Scrapes tender opportunities from Morocco's Public Procurement Portal.

URL: https://www.marchespublics.gov.ma

Notes:
- Official portal managed by Kingdom of Morocco
...
class MoroccoScraper(PlaywrightScraper):
	"""Scraper for Morocco Marchés Publics Portal.

	Morocco's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "morocco_marches"
	source_name = "Morocco Marchés Publics"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Morocco Marchés Publics for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Morocco Marchés Publics pages - French/Arabic language
		search_paths = [
			"/",  # Main portal
		]
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Morocco Marchés Publics from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Morocco Marchés Publics."""
