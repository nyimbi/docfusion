"""
Tunisia TUNEPS Scraper
======================

Scrapes tender opportunities from Tunisia's National E-Procurement System.

URL: https://tuneps.ancp.gov.tn

Notes:
- Official portal managed by Tunisian Government
- Arabic/French language portal
- TUNEPS (Tunisian National E-Procurement System)
...
class TunisiaScraper(PlaywrightScraper):
	"""Scraper for Tunisia TUNEPS Portal.

	Tunisia's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "tunisia_tuneps"
	source_name = "Tunisia TUNEPS"
	base_url = "https://tuneps.ancp.gov.tn"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Tunisia TUNEPS for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Tunisia TUNEPS pages - French/Arabic language
		search_paths = [
			"/",  # Main portal
		]
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Tunisia TUNEPS from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Tunisia TUNEPS."""
		# Extract title (French/Arabic: Objet)
