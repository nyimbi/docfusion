"""
Burundi Tenders Scraper
=======================

Scrapes tender opportunities from Burundi's procurement ecosystem.
Note: Burundi does not have a centralized official e-procurement portal.

URL: https://www.burunditenders.com (Third-party aggregator)
Primary Language: French

Notes:
- Governed by Public Procurement Regulatory Authority (ARMP)
- WTO Government Procurement Agreement principles
- Annual tender market ~US$2 billion
- World Bank and AfDB funded projects common
...
class BurundiScraper(PlaywrightScraper):
	"""Scraper for Burundi Tenders (via third-party aggregator).

	Uses third-party aggregator due to lack of centralized official portal.
	"""

	source_id = "burundi_tenders"
	source_name = "Burundi Tenders"
	base_url = "https://www.burunditenders.com"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Burundi Tenders for opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Burundi Tenders pages
		search_paths = [
			"/",  # Main page
		]
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Burundi Tenders from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Burundi Tenders."""
		# Extract title
