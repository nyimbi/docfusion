"""
Mauritius e-Procurement System Scraper
======================================

Scrapes tender opportunities from Mauritius e-Procurement portal.

URL: https://eproc.publicprocurement.govmu.org

Notes:
- Managed by Procurement Policy Office (PPO)
- Official portal: https://eproc.publicprocurement.govmu.org
- Alternative URL: mauritiuseprocurement.govmu.org (redirects)
- Launched September 2015
- Government Online Centre hosted
...
class MauritiusScraper(PlaywrightScraper):
	"""Scraper for Mauritius e-Procurement Portal.

	Mauritius e-Procurement requires JavaScript rendering.
	"""

	source_id = "mauritius_eprocurement"
	source_name = "Mauritius e-Procurement"
	base_url = "https://publicprocurement.govmu.org"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Mauritius e-Procurement for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Mauritius e-Procurement pages
		# Traditional procurement portal has bid invitations and awards
		# Note: e-proc system at eproc.publicprocurement.govmu.org requires login
		search_paths = [
			"/publicprocurement/?page_id=763",  # Summary of Bid Evaluation Reports
			"/pages/plans.aspx",  # Annual Procurement Plans
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching Mauritius e-Procurement from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Mauritius e-Procurement."""
		# Extract title
