"""
Angola ENAPP/SNCP E-Procurement Scraper
======================================

Scrapes tender opportunities from Angola's National Public Procurement
Service (SNCP - Serviço Nacional de Contratação Pública) portal.

URL: https://compraspublicas.minfin.gov.ao/ComprasPublicas

Notes:
- Official portal managed by Ministry of Finance
- Full e-procurement platform with online submission
- Portuguese language portal
- Governed by Public Contracts Law (Law no. 41/20 of December 23, 2020)
- Features e-catalogue, framework agreements, e-reverse auction
...
class AngolaScraper(PlaywrightScraper):
	"""Scraper for Angola ENAPP/SNCP Portal.

	Angola's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "angola_sncp"
	source_name = "Angola SNCP"
	base_url = "https://compraspublicas.minfin.gov.ao"
	...
	default_wait_for = "table tbody tr, .tender-item, .concurso-item"

	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Angola SNCP for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Angola SNCP pages - Portuguese language
		search_paths = [
			"/ComprasPublicas/",  # Main portal
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching Angola SNCP from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Angola SNCP."""
		# Extract title (Portuguese: Objeto, Descrição)
		title_elem = item.select_one(
			"td:first-child a, .title a, a[href*='concurso'], td a"
		)
		if not title_elem:
			return None
		...
		# Extract organization (Portuguese: Entidade adjudicante)
		org_elem = item.select_one(
			"td:nth-child(2), .organization, .entidade"
		)
		org = self.clean_text(org_elem.get_text()) if org_elem else ""

		# Extract deadline (Portuguese: Data limite)
		deadline_elem = item.select_one(
			"td:nth-child(4), td:nth-child(5), .deadline, .data-limite"
		)
		deadline = None
		if deadline_elem:
