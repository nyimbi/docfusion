"""
Comoros ANMP Scraper
====================

Scrapes tender opportunities from Comoros National Public Procurement
Agency (ANMP - Agence Nationale des Marchés Publics).

Note: Comoros has limited centralized e-procurement infrastructure.
Tender information is primarily available through third-party aggregators.

URL: https://www.biddetail.com/comoros-tenders (Third-party aggregator)
Primary Language: French/Arabic

Notes:
- Governed by Law No. 03-04/AN of 09/01/2004 on Public Procurement
- Decree No. 05-090/PR for implementation
- Annual tender market ~US$100 million
- World Bank and UNDP funded projects common
...
class ComorosScraper(PlaywrightScraper):
	"""Scraper for Comoros Tenders (via third-party aggregator).

	Uses third-party aggregator due to limited official portal.
	"""

	source_id = "comoros_tenders"
	source_name = "Comoros Tenders"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Comoros Tenders for opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Comoros Tenders pages
		search_paths = [
			"/comoros-tenders",  # Comoros specific page
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching Comoros Tenders from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Comoros Tenders."""
		# Extract title
