"""
Somaliland National Tender Board Scraper
========================================

Scrapes tender opportunities from Somaliland National Tender Board
(NTB - National Tender Board).

URL: https://ntb.govsomaliland.org

Notes:
- Official portal managed by National Tender Board
- Governed by Public Procurement Act 82 of 2018
- English language portal
- Handles procurement financed from national budget
- Features standard bidding documents for goods, works, services
...
class SomalilandScraper(PlaywrightScraper):
	"""Scraper for Somaliland National Tender Board Portal.

	Somaliland's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "somaliland_ntb"
	source_name = "Somaliland NTB"
	base_url = "https://ntb.govsomaliland.org"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Somaliland NTB for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Somaliland NTB pages
		search_paths = [
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Somaliland NTB from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Somaliland NTB."""
		# Extract title
