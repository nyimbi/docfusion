"""
South Sudan PPDAA Tender Portal Scraper
========================================

Scrapes tender opportunities from South Sudan's Public Procurement
and Disposal of Assets Authority (PPDAA) tender portal.

URL: https://tenderportal.ppdaa.gov.ss

Notes:
- Official portal managed by PPDAA
- Governed by Public Procurement and Disposal of Assets Act 2018
- Requires provider registration for full access
...
class SouthSudanScraper(PlaywrightScraper):
	"""Scraper for South Sudan PPDAA Tender Portal.

	South Sudan's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "south_sudan_ppdaa"
	source_name = "South Sudan PPDAA"
	base_url = "https://tenderportal.ppdaa.gov.ss"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape South Sudan PPDAA for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# South Sudan PPDAA tender pages
		search_paths = [
			"/current-bids",  # Current tenders
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching South Sudan PPDAA from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from South Sudan PPDAA."""
		# Extract title
