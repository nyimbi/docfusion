"""
DR Congo SIGMAP (Système Intégré de Gestion des Marchés Publics) Scraper
========================================================================

Scrapes tender opportunities from DR Congo's integrated public procurement
management system.

URL: https://marchepublic.cd

Notes:
- Official portal managed by DGCMP (Direction Générale des Marchés Publics)
- Governed by Law N° 10/010 of April 27, 2010 on Public Procurement
- French language portal
- Offers tender notices, contract awards, and expressions of interest
...
class DRCongoScraper(PlaywrightScraper):
	"""Scraper for DR Congo SIGMAP Portal.

	DR Congo's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "dr_congo_sigmap"
	source_name = "DR Congo SIGMAP"
	base_url = "https://marchepublic.cd"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape DR Congo SIGMAP for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# DR Congo SIGMAP pages - French language
		search_paths = [
			"/",  # Main page with tender listings
			"/appels-offres",  # Tender notices (if available)
			"/avis-manifestation-interet",  # Expressions of interest
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching DR Congo SIGMAP from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from DR Congo SIGMAP."""
		# Extract title (French: Objet, Intitulé)
		title_elem = item.select_one(
			"td:first-child a, .title a, .objet a, .intitule a, a[href*='appel'], td a"
		)
		if not title_elem:
			return None
		...
		# Extract organization (French: Entité adjudicatrice, Structure)
		org_elem = item.select_one(
			"td:nth-child(2), .organization, .entite, .structure"
		)
		org = self.clean_text(org_elem.get_text()) if org_elem else ""

		# Extract deadline (French: Date limite, Echéance)
		...
		# Extract reference (French: Référence, N°)
