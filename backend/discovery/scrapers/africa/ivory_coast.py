"""
Côte d'Ivoire SIGOMAP Scraper
=============================

Scrapes tender opportunities from Côte d'Ivoire's e-procurement system.

URL: https://sigomap.gouv.ci

Notes:
- Official portal managed by the Government
- French language portal
- SIGOMAP (Système Intégré de Gestion des Marchés Publics)
- Governed by public procurement regulations
...
class IvoryCoastScraper(PlaywrightScraper):
	"""Scraper for Côte d'Ivoire SIGOMAP Portal.

	Côte d'Ivoire's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "ivory_coast_sigomap"
	source_name = "Côte d'Ivoire SIGOMAP"
	base_url = "https://sigomap.gouv.ci"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Côte d'Ivoire SIGOMAP for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Côte d'Ivoire SIGOMAP pages - French language
		search_paths = [
			"/",  # Main portal
		]
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Côte d'Ivoire SIGOMAP from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Côte d'Ivoire SIGOMAP."""
