"""
Uganda Government Procurement Portal (GPP) Scraper
====================================================

Scrapes tender opportunities from Uganda's Government Procurement Portal.

URL: https://gpp.ppda.go.ug

Notes:
- Managed by PPDA (Public Procurement and Disposal of Public Assets Authority)
- Provides open data via OCDS standard
- Bid invitations and tender visualizations available
...
class UgandaScraper(PlaywrightScraper):
	"""Scraper for Uganda GPP (Government Procurement Portal).

	Uganda's GPP provides tender data via open data endpoints.
	"""

	source_id = "uganda_gpp"
	source_name = "Uganda GPP"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Uganda GPP for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Uganda GPP public tender pages
		search_paths = [
			"/public/bid-invitations/visualizations",
			"/public/open-data/ocds/tenders",
			"/",
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching Uganda GPP from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Uganda GPP."""
		# Extract title
