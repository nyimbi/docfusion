"""
Sudan E-Procurement Scraper
===========================

Scrapes tender opportunities from Sudan's emerging e-procurement system.
Note: Sudan is launching a National Electronic Procurement System.

URL: https://www.sudantenders.com (Third-party aggregator)
Primary Language: Arabic/English

Notes:
- Ministry of Finance completing design of national e-procurement system
- Transitioning from manual procurement to digital platform
- Current coverage primarily through third-party aggregators
- Public Finance Reform Program initiative
...
class SudanScraper(PlaywrightScraper):
	"""Scraper for Sudan Tenders (via third-party aggregator).

	Uses third-party aggregator until official e-procurement system launches.
	"""

	source_id = "sudan_tenders"
	source_name = "Sudan Tenders"
	base_url = "https://www.sudantenders.com"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Sudan Tenders for opportunities."""
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Sudan Tenders from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Sudan Tenders."""
		# Extract title
