"""
Eswatini ESPPRA Scraper
=======================

Scrapes tender opportunities from Eswatini Public Procurement
Regulatory Authority (ESPPRA) portal.

URL: https://esppra.co.sz

Notes:
- Official portal managed by ESPPRA
- Formerly Swaziland National Tender Board
- English language portal
- Categories: Works, Services, Goods
- Contact: info@esppra.co.sz, +268 2404 7527
...
class EswatiniScraper(PlaywrightScraper):
	"""Scraper for Eswatini ESPPRA Portal.

	Eswatini's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "eswatini_esppra"
	source_name = "Eswatini ESPPRA"
	base_url = "https://esppra.co.sz"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Eswatini ESPPRA for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Eswatini ESPPRA pages
		search_paths = [
			"/sppra/tender.php",  # Tender listings
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching Eswatini ESPPRA from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Eswatini ESPPRA."""
		# Extract title
