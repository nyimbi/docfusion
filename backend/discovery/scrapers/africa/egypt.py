"""
Egypt E-Tenders Scraper
=======================

Scrapes tender opportunities from Egypt's Government E-Tender Portal.

URL: https://etenders.gov.eg

Notes:
- Official portal managed by Egyptian Government
- Arabic/English language portal
- Egyptian Government E-Tender Portal
...
class EgyptScraper(PlaywrightScraper):
	"""Scraper for Egypt E-Tenders Portal.

	Egypt's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "egypt_etenders"
	source_name = "Egypt E-Tenders"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Egypt E-Tenders for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Egypt E-Tenders pages
		search_paths = [
			"/",  # Main portal
		]
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Egypt E-Tenders from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Egypt E-Tenders."""
		# Extract title (may be in Arabic or English)
