"""
Namibia E-Procurement (Portal Ariel) Scraper
=============================================

Scrapes tender opportunities from Namibia's Central Procurement Board
e-procurement portal (Portal Ariel).

URL: https://eprocurement.gov.na
URL: https://www.cpb.org.na

Notes:
- Official portal managed by Central Procurement Board of Namibia (CPBN)
- Governed by Public Procurement Act of 2015 (Act No. 15 of 2015)
- English language portal
- Features bid adverts, awards, and contract notices
...
class NamibiaScraper(PlaywrightScraper):
	"""Scraper for Namibia E-Procurement Portal.

	Namibia's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "namibia_cpbn"
	source_name = "Namibia CPBN"
	base_url = "https://eprocurement.gov.na"
	...
	default_wait_for = "table tbody tr, .tender-item, .bid-item"

	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Namibia E-Procurement for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Namibia E-Procurement pages
		search_paths = [
			"/",  # Main portal
		]
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Namibia E-Procurement from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Namibia E-Procurement."""
		# Extract title
