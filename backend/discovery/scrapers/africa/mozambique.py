"""
Mozambique E-GP (GEPRES) Scraper
================================

Scrapes tender opportunities from Mozambique's e-Government Procurement
platform (GEPRES - Gestão de Empresas Públicas e Recursos do Estado).

URL: https://concursos.co.mz

Notes:
- Portal managed by Ministry of Finance
- Launched April 2025 for full digital procurement cycle
- Portuguese language portal
- Offers tender notices, contract awards, expressions of interest
- Covers planning, tendering, submission, awards, contract execution
...
class MozambiqueScraper(PlaywrightScraper):
	"""Scraper for Mozambique E-GP Portal.

	Mozambique's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "mozambique_egp"
	source_name = "Mozambique E-GP"
	base_url = "https://www.concursos.co.mz"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Mozambique E-GP for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Mozambique E-GP pages - Portuguese language
		search_paths = [
			"/en/homepage/",  # English homepage
			"/",  # Portuguese homepage
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching Mozambique E-GP from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Mozambique E-GP."""
		# Extract title (Portuguese: Descrição, Objeto)
