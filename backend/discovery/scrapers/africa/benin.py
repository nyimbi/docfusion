"""
Benin SIGMaP Scraper
====================

Scrapes tender opportunities from Benin's e-procurement system.

URL: https://sigmap.bj

Notes:
- Official portal managed by the Government of Benin
- French language portal
- SIGMaP (Système Intégré de Gestion des Marchés Publics)
- Governed by public procurement code
...
class BeninScraper(PlaywrightScraper):
	"""Scraper for Benin SIGMaP Portal.

	Benin's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "benin_sigmap"
	source_name = "Benin SIGMaP"
	base_url = "https://sigmap.bj"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Benin SIGMaP for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Benin SIGMaP pages - French language
		search_paths = [
			"/",  # Main portal
		]
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Benin SIGMaP from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Benin SIGMaP."""
