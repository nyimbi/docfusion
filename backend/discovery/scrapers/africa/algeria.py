"""
Algeria Baosem Scraper
======================

Scrapes tender opportunities from Algeria's E-Procurement Platform.

URL: https://baosem.dz

Notes:
- Official portal managed by Algerian Government
- Arabic/French language portal
- Baosem Platform for public procurement
...
class AlgeriaScraper(PlaywrightScraper):
	"""Scraper for Algeria Baosem Portal.

	Algeria's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "algeria_baosem"
	source_name = "Algeria Baosem"
	base_url = "https://baosem.dz"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Algeria Baosem for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Algeria Baosem pages - French/Arabic language
		search_paths = [
			"/",  # Main portal
		]
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Algeria Baosem from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Algeria Baosem."""
