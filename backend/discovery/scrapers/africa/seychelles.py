"""
Seychelles National Tender Board Scraper
========================================

Scrapes tender opportunities from Seychelles National Tender Board
and Procurement Oversight Unit portal.

URL: https://www.ntb.sc/tenders
URL: https://www.pou.gov.sc

Notes:
- Official portal managed by National Tender Board (NTB)
- Governed by Public Procurement Act of 2008 and Regulations of 2014
- English language portal
- Features 2-stage tender process for certain procurements
- Thresholds: >SCR 850,000 for NTB, SCR 250,000-850,000 for Procurement Committee
...
class SeychellesScraper(PlaywrightScraper):
	"""Scraper for Seychelles NTB Portal.

	Seychelles' portal uses JavaScript for dynamic content loading.
	"""

	source_id = "seychelles_ntb"
	source_name = "Seychelles NTB"
	base_url = "https://www.ntb.sc"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Seychelles NTB for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Seychelles NTB pages
		search_paths = [
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Seychelles NTB from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Seychelles NTB."""
		# Extract title
