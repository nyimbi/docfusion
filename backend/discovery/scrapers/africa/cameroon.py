"""
Cameroon ARMP/COLEPS E-Procurement Scraper
==========================================

Scrapes tender opportunities from Cameroon's Public Procurement Regulatory
Agency (ARMP - Agence de Régulation des Marchés Publics) portal.

URL: https://armp.cm
URL: https://marchespublics.cm (COLEPS platform)

Notes:
- Official portal managed by ARMP
- COLEPS (Cameroon Online E-Procurement System) launched 2018
- French language portal with English support
- Offers tender notices, contract awards, expressions of interest
- Over 7 million visitors tracked
...
class CameroonScraper(PlaywrightScraper):
	"""Scraper for Cameroon ARMP/COLEPS Portal.

	Cameroon's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "cameroon_armp"
	source_name = "Cameroon ARMP"
	base_url = "https://armp.cm"
	...
	default_wait_for = "table tbody tr, .tender-item, .appel-offre, .notice-item"

	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Cameroon ARMP for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Cameroon ARMP pages
		search_paths = [
			"/",  # Main portal with tender listings
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching Cameroon ARMP from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Cameroon ARMP."""
		# Extract title
