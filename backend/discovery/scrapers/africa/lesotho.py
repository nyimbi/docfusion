"""
Lesotho Public Procurement Scraper
==================================

Scrapes tender opportunities from Lesotho's government procurement system.

URL: https://tenderwatch.co.ls (Ponaletso tender portal)
URL: https://www.gov.ls/documents/tenders/ (Government portal)

Notes:
- Governed by Public Procurement Act 2023
- English language portal
- TenderWatch portal (Ponaletso) for comprehensive listings
...
class LesothoScraper(PlaywrightScraper):
	"""Scraper for Lesotho TenderWatch Portal.

	Lesotho's portal uses JavaScript for dynamic content loading.
	"""

	source_id = "lesotho_tenders"
	source_name = "Lesotho Tenders"
	base_url = "https://tenderwatch.co.ls"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape Lesotho TenderWatch for tender opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		# Lesotho TenderWatch pages
		search_paths = [
			"/",  # Main portal
		]
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching Lesotho TenderWatch from {path}")
	...
	def _parse_tender_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single tender item from Lesotho TenderWatch."""
		# Extract title
