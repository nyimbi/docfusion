"""
JICA (Japan International Cooperation Agency) Scraper
======================================================

Scrapes procurement opportunities from JICA.

URL: https://www.jica.go.jp/english/our_work/procurement/index.html

Notes:
- JICA is Japan's official development assistance agency
- Procurement for consulting services, goods, and works
- Focuses on technical cooperation and ODA loans
- Covers Asia, Africa, Latin America, and Middle East
...
class JICAScraper(BaseScraper):
	"""Scraper for JICA procurement opportunities."""

	source_id = "jica"
	source_name = "Japan International Cooperation Agency"
	base_url = "https://www.jica.go.jp"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape JICA procurement opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		search_paths = [
			"/english/our_work/procurement/index.html",
			"/english/our_work/procurement/consulting/index.html",
			"/english/our_work/procurement/goods/index.html",
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching JICA from {path}")
				html = await self.fetch_page(path)
				soup = self.parse_html(html)

				items = soup.select(".procurement-item, .notice-item, tbody tr, .link-list li")
	...
	def _parse_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single procurement item from JICA."""
		title_elem = item.select_one("td:first-child a, .title a, a, td a, h4 a")
		if not title_elem:
			return None
