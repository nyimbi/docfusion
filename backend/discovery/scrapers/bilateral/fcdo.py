"""
FCDO (Foreign, Commonwealth & Development Office) UK Scraper
============================================================

Scrapes procurement opportunities from UK FCDO (formerly DFID).

URL: https://www.gov.uk/government/organisations/foreign-commonwealth-development-office

Notes:
- FCDO is the UK's diplomatic and development ministry
- Formed from merger of FCO and DFID in 2020
- Procurement through Contracts Finder and FCDO procurement portal
- Focuses on poverty reduction and humanitarian response
...
class FCDOUKScraper(BaseScraper):
	"""Scraper for FCDO UK procurement opportunities."""

	source_id = "fcdo_uk"
	source_name = "Foreign, Commonwealth & Development Office"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape FCDO procurement opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		search_paths = [
			"/government/organisations/foreign-commonwealth-development-office",
			"/search/contracts?organisations[]=foreign-commonwealth-development-office",
			"/government/organisations/department-for-international-development",
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching FCDO from {path}")
				html = await self.fetch_page(path)
				soup = self.parse_html(html)

				items = soup.select(".gem-c-search-result, .document-row, tbody tr, .procurement-item")
	...
	def _parse_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single procurement item from FCDO."""
		title_elem = item.select_one("a, .gem-c-link, .title a, td:first-child a")
		if not title_elem:
			return None
		...
		country_elem = item.select_one(".country, .location, .gem-c-metadata")
		country = self.clean_text(country_elem.get_text()) if country_elem else None

		deadline_elem = item.select_one(".deadline, .closing-date, .gem-c-metadata time")
		deadline = None
		if deadline_elem:
			deadline_text = self.clean_text(deadline_elem.get_text())
			deadline = self.parse_date(deadline_text)

		ref_elem = item.select_one(".reference, .contract-reference")
