"""
USAID (United States Agency for International Development) Scraper
==================================================================

Scrapes procurement opportunities from USAID.

URL: https://www.usaid.gov/work-with-us/procurement

Notes:
- USAID is the lead US government agency for development assistance
- Procurement notices for consulting, goods, and works
- Focuses on developing countries worldwide
- Uses SAM.gov for contracts, Grants.gov for grants
- Business opportunities at: https://www.usaid.gov/business-opportunities
...
class USAIDScraper(BaseScraper):
	"""Scraper for USAID procurement opportunities."""

	source_id = "usaid"
	source_name = "United States Agency for International Development"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape USAID procurement opportunities."""
		...
		search_paths = [
			...
			"/work-with-us/procurement/solicitations",
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching USAID from {path}")
				html = await self.fetch_page(path)
				soup = self.parse_html(html)

				items = soup.select(".views-row, .procurement-item, tbody tr, .solicitation-item")
	...
	def _parse_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single procurement item from USAID."""
		title_elem = item.select_one("td:first-child a, .title a, a, td a, .field-content a")
		if not title_elem:
			return None
		...
		country_elem = item.select_one("td:nth-child(2), .country, .field--name-field-country")
		...
		ref_elem = item.select_one("td:nth-child(2), .solicitation-number, .reference")
