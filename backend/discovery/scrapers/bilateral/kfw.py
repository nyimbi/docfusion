"""
KfW (Kreditanstalt für Wiederaufbau) Scraper
============================================

Scrapes procurement opportunities from KfW Development Bank.

URL: https://www.kfw.de/KfW-Group.html

Notes:
- KfW is Germany's development bank
- Procurement for consulting, goods, and works
- Focuses on financing and promotional programs
- KfW IPEX for export and project finance
...
class KfWScraper(BaseScraper):
	"""Scraper for KfW procurement opportunities."""

	source_id = "kfw"
	source_name = "KfW Development Bank"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape KfW procurement opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		search_paths = [
			"/KfW-Group/Newsroom/Aktuelles-Listenansicht/Projekt-Finanzierung-Ausschreibungen.html",
			"/Development-Aid/Topics-Sectors",
			"/procurement",
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching KfW from {path}")
				html = await self.fetch_page(path)
				soup = self.parse_html(html)

				items = soup.select(".tx-listelement-listelement .list-item, tbody tr, .procurement-item")
	...
	def _parse_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single procurement item from KfW."""
		title_elem = item.select_one("td:first-child a, .title a, a, td a, .link-item a")
		if not title_elem:
			return None
