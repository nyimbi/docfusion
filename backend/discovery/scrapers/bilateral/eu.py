"""
EU DEVCO/INTPA (European Union Development Cooperation) Scraper
===============================================================

Scrapes procurement opportunities from European Union development portals.

URL: https://international-partnerships.ec.europa.eu/

Notes:
- EU INTPA (formerly DEVCO) manages EU development cooperation
- Procurement through EU Tenders Electronic Daily (TED)
- EuropeAid for external action funding
- Focuses on ACP countries, neighborhood, and thematic programs
...
class EUScraper(BaseScraper):
	"""Scraper for EU development cooperation procurement opportunities."""

	source_id = "eu_devco"
	source_name = "European Union Development Cooperation"
	base_url = "https://international-partnerships.ec.europa.eu"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape EU development procurement opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		search_paths = [
			"/funding-and-grants/funding-opportunities",
			"/news-and-events/funding-opportunities",
			"/partnerships/funding",
		]

		for path in search_paths:
			try:
				self._log_info(f"Fetching EU DEVCO from {path}")
				html = await self.fetch_page(path)
				soup = self.parse_html(html)

				items = soup.select(".view-content .views-row, .ecl-list-item, tbody tr, .procurement-item")
	...
	def _parse_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single procurement item from EU DEVCO."""
		title_elem = item.select_one("td:first-child a, .title a, a, td a, .ecl-link")
		if not title_elem:
			return None
		...
		country_elem = item.select_one("td:nth-child(2), .country, .region, .ecl-content-block")
