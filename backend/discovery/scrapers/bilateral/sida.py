"""
SIDA (Swedish International Development Cooperation Agency) Scraper
===================================================================

Scrapes procurement opportunities from SIDA.
...
class SIDAScraper(BaseScraper):
	"""Scraper for SIDA procurement opportunities."""

	source_id = "sida"
	source_name = "Swedish International Development Cooperation Agency"
	base_url = "https://www.sida.se"
	...
	async def scrape(self) -> list[ScrapedOpportunity]:
		"""Scrape SIDA procurement opportunities."""
		opportunities: list[ScrapedOpportunity] = []
		seen_ids: set[str] = set()

		search_paths = [
			"/en/partners/procurement",
			"/en/procurement",
			"/en/work-with-us",
		]
		...
		for path in search_paths:
			try:
				self._log_info(f"Fetching SIDA from {path}")
	...
	def _parse_item(self, item: Any) -> ScrapedOpportunity | None:
		"""Parse a single procurement item from SIDA."""
