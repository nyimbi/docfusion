...
- Current live database snapshot: 4,167 total opportunities; opportunity type counts are 1,850 `rfp`, 2,098 tender/TENDER, 100 `eoi`, 42 `grant`, and 77 `other`; 1,492 selected source documents; 860 selected documents downloaded; 627 selected documents still queued under the attempts cap; 810 RFP documents with extracted text; 50,192,232 extracted text characters; 5,647 extracted `rfp_requirements`; and 6 queued parse jobs remaining.
...
- Rerun the targeted acquisition campaigns after removing the low-yield UN Women and Global Fund configured-source pages; do not spend parser capacity on the 6 queued award/report jobs created before that removal unless they are explicitly needed for cleanup/audit.
