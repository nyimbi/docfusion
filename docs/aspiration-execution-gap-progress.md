...
- Expanded the default broad discovery query profile with UN Women, UNESCO, Global Fund, UNICEF supply, and World Bank EOI coverage, plus kept UNESCO as a configured source.
...
- Removed the UN Women and Global Fund pages from configured-source defaults after live rerun evidence showed UN Women is award-PDF-heavy and Global Fund returned no tender-like records; their search-query coverage remains in the broad profile.
- Added global public-sector, global regional, and donor/NGO acquisition campaigns to the aggressive runner, covering SAM.gov/USAID/grants.gov, UK Contracts Finder, CanadaBuys, AusTender, New Zealand GETS, non-Africa regional searches, and donor/NGO site searches through the same Google/DuckDuckGo/Bing/Brave fanout.
- Expanded scheduled/default discovery with the same global public-sector source URLs and regional/document search queries so routine discovery does not remain Africa-only after the aggressive runner changes.
- Shared the low-value procurement document filter between discovery source-document seeding and queued parser selection, so award/report/procurement-plan PDFs are skipped before they consume either source-document or parser capacity.
...
- Live source-document drains:
  ...
  - UNDP dry-run `source_doc_intake_undp_detail_50_dry_20260530T_acq8`: selected 50 at `SOURCE_DOCUMENT_INTAKE_LIMIT=50`, `SOURCE_DOCUMENT_INTAKE_MAX_PER_HOST=50`, and `SOURCE_DOCUMENT_INTAKE_FILL_MAX_PER_HOST=50`, proving the per-host cap lift can fill a larger proven single-host batch.
...
- Narrow post-filter UN/multilateral rerun `aggressive_rfp_acquisition_un_postfilter_20260530T_acq13`: completed 1 campaign, processed 122 records, imported 0 new opportunities, updated 122, created 40 source-document rows, downloaded 4 source documents, and recorded 1 warning. The run exposed compact `ContractAwards` filenames and an empty Global Fund configured page, so the low-value pattern was tightened again and both pages were removed from configured-source defaults/campaigns.
- Global expansion live run `aggressive_rfp_acquisition_global_expansion_20260530T0956`: completed 3 new campaigns, failed 0, processed 34 discovery records, imported 19 new opportunities, updated 15, and recorded 123 source/search warnings. The new `global_public_sector` campaign produced all 34 accepted records; `global_regional_search` and `donor_ngo_search` produced no accepted records under the current SearXNG degradation, so they need source-specific routes or query tuning instead of blind repeated fanout.
- Parser queue dry-run `rfp_parse_queue_skip_low_value_dry_20260530b`: selected only 2 real solicitation PDFs and skipped 4 compact UN Women `ContractAwardsAbove...` award-summary candidates, proving the shared low-value filter now protects parser capacity.
- Parser queue live run `rfp_parse_queue_global_expansion_real_20260530`: selected 2 real queued solicitation PDFs, skipped 4 award-summary candidates, completed 2 parser jobs, failed 0, and extracted 23 requirements.
- Search reliability slice `6da147f8`: changed duckduckgo-only fanout to try direct DuckDuckGo HTML before public SearXNG fallback instances. Focused tests passed, typecheck passed, and a live duckduckgo-only probe recovered 10 results from `html.duckduckgo.com` after `search.lindela.io` reported DuckDuckGo HTTP connection errors, without spending public SearXNG fallback requests first.
- Current live database snapshot: 4,186 total opportunities; opportunity type counts are 1,859 `rfp`, 2,102 tender/TENDER, and 100 `eoi`; 1,492 selected source documents; 860 selected documents downloaded; 794 selected source documents with extracted text; 810 RFP documents with extracted text; 804 completed RFP parser jobs; 5,670 extracted `rfp_requirements`; and 4 queued parser jobs remaining, all known low-value award-summary jobs skipped by the default queue processor.
- Current remaining selected queue by source: Configured Source Scrape 290 queued, Rwanda UMUCYO 48, UN Procurement 43, UNICEF Supply Division 30, Ghana GHANEPS 28, African Development Bank 26, NeST Tanzania 25, UNDP 22, Kenya PPIP 21, Zambia ZPPA 20, Caribbean Development Bank 13, SearXNG 12, IOM 11, SAM.gov 11, World Bank 11, South Africa eTenders 6, African Union 4, asa_ind_0072 4, Asian Infrastructure Investment Bank 1, and EBRD 1.

Remaining after this slice:
- Do not spend parser capacity on the 4 remaining queued UN Women award-summary jobs unless they are explicitly needed for cleanup/audit; the default queue processor now skips them.
- Treat the new global public-sector campaign as live-proven and keep it in the aggressive rotation.
- Treat the new donor/NGO and global regional search campaigns as coverage scaffolding, not yet productive live sources; improve with source-specific detail/document extraction or refined high-intent queries before spending many repeated runs there.
- Investigate remaining `search.lindela.io` SearXNG engine degradation and public fallback rate limits; DuckDuckGo-only searches now have a direct-first recovery path, but Google/Bing/Brave degradation still depends on SearXNG/public fallback health.
...
- Third dry-run `source_doc_intake_umucyo_detail_batch3_dry_20260530T0302` selected 20 additional per-tender UMUCYO detail pages from the remaining Rwanda queue.
- Third live intake `source_doc_intake_umucyo_detail_batch3_live_20260530T0303` selected 20, downloaded 20, failed 0, skipped 0, completed 20 parse jobs, timed out 0 parse jobs, and extracted 240 requirements.
- The third UMUCYO batch used `local_html_text` for every selected detail page; no Docling fallback was needed.
- Updated live database verification after the third UMUCYO detail batch: Rwanda UMUCYO has 36 downloaded RFP documents, 186,883 extracted source-document text characters, and 389 extracted `rfp_requirements`.
- Updated global live database snapshot: 3,947 total opportunities, 1,800 RFP-typed opportunities, 347 RFP documents with extracted text, 31,693,125 extracted text characters, and 2,647 extracted `rfp_requirements`.
