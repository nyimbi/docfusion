...
# Or explicitly allow this process to launch CloakBrowser locally:
# CLOAKBROWSER_LOCAL_LAUNCH="1"
# CLOAKBROWSER_CACHE_DIR="/var/lib/docfusion/cloakbrowser"
# CLOAKBROWSER_EXTRA_ARGS="--disable-dev-shm-usage"
```
...
Source-document recovery uses progressively heavier acquisition paths when a direct public document URL fails: SearXNG candidate discovery, Firecrawl, the browser scraper, CloakBrowser when configured, and finally a public reader fallback for public landing pages that block server-side fetches. CloakBrowser is disabled unless one of `CLOAKBROWSER_CDP_URL`, `CLOAKBROWSER_WS_ENDPOINT`, or `CLOAKBROWSER_REMOTE_DEBUGGING_URL` is set to a live CDP WebSocket endpoint, or `CLOAKBROWSER_LOCAL_LAUNCH=1` explicitly allows the app process to launch CloakBrowser locally. Keep CDP endpoints private because they can control a real browser session. Local launch may download a Chromium binary on first use; set `CLOAKBROWSER_CACHE_DIR` to a writable persistent directory so the app does not repeatedly download the binary. Root-run Linux deployments automatically add `--no-sandbox` / `--disable-setuid-sandbox`; add comma-separated `CLOAKBROWSER_EXTRA_ARGS` only for operational launch flags such as `--disable-dev-shm-usage`. The reader fallback is controlled by `SOURCE_DOCUMENT_READER_FALLBACK_PREFIX`, defaulting to the current `r.jina.ai` reader prefix, and can be disabled with `SOURCE_DOCUMENT_READER_FALLBACKS=0`. Reader-recovered pages are stored as HTML source surrogates with `reader_landing_page_html` provenance so operators can distinguish them from direct PDF downloads. Direct source downloads keep public-host and DNS pinning checks even when a known procurement host has an incomplete TLS chain; `tenders.go.ke` and `nrf.ac.za` are allowed narrowly by default, and additional hosts can be added with comma-separated `SOURCE_DOCUMENT_INVALID_TLS_HOSTS` values only after live failure evidence.

## Aggressive Africa / Global South Acquisition

`npm run rfp:acquire` defaults to the Africa and Global South collection lane when `AGGRESSIVE_RFP_ACQUISITION_GROUPS` is unset. The default priority order is `africa_national`, `development_banks`, `donor_ngo_search`, `global_regional_search`, `global_south_marketplaces`, `un_multilateral`, `high_intent_search`, and `africa_global_south_document_search`. This keeps unattended acquisition focused on African national portals, development-bank tenders, donor/NGO implementers, regional Global South sources, DGMarket-style marketplaces, and direct RFP/PDF searches instead of spending default capacity on high-income public-sector portals.

Set `AGGRESSIVE_RFP_ACQUISITION_GROUPS=all` to run every campaign, including `global_public_sector` and the broad `document_search` campaign. Set a comma-separated group list to run an explicit subset in the order given. Source API parsers honor `AGGRESSIVE_RFP_ACQUISITION_SOURCE_SCRAPE_LIMIT` up to the service hard cap, so increasing it for World Bank or similar source-backed runs now broadens the source API request instead of only trimming an already-small default response.
