...
| P0-B RFP intake parse lifecycle | JTBD-011, JTBD-012, JTBD-072 | Upload/parse surfaces now expose status polling, governed failed-parse remediation, and optional server-side Linode E3 object storage; security scan, SSE, notifications, and full exception queue remain future work. | Added parse workflow transitions around `rfpDocuments` and `rfpParsingJobs`; parse API retries failed documents through the workflow action; upload route can store RFP binaries in Linode E3 via server-side SigV4 because E3 has no browser CORS. | Updated `RFPUploader` to show parsing progress, terminal parse errors, and parse retry. | No migration; uses existing document/job `metadata`; stores `s3://bucket/key` and storage receipt metadata when Linode E3 env vars are configured. | Retry/reject/manual extraction/cancel states, advisory lock, retry attempt history, terminal success/failure metadata history, and storage receipt metadata implemented for pilot workflow. | `npm test -- rfp-parse-workflow.test.ts linode-e3.test.ts`; combined workflow regression set; `npx tsc --noEmit --pretty false`; targeted `npm run lint -- --file ...`; live Linode E3 PUT/GET smoke check through ignored frontend env. | Verified | TBD | 8 focused RFP/storage tests pass; 19-test combined workflow regression passes; TypeScript passes; targeted lint passes; live upload/download against `mansa` bucket passes. Security scan, SSE/notification/exception dashboard remain future work. Push pending commit. |
