...
export const PLATFORM_PROOF_SCENARIOS: ProofScenarioDefinition[] = [
	...
	},
	{
		...
		requiresLiveServices: false,
		description: "Validates AI governance, LiteLLM provider behavior, deterministic opportunity summaries, deterministic quality and document analysis, document generation auth and auto-fill, content governance workflows, snippet provenance, and placeholder resolution.",
		command: {
			command: "npm",
			args: ["test", "--", "ai-governance.test.ts", "opportunity-ai-scope.test.ts", "quality-assessment.test.ts", "document-analysis-scope.test.ts", "document-analysis-auth.test.ts", "document-generation-auth.test.ts", "content-governance-workflow.test.ts", "snippet-expansion-provenance.test.ts", "snippet-placeholder-resolution.test.ts", "litellm-provider.test.ts"],
		},
		expectedArtifacts: ["vitest:ai-governance", "vitest:opportunity-ai-scope", "vitest:quality-assessment", "vitest:document-analysis-scope", "vitest:document-analysis-auth", "vitest:document-generation-auth", "vitest:content-governance-workflow", "vitest:snippet-expansion-provenance", "vitest:snippet-placeholder-resolution", "vitest:litellm-provider"],
	},
	{
	...
	},
	{
		...
		command: {
			...
			args: [
				...
				"live-pursuit-handoff-action-state.test.ts",
		...
		expectedArtifacts: [
			...
			"vitest:live-pursuit-handoff-action-state",
	...
	{
		id: "live-world-bank-response-readiness",
		wave: 9,
		title: "Live World Bank response readiness proof",
		kind: "live-safe",
		proofTargets: ["F-001", "F-005", "F-008", "F-020"],
		requiresLiveServices: true,
		description: "Finds a live World Bank procurement notice, extracts solicitation text from the public notice API, and verifies concrete Datacraft response package draft artifacts.",
		command: {
			command: "npx",
			args: ["tsx", "scripts/prove-live-opportunity-response-readiness.ts"],
			env: {
				LIVE_RESPONSE_READINESS_SOURCE_KIND: "world_bank",
				LIVE_RESPONSE_READINESS_SOURCE_URL: "https://projects.worldbank.org/en/projects-operations/procurement",
				LIVE_RESPONSE_READINESS_PROOF_PREFIX: "live_world_bank_response_readiness",
			},
		},
		expectedArtifacts: [
			".omx/state/platform-live-opportunity-response-readiness-evidence.md",
