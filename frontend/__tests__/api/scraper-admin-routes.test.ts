...
vi.mock("@/lib/scrapers/run-stats", () => scraperRunsMock);
...
beforeEach(() => {
	...
	scraperRunsMock.getScraperStatsForPeriod.mockResolvedValue({
		totalRuns: 0,
		successfulRuns: 0,
		failedRuns: 0,
		partialRuns: 0,
...
describe("scraper admin routes", () => {
	...
	it("denies proposal managers", async () => {
		...
		expect(scraperRunsMock.getScraperStatsForPeriod).not.toHaveBeenCalled();
	});
	...
	it("allows operations sessions and configured API keys", async () => {
		...
		expect(scraperRunsMock.getScraperStatsForPeriod).toHaveBeenCalledTimes(1);
