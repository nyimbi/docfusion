...
afterEach(() => {
	...
	if (originalGoogleDirectFallbacks === undefined) {
		delete process.env.GOOGLE_DIRECT_FALLBACKS;
	} else {
...
describe("SearXNG client configuration", () => {
	...
	it("uses searx.space fallback when the primary returns zero results without explicit engine errors", async () => {
		process.env.SEARXNG_URL = "https://primary.example";
		process.env.SEARXNG_SPACE_INSTANCES_URL = "https://searx.space/data/instances.json";
		process.env.SEARXNG_PUBLIC_FALLBACK_LIMIT = "1";
		process.env.GOOGLE_DIRECT_FIRST_FALLBACKS = "0";
		const fetchMock = vi.fn()
			...
			.mockResolvedValueOnce(new Response(JSON.stringify({
				...
				results: [{
					...
					url: "https://buyer.example/zero-recovered",
					content: "Request for proposals",
					engine: "google",
					score: 2,
				}],
		...
		expect(result).toMatchObject({
			sourceInstance: "https://healthy.example",
			fallbackFrom: "https://primary.example",
			fallbackReason: expect.stringContaining("returned no results"),
			results: [expect.objectContaining({ title: "Fallback Zero Result RFP" })],
		});
		expect(fetchMock).toHaveBeenCalledTimes(3);
		expect(fetchMock.mock.calls[1][0]).toBe("https://searx.space/data/instances.json");
		expect(new URL(fetchMock.mock.calls[2][0] as string).origin).toBe("https://healthy.example");
	...
	it("uses searx.space fallback when default-engine primary results report degraded fanout", async () => {
		process.env.SEARXNG_URL = "https://primary.example";
		process.env.SEARXNG_SPACE_INSTANCES_URL = "https://searx.space/data/instances.json";
		process.env.SEARXNG_PUBLIC_FALLBACK_LIMIT = "1";
		...
		const result = await searchSearxng("afdb procurement");

		expect(result).toMatchObject({
			sourceInstance: "https://primary.example",
			sourceInstances: ["https://primary.example", "https://healthy.example"],
			fallbackFrom: "https://primary.example",
			fallbackReason: expect.stringContaining("default engines"),
			number_of_results: 2,
		});
	...
	it("uses searx.space fallback when requested engine filtering returns only other engines", async () => {
		process.env.SEARXNG_URL = "https://primary.example";
		process.env.SEARXNG_SPACE_INSTANCES_URL = "https://searx.space/data/instances.json";
		process.env.SEARXNG_PUBLIC_FALLBACK_LIMIT = "1";
		process.env.GOOGLE_DIRECT_FIRST_FALLBACKS = "0";
		...
		expect(result).toMatchObject({
			...
			sourceInstances: ["https://primary.example", "https://healthy.example"],
			fallbackFrom: "https://primary.example",
			fallbackReason: expect.stringContaining("none from requested engines google"),
			number_of_results: 2,
		});
		expect(result.results.map((item) => item.title)).toEqual([
			"Primary Bing RFP",
	...
	it("temporarily suppresses public fallback instances after throttle failures", async () => {
		...
		process.env.SEARXNG_SPACE_INSTANCES_URL = "https://searx.space/data/instances.json";
		process.env.SEARXNG_PUBLIC_FALLBACK_LIMIT = "1";
		process.env.GOOGLE_DIRECT_FALLBACKS = "0";
	...
	it("falls through to Google HTML when Google engine fanout and searx.space have no usable results", async () => {
		process.env.SEARXNG_URL = "https://primary.example";
		process.env.SEARXNG_SPACE_INSTANCES_URL = "https://searx.space/data/instances.json";
		process.env.SEARXNG_PUBLIC_FALLBACK_LIMIT = "1";
		process.env.GOOGLE_DIRECT_FIRST_FALLBACKS = "0";
		const fetchMock = vi.fn()
			...
			.mockResolvedValueOnce(new Response(`
				...
				<html>
					<body>
						<div class="g">
							<a href="/url?q=https%3A%2F%2Fbuyer.example%2Fgoogle-rfp&amp;sa=U">Google RFP &amp; Tender</a>
		...
		expect(result).toMatchObject({
			...
			fallbackFrom: "https://primary.example",
			fallbackReason: expect.stringContaining("recovered with Google HTML"),
			number_of_results: 2,
		});
		expect(result.results.map((item) => item.url)).toEqual([
			"https://buyer.example/primary",
			"https://buyer.example/google-rfp",
		]);
		expect(result.results[1]).toMatchObject({
			title: "Google RFP & Tender",
			engine: "google",
		});
		expect(fetchMock).toHaveBeenCalledTimes(4);
	...
	it("uses direct DuckDuckGo before public SearXNG fanout for mixed-engine searches", async () => {
		...
		const result = await searchSearxng("rfp", { engines: ["google", "duckduckgo", "bing"] });
	...
	it("keeps default public SearXNG fanout narrow unless explicitly widened", async () => {
		process.env.SEARXNG_URL = "https://primary.example";
		process.env.SEARXNG_SPACE_INSTANCES_URL = "https://searx.space/data/instances.json";
		process.env.DUCKDUCKGO_DIRECT_FIRST_FALLBACKS = "0";
		process.env.GOOGLE_DIRECT_FALLBACKS = "0";
			...
			.mockResolvedValueOnce(new Response(JSON.stringify({
				instances: {
					...
					"https://slow.example/": {
						...
						git_url: "https://github.com/searxng/searxng",
						http: { status_code: 200 },
						timing: { search: { success_percentage: 100, all: { median: 0.9 } } },
					},
				},
			}), { status: 200 }))
		...
		expect(fetchMock).toHaveBeenCalledTimes(4);
		expect(new URL(fetchMock.mock.calls[2][0] as string).origin).toBe("https://fast.example");
		expect(new URL(fetchMock.mock.calls[3][0] as string).origin).toBe("https://medium.example");
	});
