export const LOW_VALUE_PROCUREMENT_DOCUMENT_LINK_REGEX_SOURCE = String.raw`(?:\bannual[-_\s]?report\b|\bcontract[-_\s]?awards?\b|\bcontractawards?(?:above)?|\bawards?(?:[-_\s]?above|above)|\baward[-_\s]?notice\b|\bprocurement[-_\s]?plan\b|\bvendor[-_\s]?profile\b|\bsupplier[-_\s]?(?:code|conduct)\b|\bcode[-_\s]?of[-_\s]?conduct\b|\bfinancial[-_\s]?(?:statement|report)\b|\baudit[-_\s]?report\b|\bnewsletter\b|\bpress[-_\s]?release\b|\bprivacy[-_\s]?notice\b|\bterms[-_\s]?of[-_\s]?use\b)`;
