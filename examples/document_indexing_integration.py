...
class DocumentEvent:
	...
	def __init__(self, event_type: DocumentEventType, document_id: str, 
	             title: str = None, content: str = None, metadata: Dict[str, Any] = None):
...
class DocumentService:
	...
	async def create_document(self, title: str, content: str, author_id: str, 
	                         document_type: str = "proposal", tags: list = None) -> str:
	...
	async def update_document(self, document_id: str, title: str = None, 
	                         content: str = None, tags: list = None) -> bool:
	...
	async def delete_document(self, document_id: str) -> bool:
		...
		self.logger.info(f"Deleted document {document_id} and removed from search index")
		return True
	...
	async def _remove_from_search_index(self, document_id: str):
		...
		try:
			...
			self.logger.error(f"Failed to remove document {document_id} from search: {e}")
	
	async def _send_webhook_event(self, event_type: DocumentEventType, document: Dict[str, Any]):
		...
		try:
			...
			self.logger.error(f"Failed to send webhook for document {document['id']}: {e}")
...
class SearchIndexManager:
	...
	async def backup_index(self, backup_path: str):
		...
		self.logger.info(f"Index backup to {backup_path} - implementation depends on backend")
	...
	async def rebuild_index_from_database(self, document_service: DocumentService):
		...
		try:
			...
			self.logger.warning("Rebuilding search index - this will clear existing data")
			...
			self.logger.info(f"Rebuilt search index with {len(all_documents)} documents")
