...
class PerformanceCollector:
	...
	def __init__(self, max_metrics_per_type: int = 10000):
		...
		self.metrics: Dict[MetricType, deque] = defaultdict(lambda: deque(maxlen=max_metrics_per_type))
	...
	async def record_metric(self, metric: PerformanceMetric):
		...
		try:
			...
			if metric.endpoint:
				endpoint_key = f"{metric.method}:{metric.endpoint}" if metric.method else metric.endpoint
	...
	async def record_error_metric(self, endpoint: str, method: str, error_type: str,
	                             user_id: str = None):
		"""Record error metric"""
		metric = PerformanceMetric(
	...
	async def get_metrics_summary(self, metric_type: MetricType, 
		...
		try:
			...
			summary = {
				...
				'percentiles': {
					'p50': statistics.quantiles(values, n=2)[0] if len(values) >= 2 else values[0],
	...
	async def get_endpoint_performance(self, endpoint: str, duration_minutes: int = 60) -> Dict[str, Any]:
		...
		try:
			...
			error_count = len([m for m in recent_metrics if m.metric_type == MetricType.ERROR_RATE])
			...
			return {
				...
				'throughput_per_minute': len(recent_metrics) / duration_minutes if duration_minutes > 0 else 0
			}
...
class AlertManager:
	...
	async def create_alert(self, level: AlertLevel, metric_type: MetricType,
	                      message: str, value: float, threshold: float,
	                      endpoint: str = None, metadata: Dict[str, Any] = None) -> Alert:
	...
	async def _check_thresholds(self):
		...
		for threshold in self.thresholds:
			try:
				...
				current_value = None
				if threshold.metric_type in [MetricType.RESPONSE_TIME, MetricType.CPU_USAGE, 
				                           MetricType.MEMORY_USAGE, MetricType.DISK_USAGE]:
					current_value = summary.get('mean', 0)
				elif threshold.metric_type == MetricType.ERROR_RATE:
					# Calculate error rate from recent metrics
					cutoff_time = datetime.utcnow() - timedelta(minutes=duration_minutes)
					recent_errors = [
						m for m in self.collector.metrics[MetricType.ERROR_RATE]
						if m.timestamp >= cutoff_time
					]
					total_requests = summary['count']
					error_count = len(recent_errors)
					current_value = error_count / total_requests if total_requests > 0 else 0
				...
				if current_value >= threshold.critical_threshold:
					if alert_key not in self.active_alerts:
						await self.create_alert(
							...
							metadata={'duration_minutes': duration_minutes, 'samples': summary['count']}
						)
				...
				self.logger.error(f"Threshold check failed for {threshold.metric_type}: {e}")
...
class PerformanceMonitor:
	...
	async def _log_alert_handler(self, alert: Alert):
		...
		self.logger.log(log_level, f"PERFORMANCE ALERT [{alert.level.upper()}]: {alert.message}")
	
	async def record_request(self, endpoint: str, method: str, response_time: float,
	                        status_code: int, user_id: str = None, metadata: Dict[str, Any] = None):
	...
	async def record_error(self, endpoint: str, method: str, error_type: str, user_id: str = None):
		"""Record API error"""
	...
	async def record_websocket_connections(self, connection_count: int):
		...
		await self.collector.record_metric(PerformanceMetric(
	...
	async def get_performance_summary(self, duration_minutes: int = 60) -> Dict[str, Any]:
		...
		try:
			...
			for metric_type in MetricType:
				metric_summary = await self.collector.get_metrics_summary(metric_type, duration_minutes)
	...
	async def get_top_endpoints(self, limit: int = 10, duration_minutes: int = 60) -> List[Dict[str, Any]]:
		"""Get top performing/problematic endpoints"""
		try:
			endpoint_stats = []
			
			for endpoint in self.collector.endpoint_metrics.keys():
				stats = await self.collector.get_endpoint_performance(endpoint, duration_minutes)
				if stats.get('total_requests', 0) > 0:
					endpoint_stats.append(stats)
			
			# Sort by various metrics
			by_requests = sorted(endpoint_stats, key=lambda x: x.get('total_requests', 0), reverse=True)[:limit]
			by_errors = sorted(endpoint_stats, key=lambda x: x.get('error_rate', 0), reverse=True)[:limit]
			by_response_time = sorted(endpoint_stats, key=lambda x: x.get('avg_response_time', 0), reverse=True)[:limit]
