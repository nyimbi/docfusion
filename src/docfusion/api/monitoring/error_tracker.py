...
class ErrorCapture:
	...
	def capture_exception(
		...
		error_signature = f"{exc_type.__name__}:{str(exc_value)}:{file_path}:{line_number}"
...
class ErrorAggregator:
	...
	def __init__(self, max_events: int = 10000, max_summaries: int = 1000):
		...
		self.errors_by_user: Dict[str, List[str]] = defaultdict(list)  # user_id -> error_ids
		self.errors_by_endpoint: Dict[str, List[str]] = defaultdict(list)  # endpoint -> error_ids
		...
		try:
			...
			if error_event.error_hash in self.error_summaries:
				...
				summary = ErrorSummary(
					...
					affected_users=set([error_event.context.user_id]) if error_event.context.user_id else set(),
					affected_endpoints=set([error_event.context.endpoint]) if error_event.context.endpoint else set()
				)
				...
				if len(self.error_summaries) > self.max_summaries:
					oldest_hash = min(self.error_summaries.keys(), 
					                 key=lambda h: self.error_summaries[h].first_seen)
					del self.error_summaries[oldest_hash]
			
			# Update indexes
			if error_event.context.user_id:
				self.errors_by_user[error_event.context.user_id].append(error_event.error_id)
			
			if error_event.context.endpoint:
				self.errors_by_endpoint[error_event.context.endpoint].append(error_event.error_id)
	...
	def get_error_stats(self, hours: int = 24) -> Dict[str, Any]:
		...
		# Get top errors
		top_errors = []
		for error_hash, count in sorted(error_counts.items(), key=lambda x: x[1], reverse=True)[:10]:
	...
	def get_error_trends(self, hours: int = 24, bucket_minutes: int = 60) -> List[Dict[str, Any]]:
		...
		for error in recent_errors:
			bucket_index = int((error.timestamp - cutoff_time).total_seconds() // (bucket_minutes * 60))
...
class ErrorTracker:
	...
	async def track_error(
		...
		try:
			...
			# Log error
			self.logger.error(f"Error tracked: {error_event.error_id} - {error_event.message}")
			...
			try:
				...
				asyncio.create_task(
					self.error_tracker.track_error(
						record.exc_info[1],
						context,
						ErrorSeverity.HIGH if record.levelno >= logging.CRITICAL else ErrorSeverity.MEDIUM,
						category
					)
				)
			except Exception:
