...
class APIMonitor:
	...
	def __init__(self):
		...
		self.rate_limit_violations: Dict[str, List[datetime]] = {}  # user_id -> timestamps
	...
	async def finish_request_tracking(
		...
		self.logger.debug(f"Request {request_id} completed: {status_code} in {response_time_ms:.2f}ms")
	...
	def _find_existing_error(
		...
		for error in reversed(self.error_metrics):
			...
			if (error.error_type == error_type and
				error.error_message == error_message and
				...
				try:
					...
					if health_check.status == HealthStatus.UNHEALTHY:
						await self._create_alert(
							...
							"health_check",
							{'service': service_name, 'response_time': response_time_ms}
						)
	...
	async def _check_system_health(self):
		...
		try:
			...
			system_health = HealthCheck(
				service_name="system",
				status=HealthStatus.HEALTHY if memory_usage < 0.8 and disk_usage < 0.8 else HealthStatus.DEGRADED,
	...
	async def _check_performance_alerts(self, metric: RequestMetric):
		...
		# Error rate alert
		if (self.current_stats.total_requests > 100 and  # Only alert after some requests
			self.current_stats.error_rate > self.config['alert_thresholds']['error_rate']):
			await self._create_alert(
				AlertLevel.ERROR,
				f"High Error Rate",
	...
	async def get_performance_stats(
		...
		filtered_metrics = [
			...
			(not endpoint or m.endpoint == endpoint)
		]
		...
		successful_requests = len([m for m in filtered_metrics if 200 <= m.status_code < 400])
		...
		total_bytes = sum(m.request_size_bytes + m.response_size_bytes for m in filtered_metrics)
		throughput_mb_per_second = (total_bytes / (1024 * 1024)) / time_span if time_span > 0 else 0
	...
	async def get_endpoint_analytics(
		...
		for endpoint, metrics in endpoint_metrics.items():
			...
			avg_response_time = sum(m.response_time_ms for m in metrics) / total if metrics else 0
			...
			endpoint_stats[endpoint] = {
				...
				'max_response_time': max(m.response_time_ms for m in metrics) if metrics else 0,
				'min_response_time': min(m.response_time_ms for m in metrics) if metrics else 0
			}
	...
	async def get_error_analytics(
		...
		return {
			...
			'most_common_errors': sorted(
				[(error_type, data['count']) for error_type, data in error_types.items()],
				key=lambda x: x[1],
				reverse=True
			)[:10]
		}
	...
	async def get_user_analytics(
		...
		for user_id, activity in user_activity.items():
			...
			activity['error_rate'] = activity['failed_requests'] / activity['total_requests'] if activity['total_requests'] > 0 else 0
		
		return {
			'total_active_users': len(user_activity),
			'user_activity': user_activity,
			'most_active_users': sorted(
				[(user_id, data['total_requests']) for user_id, data in user_activity.items()],
				key=lambda x: x[1],
				reverse=True
			)[:10]
		}
	...
	async def get_overall_health(self) -> Dict[str, Any]:
		...
		for check in recent_checks:
			...
			if service not in service_statuses:
				...
				elif check.status == HealthStatus.DEGRADED and service_statuses[service] != HealthStatus.UNHEALTHY:
		...
		return {
			...
			'healthy_services': len([s for s in service_statuses.values() if s == HealthStatus.HEALTHY]),
			'degraded_services': len([s for s in service_statuses.values() if s == HealthStatus.DEGRADED]),
			'unhealthy_services': len([s for s in service_statuses.values() if s == HealthStatus.UNHEALTHY]),
	...
	async def _aggregate_metrics(self):
		...
		# Calculate throughput
		total_bytes = sum(
			m.request_size_bytes + m.response_size_bytes
			for m in recent_requests
		)
		self.current_stats.throughput_mb_per_second = (total_bytes / (1024 * 1024)) / 60.0
		
		self.logger.debug(f"Metrics aggregated: {self.current_stats.requests_per_second:.1f} RPS, "
	...
	async def _cleanup_old_data(self):
		...
		for user_id in list(self.rate_limit_violations.keys()):
			self.rate_limit_violations[user_id] = [
				ts for ts in self.rate_limit_violations[user_id] if ts > rate_limit_cutoff
			]
			if not self.rate_limit_violations[user_id]:
				del self.rate_limit_violations[user_id]
		
		self.logger.info(f"Cleaned up {old_count - len(self.request_metrics)} old request metrics")
