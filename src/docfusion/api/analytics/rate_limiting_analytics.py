...
class RateLimitStore:
    ...
    async def get_state(self, scope_key: str, rule_id: str) -> RateLimitState:
        ...
        if key not in self.states:
            self.states[key] = RateLimitState(scope_key=scope_key, rule_id=rule_id)
...
class RateLimitEngine:
    ...
    def get_applicable_rules(
        self, request: Request, user_id: str = None, user_tier: str = None
    ) -> List[RateLimitRule]:
    ...
    def _get_scope_key(
        self, rule: RateLimitRule, request: Request, user_id: str = None
    ) -> str:
...
class RateLimitAnalyzer:
    ...
    async def generate_analytics(self, hours: int = 24) -> RateLimitAnalytics:
        ...
        try:
            ...
            recent_events = [
                e for e in self.engine.events if start_time <= e.timestamp <= end_time
            ]

            if not recent_events:
                return RateLimitAnalytics(period_start=start_time, period_end=end_time)
            ...
            top_blocked_ips = [
                ...
                for ip, count in sorted(
                    ip_blocks.items(), key=lambda x: x[1], reverse=True
                )[:10]
            ]
            ...
            top_blocked_users = [
                ...
                for user_id, count in sorted(
                    user_blocks.items(), key=lambda x: x[1], reverse=True
                )[:10]
            ]
            ...
            top_blocked_endpoints = [
                ...
                for endpoint, count in sorted(
                    endpoint_blocks.items(), key=lambda x: x[1], reverse=True
                )[:10]
            ]
            ...
            for rule_id, stats in rule_stats.items():
                effectiveness = (
                    (stats["blocked"] + stats["delayed"]) / stats["total"]
                    if stats["total"] > 0
                    else 0.0
                )
...
class RateLimitMiddleware:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        try:
            ...
            # Check rate limits
            is_limited, event = await self.engine.check_rate_limit(
                    ...
                    return JSONResponse(
                        ...
                        headers={
                            ...
                            "X-RateLimit-Remaining": str(
                                max(0, event.limit - event.current_count)
                            ),
    ...
    def _register_endpoints(self):
        ...
        async def get_rate_limit_status(request: Request):
            ...
            try:
                ...
                status = await self.engine.get_rate_limit_status(
                    request, user_id, user_tier
                )
                return status
            except Exception as e:
