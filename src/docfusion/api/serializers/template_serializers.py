...
class TemplateCreateRequest(BaseModel):
	...
	def validate_content_placeholders(cls, v, values):
		...
		if undefined_placeholders:
			raise ValueError(f"Undefined placeholders in content: {', '.join(undefined_placeholders)}")
...
class TemplateUpdateRequest(BaseModel):
	...
	access_level: Optional[TemplateAccessLevel] = Field(
		None,
		description="Updated template access level"
	)
	
	output_formats: Optional[List[OutputFormat]] = Field(
		None,
		description="Updated supported output formats"
	)
	
	is_active: Optional[bool] = Field(
		None,
		description="Updated template active status"
	)
...
class TemplatePreviewRequest(BaseModel):
	...
	sample_data: Optional[Dict[str, Any]] = Field(
		None,
		description="Sample data to populate template fields"
	)
	
	output_format: OutputFormat = Field(
		OutputFormat.HTML,
		description="Output format for preview"
	)
...
class TemplateResponse(BaseModel):
	...
	usage_count: Optional[int] = Field(None, description="Number of times template was used")
...
class TemplateListResponse(BaseModel):
	...
	access_levels: Optional[List[str]] = Field(None, description="Available access levels")
...
class TemplateValidationResult(BaseModel):
	...
	field_validation: Optional[Dict[str, List[str]]] = Field(
		None,
		description="Field-specific validation results"
	)
	
	placeholder_analysis: Optional[Dict[str, Any]] = Field(
		None,
		description="Analysis of template placeholders"
	)
...
class TemplateUsageAnalytics(BaseModel):
	...
	top_users: List[Dict[str, Union[str, int]]] = Field(
		default_factory=list,
		description="Top users of this template"
	)
	...
	popular_field_values: Dict[str, List[str]] = Field(
		...
		description="Most common values for each field"
	)
...
class BulkTemplateOperationResult(BaseModel):
	...
	total_requested: int = Field(..., description="Total templates requested for operation")
	...
	processed_ids: List[str] = Field(
		default_factory=list,
		description="Successfully processed template IDs"
	)
...
class TemplateImportResult(BaseModel):
	...
	total_requested: int = Field(..., description="Total templates requested for import")
	...
	imported_template_ids: List[str] = Field(
		default_factory=list,
		description="IDs of successfully imported templates"
	)
...
class TemplateExportRequest(BaseModel):
	...
	include_content: bool = Field(
		True,
		description="Whether to include template content in export"
	)
