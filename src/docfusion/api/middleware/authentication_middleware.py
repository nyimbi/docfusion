...
from ...security import SecurityManager, UserAuthentication, APIAuthentication
...
class AuthenticationMiddleware:
	...
	async def _validate_session(
		...
		try:
			...
			# Get user permissions
			permissions = await self.security.rbac.get_user_permissions(session['user_id'])
...
def initialize_auth_middleware(security_manager: SecurityManager) -> AuthenticationMiddleware:
...
async def get_api_key_user(api_key: str = Depends(APIKeyHeader(name="X-API-Key"))) -> Dict[str, Any]:
	...
	def decorator(func):
		async def wrapper(*args, current_user: Dict[str, Any] = Depends(get_current_user), **kwargs):
			...
			for role in user_permissions.get('roles', []):
				...
				if any(perm['name'] == required_permission for perm in role_permissions):
					has_permission = True
					break
...
# Factory function
def create_authentication_middleware(security_manager: SecurityManager) -> AuthenticationMiddleware:
