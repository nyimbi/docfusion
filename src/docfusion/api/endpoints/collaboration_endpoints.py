...
class CommentCreateRequest(BaseModel):
    ...
    selection_start: Optional[int] = Field(
        None, description="Text selection start position"
    )
    selection_end: Optional[int] = Field(
        None, description="Text selection end position"
    )
...
class CommentResponse(BaseModel):
    ...
    replies: List["CommentResponse"] = Field(
        default_factory=list, description="Reply comments"
    )
    resolved_by: Optional[str] = Field(
        None, description="User who resolved the comment"
    )
...
class SuggestionCreateRequest(BaseModel):
    ...
    description: str = Field(
        ..., min_length=1, max_length=1000, description="Suggestion description"
    )
    original_text: Optional[str] = Field(
        None, description="Original text being suggested to change"
    )
    suggested_text: Optional[str] = Field(
        None, description="Suggested replacement text"
    )
    selection_start: Optional[int] = Field(
        None, description="Text selection start position"
    )
...
class SuggestionResponse(BaseModel):
    ...
    reviewed_by: Optional[str] = Field(
        None, description="User who reviewed the suggestion"
    )
...
class CollaborationEndpoints:
    ...
    def __init__(
        ...
        ] = {}  # document_id -> list of collaborators
        self.activity_log: Dict[
            str, List[Dict[str, Any]]
        ] = {}  # document_id -> list of activities
    ...
    def _register_endpoints(self):
        ...
        @self.router.get(
            "/documents/{document_id}/comments", response_model=List[CommentResponse]
        )
        async def get_document_comments(
            ...
            include_resolved: bool = Query(
                True, description="Include resolved comments"
            ),
            current_user: Dict[str, Any] = Depends(get_current_user),
        ):
        ...
        @self.router.post(
            "/comments/{comment_id}/resolve", response_model=Dict[str, Any]
        )
        ...
        @self.router.post(
            "/suggestions", response_model=SuggestionResponse, status_code=201
        )
        ...
        async def get_document_suggestions(
            document_id: str = PathParam(..., description="Document ID"),
            status: Optional[SuggestionStatus] = Query(
                None, description="Filter by status"
            ),
        ...
        @self.router.post(
            "/suggestions/{suggestion_id}/accept", response_model=Dict[str, Any]
        )
        ...
        @self.router.post(
            "/suggestions/{suggestion_id}/reject", response_model=Dict[str, Any]
        )
        async def reject_suggestion(
            ...
            """Reject a suggestion"""
            return await self.reject_suggestion_handler(
                suggestion_id, reason, current_user
            )
        ...
        @self.router.post(
            "/documents/{document_id}/collaborators", response_model=Dict[str, Any]
        )
        async def add_collaborator(
            ...
            return await self.add_collaborator_handler(
        ...
        async def get_document_collaborators(
            ...
            return await self.get_document_collaborators_handler(
        ...
        async def update_collaborator_permission(
            ...
            permission: CollaborationPermission = Query(
        ...
        @self.router.delete(
            "/documents/{document_id}/collaborators/{user_id}", status_code=204
        )
        ...
        @self.router.get(
            "/documents/{document_id}/activity", response_model=List[ActivityLogEntry]
        )
        async def get_document_activity(
            document_id: str = PathParam(..., description="Document ID"),
            limit: int = Query(50, ge=1, le=100, description="Maximum results"),
            since: Optional[datetime] = Query(
                None, description="Show activity since this timestamp"
            ),
            current_user: Dict[str, Any] = Depends(get_current_user),
        ):
            """Get activity log for a document"""
            return await self.get_document_activity_handler(
                document_id, limit, since, current_user
            )

        # ==================== COLLABORATION STATUS ====================

        @self.router.get(
            "/documents/{document_id}/status", response_model=Dict[str, Any]
        )
        async def get_collaboration_status(
            ...
            return await self.get_collaboration_status_handler(
                document_id, current_user
            )
    ...
    async def create_comment_handler(
        self, request: CommentCreateRequest, current_user: Dict[str, Any]
    ) -> CommentResponse:
        ...
        try:
            ...
            user_permission = self._get_user_collaboration_permission(
                request.document_id, user_id
            )
            if user_permission in [CollaborationPermission.VIEWER]:
                raise HTTPException(
                    status_code=403, detail="Comment permission required"
                )
            ...
            for comment in self.comments.values():
                if comment["document_id"] == document_id:
                    ...
                    if (
                        ...
                        and comment["status"] == CommentStatus.RESOLVED
                    ):
                        continue
    ...
    async def update_comment_handler(
        self, comment_id: str, content: str, current_user: Dict[str, Any]
    ) -> CommentResponse:
        ...
        try:
            ...
            if comment["author_id"] != user_id:
                raise HTTPException(
                    status_code=403, detail="Can only edit your own comments"
                )
    ...
    async def resolve_comment_handler(
        self, comment_id: str, current_user: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if user_permission in [
                CollaborationPermission.VIEWER,
                CollaborationPermission.COMMENTER,
            ]:
                raise HTTPException(
                    status_code=403,
                    detail="Editor permission required to resolve comments",
                )
    ...
    async def delete_comment_handler(
        ...
        try:
            ...
            if comment["author_id"] != user_id and user_permission not in [
                CollaborationPermission.OWNER,
                CollaborationPermission.EDITOR,
            ]:
                raise HTTPException(
                    status_code=403,
                    detail="Can only delete your own comments or need editor permission",
                )
            ...
            # Also delete replies
            replies_to_delete = [
                cid for cid, c in self.comments.items() if c["reply_to"] == comment_id
            ]
    ...
    async def create_suggestion_handler(
        self, request: SuggestionCreateRequest, current_user: Dict[str, Any]
    ) -> SuggestionResponse:
        ...
        try:
            ...
            if user_permission in [CollaborationPermission.VIEWER]:
                raise HTTPException(
                    status_code=403,
                    detail="Comment permission required for suggestions",
                )
    ...
    async def get_document_suggestions_handler(
        ...
        try:
            ...
            return [
                SuggestionResponse(**suggestion) for suggestion in document_suggestions
            ]
    ...
    async def accept_suggestion_handler(
        self, suggestion_id: str, current_user: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check document permission (need editor permission or higher)
            user_permission = self._get_user_collaboration_permission(
                suggestion["document_id"], user_id
            )
            if user_permission not in [
                CollaborationPermission.EDITOR,
                CollaborationPermission.OWNER,
            ]:
                raise HTTPException(
                    status_code=403,
                    detail="Editor permission required to accept suggestions",
                )
    ...
    async def reject_suggestion_handler(
        self, suggestion_id: str, reason: Optional[str], current_user: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if user_permission not in [
                ...
                raise HTTPException(
                    status_code=403,
                    detail="Editor permission required to reject suggestions",
                )
            ...
            await self._log_activity(
                ...
                current_user.get("username", "Unknown"),
                "suggestion_rejected",
                f"Rejected suggestion: {suggestion['title']}"
    ...
    async def delete_suggestion_handler(
        ...
        try:
            ...
            if suggestion["author_id"] != user_id and user_permission not in [
                CollaborationPermission.OWNER,
                CollaborationPermission.EDITOR,
            ]:
                raise HTTPException(
                    status_code=403,
                    detail="Can only delete your own suggestions or need editor permission",
                )
    ...
    async def add_collaborator_handler(
        ...
        try:
            ...
            if not permission_result["has_permission"]:
                raise HTTPException(
                    status_code=403, detail="Manage permission required"
                )
            ...
            existing = next(
                (
                    c
                    for c in self.collaborators[document_id]
            ...
            for collaborator in document_collaborators:
                if self.websocket_endpoints:
                    ...
                    collaborator["is_online"] = collaborator["user_id"] in getattr(
                        self.websocket_endpoints.connection_manager,
                        "active_connections",
                        {},
                    )
    ...
    async def update_collaborator_permission_handler(
        ...
        try:
            ...
            collaborator = next(
                (
                    c
                    for c in document_collaborators
                    if c["user_id"] == collaborator_user_id
                ),
                None,
            )
    ...
    async def remove_collaborator_handler(
        self, document_id: str, collaborator_user_id: str, current_user: Dict[str, Any]
    ):
    ...
    async def get_collaboration_status_handler(
        ...
        try:
            ...
            document_comments = [
                c for c in self.comments.values() if c["document_id"] == document_id
            ]
            document_suggestions = [
                s for s in self.suggestions.values() if s["document_id"] == document_id
            ]
            document_collaborators = self.collaborators.get(document_id, [])

            # Count by status
            open_comments = len(
                [c for c in document_comments if c["status"] == CommentStatus.OPEN]
            )
            resolved_comments = len(
                [c for c in document_comments if c["status"] == CommentStatus.RESOLVED]
            )

            pending_suggestions = len(
                [
                    s
                    for s in document_suggestions
            ...
            return {
                ...
                "can_comment": self._get_user_collaboration_permission(
                    document_id, user_id
                )
                != CollaborationPermission.VIEWER,
                "can_edit": self._get_user_collaboration_permission(
                    document_id, user_id
                )
                in [CollaborationPermission.EDITOR, CollaborationPermission.OWNER],
                "can_manage": self._get_user_collaboration_permission(
                    document_id, user_id
                )
                == CollaborationPermission.OWNER,
            }
    ...
    def _get_user_collaboration_permission(
        ...
        document_collaborators = self.collaborators.get(document_id, [])
        collaborator = next(
            (c for c in document_collaborators if c["user_id"] == user_id), None
        )
    ...
    async def _notify_collaborators(
        ...
        await self.websocket_endpoints.connection_manager.send_to_document_subscribers(
            document_id, event_type, data, exclude_user=exclude_user
        )
...
def create_collaboration_endpoints(
    ...
    return CollaborationEndpoints(
        storage_service, security_manager, websocket_endpoints
    )
