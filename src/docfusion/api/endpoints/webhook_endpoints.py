...
from fastapi import APIRouter, HTTPException, Depends, Query, Path as PathParam, Header, BackgroundTasks
...
class WebhookEvent(BaseModel):
	...
	resource_id: Optional[str] = Field(None, description="Resource ID associated with event")
...
class WebhookManager:
		...
		async def delivery_worker():
			while True:
				try:
					...
					task = asyncio.create_task(self._process_webhook_delivery(delivery_data))
	...
	async def update_webhook(
		...
		# Update fields
		allowed_updates = ['url', 'events', 'secret', 'active', 'description', 'headers', 'timeout', 'retry_count', 'metadata']
	...
	async def get_webhook(self, webhook_id: str, user_id: str) -> Optional[Dict[str, Any]]:
		"""Get webhook by ID"""
		if webhook_id not in self.webhooks:
			return None
	...
	async def trigger_webhook_event(self, event: WebhookEvent):
		...
		self.logger.debug(f"Queued event {event.event_id} for {len(target_webhooks)} webhooks")
		...
		try:
			...
			try:
				...
				if attempt < max_attempts:
					...
					self.logger.warning(f"Webhook delivery {delivery_id} failed (attempt {attempt}/{max_attempts}), retrying in {retry_delay}s: {e}")
					
					# Schedule retry
					asyncio.create_task(self._schedule_retry(delivery_data, retry_delay))
				else:
					# Final failure
					delivery['status'] = WebhookStatus.FAILED
					...
					if webhook['failure_count'] >= 10:
						...
						self.logger.warning(f"Disabled webhook {webhook['webhook_id']} due to excessive failures")
					
					# Update global stats
					self.stats['total_deliveries'] += 1
					self.stats['failed_deliveries'] += 1
					
					self.logger.error(f"Webhook delivery {delivery_id} permanently failed after {max_attempts} attempts: {e}")
		
		except Exception as e:
			self.logger.error(f"Webhook delivery processing error for {delivery_id}: {e}")
			if delivery_id in self.deliveries:
				self.deliveries[delivery_id]['status'] = WebhookStatus.FAILED
				self.deliveries[delivery_id]['error_message'] = f"Processing error: {str(e)}"
	...
	def _update_average_response_time(self, duration_ms: float):
		...
		if total_successful == 1:
			...
			# Moving average
			self.stats['average_response_time'] = ((current_avg * (total_successful - 1)) + duration_ms) / total_successful
...
class WebhookEndpoints:
	...
	def _register_endpoints(self):
		...
		@self.router.get("/{webhook_id}/deliveries", response_model=List[WebhookDeliveryResponse])
		async def get_webhook_deliveries(
			...
			return await self.get_webhook_deliveries_handler(webhook_id, limit, current_user)
		...
		async def trigger_event(
			event_type: WebhookEventType = Query(..., description="Event type to trigger"),
			data: Dict[str, Any] = {},
			current_user: Dict[str, Any] = Depends(get_current_user)
		):
	...
	async def update_webhook_handler(
		...
		try:
			...
			success = await self.webhook_manager.update_webhook(webhook_id, user_id, updates)
			if not success:
				raise HTTPException(status_code=404, detail="Webhook not found")
	...
	async def get_webhook_deliveries_handler(
		...
		try:
			...
			deliveries = await self.webhook_manager.get_webhook_deliveries(webhook_id, limit)
			
			responses = []
			for delivery in deliveries:
				responses.append(WebhookDeliveryResponse(
	...
	async def get_webhook_stats_handler(
		...
		try:
			...
			if auth_result.has_permission:
				...
				return {
					'system_stats': stats,
					'user_webhook_count': len(await self.webhook_manager.list_user_webhooks(user_id)),
					'permissions': 'full'
				}
			else:
				...
				user_stats = {
					...
					'user_total_deliveries': sum(w['total_deliveries'] for w in user_webhooks),
					'user_successful_deliveries': sum(w['successful_deliveries'] for w in user_webhooks),
					'permissions': 'limited'
				}
				return user_stats
