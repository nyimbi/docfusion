...
from ..middleware.authentication_middleware import get_current_user, get_api_key_user
...
class DateFilter(BaseModel):
	...
	relative: Optional[str] = Field(None, description="Relative date (e.g., '7d', '1m', '1y')")
...
class SearchFilters(BaseModel):
	...
	date_filters: Optional[List[DateFilter]] = Field(None, description="Date range filters")
	range_filters: Optional[List[RangeFilter]] = Field(None, description="Numeric range filters")
	facet_filters: Optional[List[FacetFilter]] = Field(None, description="Facet filters")
...
class SearchResultItem(BaseModel):
	...
	highlights: Optional[List[SearchHighlight]] = Field(None, description="Highlighted matches")
...
class SearchResponse(BaseModel):
	...
	suggestions: Optional[List[SearchSuggestion]] = Field(None, description="Search suggestions")
...
class SearchBackend(ABC):
	...
	@abstractmethod
	async def search(self, request: SearchRequest) -> Tuple[List[Tuple[str, float]], Dict[str, Any]]:
...
class MeiliSearchBackend(SearchBackend):
	...
	def __init__(self, host: str = "http://localhost:7700", api_key: Optional[str] = None, index_name: str = "documents"):
		self.host = host
	...
	async def _setup_index(self):
		...
		try:
			...
			await asyncio.get_event_loop().run_in_executor(
				...
				['title', 'content', 'metadata.tags', 'metadata.category', 'metadata.author']
			)
	...
	async def search(self, request: SearchRequest) -> Tuple[List[Tuple[str, float]], Dict[str, Any]]:
		...
		try:
			...
			query_params = {
				...
				'attributesToHighlight': ['title', 'content'] if request.highlight else [],
	...
	def _build_meilisearch_filters(self, filters: Optional[SearchFilters]) -> Optional[List[str]]:
		...
		if filters.authors:
			authors_filter = ' OR '.join([f'author = "{author}"' for author in filters.authors])
			filter_expressions.append(f'({authors_filter})')
		
		if filters.document_types:
			types_filter = ' OR '.join([f'type = "{doc_type}"' for doc_type in filters.document_types])
			filter_expressions.append(f'({types_filter})')
		
		if filters.categories:
			categories_filter = ' OR '.join([f'category = "{category}"' for category in filters.categories])
		...
		if filters.date_filters:
			for date_filter in filters.date_filters:
				if date_filter.from_date:
					filter_expressions.append(f'{date_filter.field} >= {date_filter.from_date.timestamp()}')
				if date_filter.to_date:
					filter_expressions.append(f'{date_filter.field} <= {date_filter.to_date.timestamp()}')
...
class OpenSearchBackend(SearchBackend):
	...
	def __init__(self, host: str = "localhost", port: int = 9200, 
	             ...
	             use_ssl: bool = False, index_name: str = "documents"):
		self.host = host
		self.port = port
		...
		if OpenSearch is None:
			raise ImportError("opensearch-py package is required for OpenSearch backend")
	...
	def _build_opensearch_filters(self, filters: Optional[SearchFilters]) -> List[Dict[str, Any]]:
		...
		if filters.authors:
			filter_clauses.append({
				"terms": {"metadata.author": filters.authors}
			})
		
		if filters.document_types:
			filter_clauses.append({
				"terms": {"metadata.type": filters.document_types}
			})
		
		if filters.categories:
			filter_clauses.append({
				"terms": {"metadata.category": filters.categories}
			})
		...
		if filters.date_filters:
			for date_filter in filters.date_filters:
				...
				if date_filter.from_date:
					range_filter["range"][f"metadata.{date_filter.field}"]["gte"] = date_filter.from_date.isoformat()
				if date_filter.to_date:
					range_filter["range"][f"metadata.{date_filter.field}"]["lte"] = date_filter.to_date.isoformat()
		...
		if filters.range_filters:
			for range_filter in filters.range_filters:
				...
				if range_filter.min_value is not None:
					range_clause["range"][f"metadata.{range_filter.field}"]["gte"] = range_filter.min_value
				if range_filter.max_value is not None:
					range_clause["range"][f"metadata.{range_filter.field}"]["lte"] = range_filter.max_value
...
class InMemorySearchBackend(SearchBackend):
	...
	async def search(self, request: SearchRequest) -> Tuple[List[Tuple[str, float]], Dict[str, Any]]:
		...
		# Sort and return
		results = [(doc_id, score) for doc_id, score in sorted(doc_scores.items(), key=lambda x: x[1], reverse=True)]
...
class SearchEngine:
	...
	def __init__(self, security_manager: SecurityManager, backend_type: str = "memory", backend_config: Optional[Dict[str, Any]] = None):
	...
	def _create_backend(self, backend_type: str, config: Dict[str, Any]) -> SearchBackend:
	...
	async def switch_backend(self, backend_type: str, config: Optional[Dict[str, Any]] = None):
	...
	async def search(
		...
		try:
			...
			filtered_docs = await self._apply_security_filter(matching_docs, user_id, context)
			...
			if request.include_debug:
				...
				response.debug_info = SearchDebugInfo(
					...
					total_docs_examined=backend_debug.get('total_hits', len(matching_docs)),
	...
	async def _apply_security_filter(
		...
		for doc_id, score in docs:
			try:
				...
				self.logger.warning(f"Permission check failed for document {doc_id}: {e}")
	...
	async def _calculate_facets(self, docs: List[Tuple[str, float]]) -> List[SearchFacet]:
	...
	def _update_average_query_time(self, query_time: float):
		...
		if total_searches == 1:
			...
			self.search_stats['average_query_time'] = ((current_avg * (total_searches - 1)) + query_time) / total_searches
...
class SearchEndpoints:
	...
	def _register_endpoints(self):
		...
		async def save_search(
			...
			return await self.save_search_handler(name, description, request, is_public, current_user)
		...
		async def execute_saved_search(
			...
			return await self.execute_saved_search_handler(search_id, page, per_page, current_user)
		...
		async def index_document(
			...
			return await self.index_document_handler(doc_id, title, content, metadata, current_user)
		
		@self.router.post("/backend/switch", response_model=Dict[str, Any])
		async def switch_backend(
			backend_type: str = Query(..., description="Backend type (memory, meilisearch, opensearch)"),
			config: Dict[str, Any] = {},
			current_user: Dict[str, Any] = Depends(get_current_user)
		):
	...
	async def save_search_handler(
		...
		try:
			...
			self.logger.info(f"Saved search {saved_search.search_id} created by user {user_id}")
			return saved_search
