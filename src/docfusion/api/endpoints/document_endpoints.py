...
from fastapi import APIRouter, HTTPException, Depends, Query, Path as PathParam, UploadFile, File
...
from ..serializers.document_serializers import (
	...
	DocumentListResponse, RenderRequest, DocumentHistoryResponse
)
...
class DocumentEndpoints:
	...
	def _register_endpoints(self):
		...
		async def get_document(
			...
			"""Get document by ID"""
			return await self.get_document_handler(document_id, include_content, current_user)
		...
		async def update_document(
			...
			return await self.update_document_handler(document_id, request, current_user)
		...
		async def render_document(
			...
			return await self.render_document_handler(document_id, request, current_user)
		
		@self.router.get("/{document_id}/history", response_model=DocumentHistoryResponse)
		async def get_document_history(
			...
			return await self.get_document_history_handler(document_id, limit, current_user)
		...
		async def share_document(
			...
			return await self.share_document_handler(
				document_id, target_user_id, target_email, 
				permission_level, expires_in_days, current_user
			)
		...
		async def upload_document(
			...
			extract_content: bool = Query(True, description="Extract and process content"),
			current_user: Dict[str, Any] = Depends(get_current_user)
		):
		...
		async def search_documents(
			...
			search_type: str = Query("hybrid", description="Search type: full_text, semantic, hybrid"),
			limit: int = Query(20, ge=1, le=100, description="Maximum results"),
			similarity_threshold: Optional[float] = Query(None, description="Similarity threshold for semantic search"),
			current_user: Dict[str, Any] = Depends(get_current_user)
		):
	...
	async def list_documents_handler(
		...
		try:
			...
			if tags:
				filtered_docs = [
					doc for doc in filtered_docs
					if any(tag in doc.get('metadata', {}).get('tags', []) for tag in tags)
				]
			...
			if search:
				...
				if search_result['success']:
					search_doc_ids = {result.document_id for result in search_result['results']}
					filtered_docs = [
						doc for doc in filtered_docs
	...
	async def update_document_handler(
		...
		try:
			...
			updated_doc = await self.get_document_handler(document_id, True, current_user)
			return updated_doc
