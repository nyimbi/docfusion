...
from ..serializers.template_serializers import (
	...
	TemplateListResponse, TemplatePreviewRequest, TemplateSearchRequest
)
...
class TemplateEndpoints:
	...
	def _register_endpoints(self):
		...
		async def get_template(
			...
			"""Get template by ID"""
			return await self.get_template_handler(template_id, include_content, current_user)
		...
		async def update_template(
			...
			return await self.update_template_handler(template_id, request, current_user)
		...
		async def preview_template(
			...
			return await self.preview_template_handler(template_id, request, current_user)
		...
		@self.router.post("/{template_id}/duplicate", response_model=TemplateResponse, status_code=201)
		async def duplicate_template(
			...
			return await self.duplicate_template_handler(template_id, new_name, current_user)
		...
		async def upload_template(
			...
			return await self.upload_template_handler(file, name, category, description, current_user)
	...
	async def update_template_handler(
		...
		try:
			...
			template_info = await self.get_template_handler(template_id, False, current_user)
			...
			metadata = {
				...
				"template_description": request.description or template_info.description,
				"template_version": "1.1",  # Increment version
				"template_fields": request.fields or template_info.fields,
				"template_access_level": request.access_level or template_info.access_level,
				"is_template": True,
