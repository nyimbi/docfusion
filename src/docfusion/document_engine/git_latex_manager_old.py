...
# Using custom uuid7str since uuid-extensions may not be available
def uuid7str() -> str:
...
"""
	...
	def _create_metadata_header(
		...
		return f"""% Block metadata: type={block_type}, priority=high, dependencies=[]
	...
	async def list_content_blocks(self) -> list[FileContentBlock]:
		...
		for tex_file in self.blocks_dir.rglob("*.tex"):
			try:
				...
				print(f"Error loading block {tex_file}: {e}")
...
class MockAIService:
	...
	async def enhance_latex_content(
		...
		return current_content + enhancement