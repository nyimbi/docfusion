...
class StructureRecognizer:
	...
	def _compile_patterns(self):
		...
		self.title_regexes = [re.compile(p, re.MULTILINE) for p in self.config['title_patterns']]
		self.heading_regexes = [re.compile(p, re.MULTILINE) for p in self.config['heading_patterns']]
		self.list_regexes = [re.compile(p, re.MULTILINE) for p in self.config['list_patterns']]
	...
	async def recognize_structure(
		...
		try:
			...
			self.logger.info(f"Structure recognized successfully, time: {result.processing_time:.2f}s")
	...
	async def _pattern_based_recognition(self, text: str) -> List[StructureElement]:
		...
		for line_idx, line in enumerate(lines):
			...
			# Check for headings
				...
				element = StructureElement(
					id=uuid7str(),
					structure_type=StructureType.HEADING if level == 1 else StructureType.SUBHEADING,
					text=line_stripped,
					level=level,
					start_char=start_char,
					end_char=end_char,
	...
	async def _line_based_analysis(self, text: str) -> List[StructureElement]:
		...
		for i, (line, stats) in enumerate(zip(lines, line_stats)):
			...
			# List items
				...
				confidence = 0.8
				level = stats['indent_level'] // 4  # Estimate level based on indentation
	...
	async def _indentation_analysis(self, text: str) -> List[StructureElement]:
		...
		if indent_levels:
			...
			indent_mapping = {indent: level for level, indent in enumerate(unique_indents)}
		else:
			indent_mapping = {}
	...
	async def _ai_enhanced_recognition(self, text: str) -> Tuple[List[StructureElement], Dict[str, Any]]:
		...
		try:
			...
			for chunk_idx, chunk in enumerate(chunks):
				chunk_elements, chunk_analysis = await self._analyze_chunk_structure(chunk, chunk_idx)
	...
	async def _analyze_chunk_structure(self, chunk: str, chunk_idx: int) -> Tuple[List[StructureElement], Dict[str, Any]]:
		...
		try:
			...
			if response.status_code == 200:
				...
				try:
					...
					elements = self._convert_ai_elements_to_structure_elements(ai_data.get('elements', []), chunk)
					analysis = {
...
{{
  ...
  "confidence": 0.8
	...
	def _convert_ai_elements_to_structure_elements(self, ai_elements: List[Dict[str, Any]], chunk: str) -> List[StructureElement]:
		...
		for ai_element in ai_elements:
			try:
				...
				element = StructureElement(
					...
					end_char=ai_element.get('end_char', len(ai_element.get('text', ''))),
	...
	def _parse_ai_text_response(self, response_text: str, chunk: str) -> List[StructureElement]:
	...
	def _merge_structure_elements(self, elements: List[StructureElement]) -> List[StructureElement]:
		...
		for element in elements:
			...
			for existing in merged:
				if (element.start_char >= existing.start_char and 
					element.start_char <= existing.end_char):
	...
	def _build_hierarchy(self, elements: List[StructureElement]) -> Dict[str, List[str]]:
	...
	def _create_outline(self, elements: List[StructureElement], hierarchy: Dict[str, List[str]]) -> List[Dict[str, Any]]:
	...
	def _calculate_structure_statistics(self, elements: List[StructureElement]) -> Dict[str, Any]:
...
{{
  ...
  "confidence": 0.8
	...
	def get_recognizer_info(self) -> Dict[str, Any]:
		...
		return {
			'supported_methods': ['pattern_matching', 'line_analysis', 'indentation_analysis', 'ai_enhancement'],
...
# Factory function
def create_structure_recognizer(config: Optional[Dict[str, Any]] = None) -> StructureRecognizer:
