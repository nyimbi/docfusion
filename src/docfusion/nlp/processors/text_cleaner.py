...
class OllamaTextCleaner:
	...
	async def clean_text(
		...
		try:
			...
			if (use_ai or self.config['use_ai_cleaning']) and len(cleaned_text) < self.config['max_text_length']:
				cleaned_text = await self._ai_enhanced_cleaning(cleaned_text, result)
			
			# Step 3: Final normalization
			cleaned_text = await self._final_normalization(cleaned_text, result, preserve_formatting)
			...
			self.logger.info(f"Text cleaned successfully, time: {result.processing_time:.2f}s")
		...
		try:
			response = await self.ollama_client.post("/api/generate", json={
				...
				"options": {
					...
					"top_p": 0.9,
					"num_predict": min(len(text) * 2, 8192)  # Reasonable response length
				}
			})
			...
			if response.status_code == 200:
				...
				if self._validate_ai_cleaned_text(text, cleaned_text):
					...
					result.warnings.append("AI cleaning validation failed, using original text")
...
Cleaned text:"""
	...
	def _validate_ai_cleaned_text(self, original: str, cleaned: str) -> bool:
		...
		suspicious_patterns = [
			r'as an ai', r'i cannot', r'i apologize',
			r'here is the cleaned text', r'cleaned version:',
			r'note:', r'disclaimer:'
		]
	...
	def _remove_headers_footers(self, text: str, result: CleaningResult) -> str:
		...
		for line in lines:
			...
			is_header = any(re.match(pattern, line_stripped, re.IGNORECASE) 
							for pattern in self.header_patterns)
			
			# Check for footer patterns
			is_footer = any(re.match(pattern, line_stripped, re.IGNORECASE) 
	...
	def _remove_page_numbers(self, text: str, result: CleaningResult) -> str:
		...
		for line in lines:
			...
			if any(re.match(pattern, line_stripped, re.IGNORECASE) for pattern in page_patterns):
				if len(line_stripped) < 20:  # Only remove short lines that look like page numbers
	...
	async def detect_document_structure(self, text: str) -> Dict[str, Any]:
		...
		if not self.config['ai_structure_detection'] or len(text) > self.config['max_text_length']:
			return {}
