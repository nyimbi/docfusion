...
class DocumentTokenizer:
	...
	def _initialize_nlp_components(self):
		...
		if self.config['use_spacy'] and HAS_SPACY_SUPPORT:
			try:
				...
				self.logger.warning(f"Could not load spaCy model '{self.config['spacy_model']}', trying fallback")
				try:
					# Try smaller model
	...
	async def tokenize_document(
		...
		try:
			...
			if self.nlp_model:
				await self._tokenize_with_spacy(text, result, preserve_sentences, include_entities)
			...
			self.logger.info(f"Document tokenized successfully, time: {result.processing_time:.2f}s")
	...
	async def _tokenize_with_spacy(
		...
		if preserve_sentences:
			...
			for sent in doc.sents:
				sentence_tokens = [
					...
					if tok.start_char >= sent.start_char and tok.end_char <= sent.end_char
				]
		...
		# Extract entities
		if (include_entities is None and self.config['include_entities']) or include_entities:
	...
	async def _extract_entities_spacy(self, doc, result: TokenizationResult):
		...
		for sentence in result.sentences:
			sentence.entities = [
				...
				if ent['start_char'] >= sentence.start_char and ent['end_char'] <= sentence.end_char
			]
	...
	def _calculate_statistics(self, result: TokenizationResult):
		...
		result.statistics = {
			...
			'word_count': len([tok for tok in tokens if tok.token_type == TokenType.WORD]),
			'punctuation_count': len([tok for tok in tokens if tok.token_type == TokenType.PUNCTUATION]),
			'number_count': len([tok for tok in tokens if tok.token_type == TokenType.NUMBER]),
			'stopword_count': len([tok for tok in tokens if tok.is_stop]),
			'entity_count': len(result.entities),
			'average_word_length': sum(len(tok.text) for tok in tokens if tok.token_type == TokenType.WORD) / max(len([tok for tok in tokens if tok.token_type == TokenType.WORD]), 1),
			'vocabulary_richness': len(set(tok.lemma.lower() for tok in tokens if tok.token_type == TokenType.WORD)) / max(len([tok for tok in tokens if tok.token_type == TokenType.WORD]), 1),
	...
	def _calculate_complexity_scores(self, result: TokenizationResult):
		...
		avg_sentence_length = sum(len(sent.tokens) for sent in sentences) / len(sentences)
		avg_word_length = sum(len(tok.text) for tok in tokens if tok.token_type == TokenType.WORD) / max(len([tok for tok in tokens if tok.token_type == TokenType.WORD]), 1)
		
		for sentence in sentences:
			# Simple complexity score based on length and vocabulary
			word_tokens = [tok for tok in sentence.tokens if tok.token_type == TokenType.WORD]
			if word_tokens:
				sentence.complexity_score = (
					len(word_tokens) / avg_sentence_length * 0.4 +
					sum(len(tok.text) for tok in word_tokens) / len(word_tokens) / avg_word_length * 0.3 +
					len(set(tok.lemma.lower() for tok in word_tokens)) / len(word_tokens) * 0.3
				)
	...
	async def extract_keywords(
		...
		result = await self.tokenize_document(text, preserve_sentences=False, include_entities=False)
		...
		for token in result.tokens:
			if (token.token_type == TokenType.WORD and 
				...
				len(token.text) >= self.config['min_token_length']):
...
# Factory function
def create_document_tokenizer(config: Optional[Dict[str, Any]] = None) -> DocumentTokenizer:
