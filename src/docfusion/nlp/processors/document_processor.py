...
class DocumentProcessor:
	...
	async def process_document(
		...
		try:
			...
			if file_size > self.config['max_file_size']:
				result.errors.append(f"File size ({file_size} bytes) exceeds maximum ({self.config['max_file_size']} bytes)")
				return result
			...
			self.logger.info(f"Processed document: {file_path.name}, success: {result.success}, time: {result.processing_time:.2f}s")
		...
		try:
			...
			try:
				...
				if pdf_reader.metadata:
					...
					if creation_date:
						result.metadata.creation_date = self._parse_pdf_date(creation_date)
				...
				for page_num, page in enumerate(pdf_reader.pages):
					if page_num >= self.config['max_pages_processed']:
						result.warnings.append(f"Stopped processing at page {page_num} (max limit reached)")
						break
					...
					try:
						...
						result.warnings.append(f"Failed to extract text from page {page_num + 1}: {e}")
			...
			with pdfplumber.open(pdf_file) as pdf:
				...
				for page_num, page in enumerate(pdf.pages):
					...
					try:
						...
						result.warnings.append(f"Failed to process page {page_num + 1}: {e}")
	...
	async def _process_html(self, content: bytes, result: ProcessingResult):
		...
		try:
			...
			try:
				...
				result.warnings.append(f"Encoding detection failed, used UTF-8 with error handling")
			...
			try:
				...
				for fallback_encoding in ['latin-1', 'cp1252', 'iso-8859-1']:
					try:
						...
						result.warnings.append(f"Encoding detection failed, used {fallback_encoding}")
						break
					except UnicodeDecodeError:
						continue
				else:
					...
					result.warnings.append("All encoding attempts failed, used UTF-8 with error handling")
	...
	async def _process_image_ocr(self, content: bytes, result: ProcessingResult):
		...
		try:
			...
			result.metadata.custom_properties['ocr_languages'] = self.config['ocr_languages']
...
# Factory function
def create_document_processor(config: Optional[Dict[str, Any]] = None) -> DocumentProcessor:
