...
class TopicProgression:
	...
	topic_transitions: List[Tuple[str, str, float]] = field(default_factory=list)  # (from_topic, to_topic, smoothness)
	topic_development_depth: Dict[str, float] = field(default_factory=dict)
	coherence_breaks: List[int] = field(default_factory=list)  # Sentence positions where coherence breaks
...
class CoherenceAnalyzer:
	...
	def __init__(
		...
		self.logger.info(f"CoherenceAnalyzer initialized with model: {self.llm_config.model} from centralized config")
	...
	def _initialize_patterns(self):
		...
		self.transition_patterns = {
			TransitionType.ADDITION: {
				...
				'similarly', 'likewise', 'equally', 'as well as', 'not only', 'along with'
			},
			TransitionType.CONTRAST: {
				'however', 'nevertheless', 'nonetheless', 'but', 'yet', 'although', 'though',
				'despite', 'in contrast', 'on the other hand', 'conversely', 'whereas', 'while'
			},
			TransitionType.CAUSE_EFFECT: {
				'therefore', 'thus', 'consequently', 'as a result', 'because', 'since',
				'due to', 'owing to', 'leads to', 'results in', 'causes', 'enables'
			},
			TransitionType.TEMPORAL: {
				'first', 'second', 'third', 'finally', 'then', 'next', 'subsequently',
				'previously', 'earlier', 'later', 'meanwhile', 'simultaneously', 'before', 'after'
			},
			TransitionType.COMPARISON: {
				'similarly', 'likewise', 'in comparison', 'compared to', 'like', 'unlike',
				'as opposed to', 'different from', 'in the same way', 'analogous to'
			},
			TransitionType.EMPHASIS: {
				'indeed', 'certainly', 'obviously', 'clearly', 'undoubtedly', 'without doubt',
				'most importantly', 'significantly', 'notably', 'particularly', 'especially'
			},
			TransitionType.CONCLUSION: {
				'in conclusion', 'to conclude', 'finally', 'in summary', 'to summarize',
				'overall', 'ultimately', 'in the end', 'as a result', 'therefore'
			},
			TransitionType.EXAMPLE: {
				'for example', 'for instance', 'such as', 'including', 'like', 'namely',
				'specifically', 'particularly', 'to illustrate', 'case in point'
			}
		}
		...
		self.reference_patterns = {
			'pronouns': {'it', 'they', 'them', 'this', 'that', 'these', 'those', 'he', 'she', 'we'},
			'demonstratives': {'this', 'that', 'these', 'those', 'such', 'aforementioned'},
			'definite_articles': {'the'}
		}
		
		# Argument indicator patterns
		self.argument_patterns = {
			'claims': {'argue', 'claim', 'assert', 'propose', 'suggest', 'contend', 'maintain'},
			'evidence': {'because', 'since', 'given that', 'evidence shows', 'studies indicate', 'research demonstrates'},
			'conclusions': {'therefore', 'thus', 'hence', 'consequently', 'as a result', 'it follows that'}
		}
	...
	def _compile_regex_patterns(self):
		...
		self.regex_patterns = {
			...
			'references': re.compile(r'\b(?:this|that|these|those|it|they|them)\b', re.IGNORECASE),
			'claims': re.compile(r'\b(?:argue|claim|assert|propose|suggest|contend|maintain)\b', re.IGNORECASE),
			'evidence_markers': re.compile(r'\b(?:because|since|given that|evidence|studies|research|data)\b', re.IGNORECASE)
		}
	...
	async def analyze_coherence(
		...
		try:
			...
			result.topic_progression = await self._analyze_topic_progression(sentences, paragraphs)
			
			# Step 3: Evaluate argument flow
			result.argument_flow = await self._analyze_argument_flow(sentences, paragraphs)
			
			# Step 4: Calculate coherence scores
			result.coherence_scores = await self._calculate_coherence_scores(
				sentences, result.transitions, result.topic_progression, result.argument_flow
			)
			...
			if use_ai and aiohttp:
				try:
					...
					result.structural_recommendations = await self._generate_structural_recommendations(result)
			...
			if not use_ai or not aiohttp:
				result.improvement_suggestions = self._generate_rule_based_suggestions(result)
				result.structural_recommendations = self._generate_rule_based_structural_recommendations(result)
			...
			self.logger.info(f"Coherence analysis completed, overall score: {result.coherence_scores.overall_score:.3f}")
	...
	async def _analyze_transitions(self, sentences: List[str]) -> List[TransitionAnalysis]:
				...
				for transition_type, transition_set in self.transition_patterns.items():
					for transition in transition_set:
						if phrase.startswith(transition) or transition in phrase:
							...
							analysis.effectiveness_score = self._calculate_transition_effectiveness(
								sentence, sentences, i, transition_type
							)
							
							# Check connection to previous sentence
							if i > 0:
								analysis.connects_to_previous = self._evaluates_connection(
							...
							analysis.clarity_contribution = min(analysis.effectiveness_score * 1.2, 1.0)
	...
	def _calculate_transition_effectiveness(
		self, sentence: str, all_sentences: List[str], position: int, transition_type: TransitionType
	) -> float:
		...
		if transition_type == TransitionType.CAUSE_EFFECT:
			if any(word in sentence_lower for word in ['result', 'effect', 'impact', 'outcome']):
				effectiveness += 0.2
		
		if transition_type == TransitionType.CONTRAST:
			if any(word in sentence_lower for word in ['different', 'opposite', 'unlike', 'alternative']):
				effectiveness += 0.2
		
		if transition_type == TransitionType.EXAMPLE:
			if any(word in sentence_lower for word in ['example', 'instance', 'case', 'illustration']):
				effectiveness += 0.2
		
		return min(effectiveness, 1.0)
	
	def _evaluates_connection(self, prev_sentence: str, current_sentence: str, transition_type: TransitionType) -> bool:
		...
		if transition_type == TransitionType.CONTRAST:
			...
			contrast_indicators = ['not', 'never', 'different', 'opposite', 'unlike', 'however']
		...
		if transition_type == TransitionType.CAUSE_EFFECT:
			...
			return (any(word in prev_lower for word in cause_words) or 
					any(word in curr_lower for word in effect_words))
	...
	def _strengthens_argument(self, sentence: str, transition_type: TransitionType) -> bool:
		...
		if any(pattern in sentence_lower for pattern in self.argument_patterns['evidence']):
			return True
		
		if any(pattern in sentence_lower for pattern in self.argument_patterns['conclusions']):
			return True
		
		return False
	
	async def _analyze_topic_progression(self, sentences: List[str], paragraphs: List[str]) -> TopicProgression:
		...
		try:
			...
			for i in range(1, len(sentences)):
				...
				prev_topics = self._get_sentence_topics(sentences[i-1], topic_keywords)
						...
						for curr_topic in current_topics:
							for prev_topic in prev_topics:
								if curr_topic != prev_topic:
									progression.topic_transitions.append((prev_topic, curr_topic, smoothness))
			
			# Calculate topic development depth
			for topic in progression.main_topics:
				topic_count = sum(1 for sentence in sentences if topic.lower() in sentence.lower())
				total_sentences = len(sentences)
				progression.topic_development_depth[topic] = topic_count / total_sentences if total_sentences > 0 else 0
			...
			for i in range(1, len(sentences)):
				...
				if current_topics and prev_topics:
					overlap = len(set(current_topics) & set(prev_topics))
					if overlap == 0 and len(current_topics) > 0 and len(prev_topics) > 0:
						progression.coherence_breaks.append(i)
			
			# Calculate progression metrics
			if len(progression.main_topics) > 0:
				avg_development = sum(progression.topic_development_depth.values()) / len(progression.topic_development_depth)
				progression.development_consistency = avg_development
				
				# Topic focus score (higher = more focused)
				max_development = max(progression.topic_development_depth.values()) if progression.topic_development_depth else 0
				progression.topic_focus_score = max_development
				
				# Narrative flow (fewer coherence breaks = better flow)
				break_penalty = len(progression.coherence_breaks) / len(sentences) if len(sentences) > 0 else 0
	...
	def _get_sentence_topics(self, sentence: str, topic_keywords: Dict[str, int]) -> List[str]:
	...
	async def _analyze_argument_flow(self, sentences: List[str], paragraphs: List[str]) -> ArgumentFlow:
		...
		try:
			...
			for sentence in sentences:
				sentence_lower = sentence.lower()
				if any(claim_word in sentence_lower for claim_word in self.argument_patterns['claims']):
					flow.main_claims.append(sentence.strip())
			
			# Identify supporting evidence
			for sentence in sentences:
				sentence_lower = sentence.lower()
				if any(evidence_word in sentence_lower for evidence_word in self.argument_patterns['evidence']):
					flow.supporting_evidence.append(sentence.strip())
			
			# Calculate claim support ratio
			if len(flow.main_claims) > 0:
				flow.claim_support_ratio = len(flow.supporting_evidence) / len(flow.main_claims)
			...
			contradiction_indicators = ['but not', 'however not', 'despite', 'although', 'contradicts']
			...
			if contradiction_count > len(sentences) * 0.2:  # More than 20% contradictory statements
				consistency_score -= 0.3
			
			flow.logical_consistency = max(consistency_score, 0.0)
			
			# Assess conclusion strength
			conclusion_indicators = ['therefore', 'thus', 'in conclusion', 'finally', 'ultimately']
			...
			elif any('problem' in sentence.lower() and 'solution' in sentence.lower() for sentence in sentences):
				flow.structure_type = ArgumentStructure.PROBLEM_SOLUTION
			elif any('cause' in sentence.lower() and 'effect' in sentence.lower() for sentence in sentences):
	...
	async def _calculate_coherence_scores(
		...
		try:
			...
			semantic_score = topic_progression.topic_focus_score * 0.6 + topic_progression.development_consistency * 0.4
			...
			if len(sentences) > 0:
				...
				avg_transition_effectiveness = sum(t.effectiveness_score for t in transitions) / len(transitions) if transitions else 0.5
				structural_score = min(transition_coverage * 2, 1.0) * 0.4 + avg_transition_effectiveness * 0.6
			else:
			...
			scores.reference_resolution = self._calculate_reference_resolution(sentences)
	...
	def _calculate_reference_resolution(self, sentences: List[str]) -> float:
		...
		for i in range(1, len(sentences)):
			...
			if references:
				...
				potential_antecedents = [word for word in prev_words if len(word) > 3 and word.isalpha()]
				
				if potential_antecedents:
					resolved_count += len(references)  # Assume resolved if potential antecedents exist
	...
	async def _identify_coherence_breaks(
		...
		try:
			...
			for break_position in topic_progression.coherence_breaks:
				...
				coherence_break.description = f"Abrupt topic change at sentence {break_position + 1}"
			...
			for i in range(1, len(sentences)):
				...
				if any(word in current_sentence for word in ['but', 'however', 'different', 'unlike']):
					# Check if proper transition is present
					has_transition = any(transition in current_sentence for transition in self.all_transitions)
					...
					if not has_transition:
						...
						coherence_break.description = f"Missing transition word at sentence {i + 1}"
		...
		output_format = """JSON format:
{
				...
				if response.status == 200:
					...
					try:
						...
						return {
							...
							"improvement_recommendations": ["Review AI analysis in raw_analysis field"]
						}
				else:
					raise Exception(f"Ollama API error: {response.status}")
	
	async def _generate_improvement_suggestions(self, text: str, result: CoherenceAnalysisResult) -> List[str]:
	...
	async def _generate_structural_recommendations(self, result: CoherenceAnalysisResult) -> List[str]:
		...
		recommendations.extend(self._generate_rule_based_structural_recommendations(result))
		
		return recommendations[:6]  # Limit to 6 recommendations
	
	def _generate_rule_based_suggestions(self, result: CoherenceAnalysisResult) -> List[str]:
		"""Generate rule-based improvement suggestions"""
		suggestions = []
		
		# Low lexical coherence
		if result.coherence_scores.lexical_score < 0.5:
			suggestions.append("Improve word choice consistency and reduce repetitive vocabulary")
		
		# Low semantic coherence
		if result.coherence_scores.semantic_score < 0.5:
			suggestions.append("Maintain focus on main topics and reduce tangential content")
		
		# Poor transitions
		if result.coherence_scores.transition_quality < 0.5:
			suggestions.append("Add more transitional phrases to connect ideas smoothly")
		
		# Weak argument flow
		if result.coherence_scores.argument_flow < 0.5:
			suggestions.append("Strengthen logical connections between claims and evidence")
		
		# Topic progression issues
		if result.topic_progression.narrative_flow < 0.5:
			suggestions.append("Organize content to follow a clearer logical progression")
		
		# Reference resolution problems
		if result.coherence_scores.reference_resolution < 0.6:
			suggestions.append("Clarify pronoun references and ensure antecedents are clear")
		
		return suggestions
	
	def _generate_rule_based_structural_recommendations(self, result: CoherenceAnalysisResult) -> List[str]:
		"""Generate rule-based structural recommendations"""
		recommendations = []
		
		# Few transitions
		if len(result.transitions) < len(result.topic_progression.main_topics):
			recommendations.append("Add paragraph-level transitions to improve flow between sections")
		
		# Many coherence breaks
		if len(result.coherence_breaks) > 3:
			recommendations.append("Address abrupt topic changes with better transitions")
		
		# Weak conclusion
		if result.argument_flow.conclusion_strength < 0.5:
			recommendations.append("Strengthen conclusion by clearly connecting to main arguments")
		
		# Poor claim support
		if result.argument_flow.claim_support_ratio < 0.5:
			recommendations.append("Provide more evidence and examples to support main claims")
		
		# Low topic focus
		if result.topic_progression.topic_focus_score < 0.4:
			recommendations.append("Focus content on fewer main topics for better coherence")
	...
	def _compile_coherence_statistics(self, result: CoherenceAnalysisResult, text: str) -> Dict[str, Any]:
	...
	def get_analyzer_info(self) -> Dict[str, Any]:
		...
		return {
			...
			'supported_coherence_types': [coherence_type.value for coherence_type in CoherenceType],
			'supported_transition_types': [transition_type.value for transition_type in TransitionType],
			'supported_argument_structures': [structure.value for structure in ArgumentStructure],
			'analysis_features': {
