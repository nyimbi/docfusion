...
try:
	from textstat import (
		...
		text_standard, syllable_count, sentence_count as textstat_sentence_count,
		lexicon_count, avg_sentence_length, difficult_words
	)
except ImportError:
...
try:
	from ..config.llm_config import LLMConfiguration, get_llm_config, LLMTask
	from ..prompting_strategies import (
		create_chain_of_thought_prompt, create_tree_of_thought_prompt, 
		filter_thinking_tags, PromptingStrategy
	)
...
class ReadabilityAnalysisResult:
	...
	reading_level: ReadingLevelAssessment = field(default_factory=ReadingLevelAssessment)
	accessibility: AccessibilityAssessment = field(default_factory=AccessibilityAssessment)
...
class ReadabilityAnalyzer:
	...
	def __init__(
		...
		if llm_config and HAS_ADVANCED_PROMPTING:
			...
			self.llm_config = type('MockConfig', (), {
		...
		self.enable_advanced_prompting = enable_advanced_prompting and HAS_ADVANCED_PROMPTING
	...
	def _initialize_standards(self):
		...
		self.audience_suitability = {
			...
			ReadingLevel.HIGH_SCHOOL: [AudienceType.GENERAL_PUBLIC, AudienceType.BUSINESS_PROFESSIONAL],
			ReadingLevel.COLLEGE: [AudienceType.BUSINESS_PROFESSIONAL, AudienceType.GOVERNMENT_OFFICIAL],
			ReadingLevel.GRADUATE: [AudienceType.TECHNICAL_EXPERT, AudienceType.ACADEMIC_RESEARCHER],
			ReadingLevel.PROFESSIONAL: [AudienceType.TECHNICAL_EXPERT, AudienceType.ACADEMIC_RESEARCHER]
		}
	...
	def _initialize_vocabularies(self):
		...
		self.technical_vocabulary = {
			...
			'specification', 'protocol', 'interface', 'database', 'authentication',
			'authorization', 'encryption', 'validation', 'verification', 'deployment'
		}
		
		# Business jargon
		self.business_jargon = {
			'synergy', 'leverage', 'paradigm', 'stakeholder', 'deliverable', 'actionable',
			'scalable', 'monetize', 'optimize', 'streamline', 'facilitate', 'implement',
			'strategize', 'prioritize', 'incentivize', 'operationalize', 'institutionalize'
		}
		
		# Academic vocabulary
		self.academic_vocabulary = {
			'hypothesis', 'methodology', 'empirical', 'theoretical', 'conceptual',
			'systematic', 'comprehensive', 'analytical', 'synthesis', 'paradigm',
			'phenomenon', 'correlation', 'causation', 'significance', 'validity'
		}
	...
	async def analyze_readability(
		...
		try:
			...
			result.reading_level = await self._assess_reading_level(result.metrics, target_audience)
			
			# Step 3: Evaluate accessibility
			if self.enable_accessibility_analysis:
				result.accessibility = await self._evaluate_accessibility(text, result.metrics)
			
			# Step 4: AI-enhanced analysis (if enabled)
			if use_ai and aiohttp:
				try:
					result.ai_analysis = await self._ai_readability_analysis(text, target_audience)
					result.readability_recommendations = await self._generate_ai_recommendations(text, result)
					result.simplification_suggestions = await self._generate_simplification_suggestions(text, result)
			...
			if not use_ai or not aiohttp or not result.readability_recommendations:
				result.readability_recommendations = self._generate_rule_based_recommendations(result)
				result.simplification_suggestions = self._generate_rule_based_simplifications(text, result)
			...
			self.logger.info(f"Readability analysis completed, reading level: {result.reading_level.primary_level.value}")
	...
	async def _calculate_readability_metrics(self, text: str) -> ReadabilityMetrics:
		...
		try:
			...
			if difficult_words:
				...
				if metrics.word_count > 0:
					metrics.difficult_words_ratio = metrics.difficult_words_count / metrics.word_count
			else:
				# Fallback: estimate difficult words (3+ syllables)
				words = text.split()
				complex_word_count = 0
				for word in words:
					word_syllables = self._count_syllables(word.lower().strip('.,!?;:"()[]{}'))
					if word_syllables >= 3:
						complex_word_count += 1
				
				metrics.complex_words_count = complex_word_count
				if metrics.word_count > 0:
					metrics.complex_words_ratio = complex_word_count / metrics.word_count
			...
			technical_count = sum(1 for term in self.technical_vocabulary if term in text_lower)
			business_count = sum(1 for term in self.business_jargon if term in text_lower)
			academic_count = sum(1 for term in self.academic_vocabulary if term in text_lower)
			
			metrics.technical_terms_count = technical_count + business_count + academic_count
			
			if metrics.word_count > 0:
				metrics.jargon_ratio = metrics.technical_terms_count / metrics.word_count
	...
	async def _assess_reading_level(self, metrics: ReadabilityMetrics, target_audience: Optional[AudienceType]) -> ReadingLevelAssessment:
		...
		try:
			...
			if grade_level_estimates:
				avg_grade_level = sum(grade_level_estimates) / len(grade_level_estimates)
				...
				for level, (min_grade, max_grade) in self.grade_level_ranges.items():
					if min_grade <= avg_grade_level <= max_grade:
						assessment.primary_level = level
						assessment.grade_level_range = (max(min_grade, avg_grade_level - 1), min(max_grade, avg_grade_level + 1))
						break
			...
			assessment.suitable_audiences = self.audience_suitability.get(assessment.primary_level, [])
			...
			assessment.accessibility_score = sum(accessibility_factors) / len(accessibility_factors)
			...
			assessment.confidence_score = sum(confidence_factors) / len(confidence_factors)
	...
	async def _evaluate_accessibility(self, text: str, metrics: ReadabilityMetrics) -> AccessibilityAssessment:
		...
		try:
			...
			if metrics.avg_sentence_length <= 15:
				...
				assessment.barriers.append("Sentences are too long (>25 words on average)")
				assessment.improvements_needed.append("Break long sentences into shorter ones")
			...
			if metrics.difficult_words_ratio <= 0.05:
				...
				assessment.improvements_needed.append("Replace complex words with simpler alternatives")
			...
			if paragraph_count > 0:
				...
				if avg_sentences_per_paragraph <= 5:
					...
					assessment.improvements_needed.append("Break long paragraphs into shorter ones")
			else:
				assessment.structure_clarity = 0.5
			...
			passive_ratio = passive_voice_count / sentence_count if sentence_count > 0 else 0
			...
			if metrics.jargon_ratio <= 0.02:
				...
				assessment.improvements_needed.append("Explain technical terms or use simpler language")
			
			assessment.plain_language_score = sum(plain_language_factors) / len(plain_language_factors)
			...
			elif 0.2 <= metrics.vocabulary_diversity < 0.3 or 0.7 < metrics.vocabulary_diversity <= 0.8:
			...
			assessment.overall_accessibility = sum(accessibility_scores) / len(accessibility_scores)
			...
			if assessment.overall_accessibility < 0.6:
				...
				if "Organize content with clear headings" not in assessment.improvements_needed:
					assessment.improvements_needed.append("Organize content with clear headings")
	...
	async def _ai_readability_analysis(self, text: str, target_audience: Optional[AudienceType]) -> Dict[str, Any]:
		...
		audience_context = f" for {target_audience.value.replace('_', ' ')} audience" if target_audience else ""
		...
		async with aiohttp.ClientSession(timeout=timeout) as session:
			async with session.post(
				...
				if response.status == 200:
					...
					if self.llm_config.enable_thinking_filter and self.enable_advanced_prompting:
					...
					try:
						...
						return {
							"readability_assessment": {"reading_level": "analysis_available"},
							"raw_analysis": ai_response,
							"recommendations": ["Review AI analysis in raw_analysis field"]
						}
				else:
					raise Exception(f"Ollama API error: {response.status}")
	
	def _create_advanced_readability_prompt(self, text: str, target_audience: Optional[AudienceType]) -> str:
...
{{
	...
	async def _generate_ai_recommendations(self, text: str, result: ReadabilityAnalysisResult) -> List[str]:
	...
	async def _generate_simplification_suggestions(self, text: str, result: ReadabilityAnalysisResult) -> List[str]:
		...
		if result.ai_analysis and 'simplification_targets' in result.ai_analysis:
			for target in result.ai_analysis['simplification_targets']:
				if isinstance(target, dict) and 'original' in target and 'suggested' in target:
					suggestions.append(f"Replace '{target['original']}' with '{target['suggested']}'")
	...
	def _generate_rule_based_recommendations(self, result: ReadabilityAnalysisResult) -> List[str]:
		...
		if result.metrics.jargon_ratio > 0.05:
			recommendations.append("Define technical terms or use plain language alternatives")
		
		# Structure recommendations
		if result.metrics.paragraph_count > 0:
			avg_sentences_per_paragraph = result.metrics.sentence_count / result.metrics.paragraph_count
		...
		if result.reading_level.primary_level.value != target_level.value:
			level_order = [ReadingLevel.ELEMENTARY, ReadingLevel.MIDDLE_SCHOOL, 
						  ReadingLevel.HIGH_SCHOOL, ReadingLevel.COLLEGE, 
						  ReadingLevel.GRADUATE, ReadingLevel.PROFESSIONAL]
			
			current_index = level_order.index(result.reading_level.primary_level)
			target_index = level_order.index(target_level)
			
			if current_index > target_index:
				recommendations.append(f"Simplify content to reach {target_level.value.replace('_', ' ')} reading level")
			else:
				recommendations.append(f"Content may be too simple for {target_level.value.replace('_', ' ')} audience")
	...
	def _generate_rule_based_simplifications(self, text: str, result: ReadabilityAnalysisResult) -> List[str]:
		...
		for jargon in self.business_jargon:
			if jargon in text_lower:
				suggestions.append(f"Consider replacing business jargon '{jargon}' with clearer language")
		
		return suggestions[:5]  # Limit to 5 simplification suggestions
	
	def _calculate_readability_confidence(self, result: ReadabilityAnalysisResult) -> float:
	...
	def _compile_readability_statistics(self, result: ReadabilityAnalysisResult, text: str) -> Dict[str, Any]:
		...
		return {
			...
			'audience_analysis': {
				'suitable_audiences': [audience.value for audience in result.reading_level.suitable_audiences],
