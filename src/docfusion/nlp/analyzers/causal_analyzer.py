...
class CausalAnalyzer:
	...
	def __init__(
		...
		if not HAS_SCIPY or not HAS_STATSMODELS:
			self.logger.warning("Some causal analysis libraries not available. Install scipy and statsmodels for full functionality.")
		
		self.logger.info(f"CausalAnalyzer initialized with {confidence_level.value} confidence level")
	...
	async def analyze_causality(
		...
		try:
			...
			if CausalityType.CORRELATION in methods:
				correlations = await self._analyze_correlations(df, treatment_variables, outcome_variables)
				result.causal_relationships.extend(correlations)
			
			if CausalityType.GRANGER_CAUSALITY in methods and HAS_STATSMODELS:
				granger_results = await self._analyze_granger_causality(df, treatment_variables, outcome_variables)
				result.causal_relationships.extend(granger_results)
			
			if CausalityType.INSTRUMENTAL_VARIABLE in methods:
				iv_results = await self._analyze_instrumental_variables(df, treatment_variables, outcome_variables, control_variables)
				result.causal_relationships.extend(iv_results)
			
			# Build causal models
			models = await self._build_causal_models(df, treatment_variables, outcome_variables, control_variables)
			result.causal_models.extend(models)
			
			# Generate insights and recommendations
			result.causal_insights = await self._generate_causal_insights(result.causal_relationships)
			result.recommendations = await self._generate_recommendations(result.causal_relationships, result.causal_models)
			
			# Calculate summary statistics
			result.total_relationships = len(result.causal_relationships)
			result.significant_relationships = len([r for r in result.causal_relationships if r.p_value < self.alpha_levels[self.confidence_level]])
			
			# Find strongest relationship
			if result.causal_relationships:
				result.strongest_relationship = min(result.causal_relationships, key=lambda x: x.p_value)
			
			result.processing_time = asyncio.get_event_loop().time() - start_time
			result.success = len(result.errors) == 0
			
			self.logger.info(f"Causal analysis completed: {result.significant_relationships}/{result.total_relationships} significant relationships found")
	...
	async def _assess_data_quality(self, df: pd.DataFrame) -> Dict[str, Any]:
		...
		if len(df) < self.min_sample_size:
			quality_assessment['issues'].append(f"Sample size ({len(df)}) below recommended minimum ({self.min_sample_size})")
		...
		if missing_pct > 0.1:
			quality_assessment['issues'].append(f"High missing data rate: {missing_pct:.1%}")
		...
		outlier_pct = outlier_counts / (len(df) * len(numeric_cols)) if len(numeric_cols) > 0 else 0
				...
				if treatment in df.columns and outcome in df.columns:
					...
					try:
						...
						relationship = CausalRelationship(
							...
							practical_significance=abs(corr_coef) > self.effect_size_threshold,
						...
						if len(clean_data) > 3:
							...
							t_crit = stats.t.ppf(1 - self.alpha_levels[self.confidence_level]/2, len(clean_data) - 2)
						...
						self.logger.warning(f"Correlation analysis failed for {treatment}->{outcome}: {e}")
			...
			for outcome in outcome_vars:
				if treatment in df.columns and outcome in df.columns:
					try:
						...
						test_data = np.column_stack([clean_data[outcome], clean_data[treatment]])
						gc_results = grangercausalitytests(test_data, maxlag=self.max_lag_order, verbose=False)
						...
						for lag in range(1, min(self.max_lag_order + 1, len(gc_results) + 1)):
						...
						relationship = CausalRelationship(
							...
							sample_size=len(clean_data),
							causal_strength=self._determine_causal_strength(best_p_value),
							practical_significance=best_p_value < self.alpha_levels[self.confidence_level],
						...
						self.logger.warning(f"Granger causality test failed for {treatment}->{outcome}: {e}")
				...
				if treatment in df.columns and outcome in df.columns:
					try:
						...
						relationship = CausalRelationship(
							...
							sample_size=len(clean_data),
							causal_strength=self._determine_causal_strength(p_value),
							practical_significance=p_value < self.alpha_levels[self.confidence_level],
							interpretation=f"IV estimate: {treatment_coef:.3f} (F-stat: {f_stat:.1f})",
							assumptions_met=["instrument_relevance"] if f_stat > 10 else [],
						...
						self.logger.warning(f"IV analysis failed for {treatment}->{outcome}: {e}")
	...
	async def _build_causal_models(
		...
		for outcome in outcome_vars:
			...
			try:
				...
				available_predictors = [var for var in all_predictors if var in df.columns]
				...
				causal_model = CausalModel(
					...
					treatment_variables=[var for var in treatment_vars if var in available_predictors],
					control_variables=[var for var in (control_vars or []) if var in available_predictors],
					model_fit={
				...
				for i, var in enumerate(available_predictors):
					if i + 1 < len(model.params):  # Skip constant
						...
						relationship = CausalRelationship(
							...
							practical_significance=p_val < self.alpha_levels[self.confidence_level],
				...
				if self.enable_robustness_checks:
					causal_model.heteroscedasticity_test = self._test_heteroscedasticity(model)
					causal_model.multicollinearity_test = self._test_multicollinearity(X.iloc[:, 1:])  # Skip constant
					causal_model.autocorrelation_test = self._test_autocorrelation(model)
	...
	async def _generate_causal_insights(self, relationships: List[CausalRelationship]) -> List[str]:
		...
		if not significant_relationships:
			insights.append("No statistically significant causal relationships detected at 95% confidence level")
			return insights
		
		# Strongest relationships
		strong_relationships = [r for r in significant_relationships if r.causal_strength in [CausalStrength.STRONG, CausalStrength.VERY_STRONG]]
		if strong_relationships:
			strongest = min(strong_relationships, key=lambda x: x.p_value)
			insights.append(f"Strongest causal relationship: {strongest.cause_variable} → {strongest.effect_variable} (p={strongest.p_value:.4f})")
		
		# Effect sizes
		large_effects = [r for r in significant_relationships if r.effect_size > 0.5]
		if large_effects:
			insights.append(f"Found {len(large_effects)} relationships with large effect sizes (>0.5)")
		
		# Positive vs negative effects
		positive_effects = [r for r in significant_relationships if r.direction == "positive"]
		negative_effects = [r for r in significant_relationships if r.direction == "negative"]
		
		if positive_effects and negative_effects:
			insights.append(f"Mixed directional effects: {len(positive_effects)} positive, {len(negative_effects)} negative relationships")
		elif positive_effects:
			insights.append(f"Predominantly positive relationships detected ({len(positive_effects)} relationships)")
		elif negative_effects:
			insights.append(f"Predominantly negative relationships detected ({len(negative_effects)} relationships)")
		
		return insights
	
	async def _generate_recommendations(self, relationships: List[CausalRelationship], models: List[CausalModel]) -> List[str]:
		...
		if not significant_relationships:
			recommendations.append("Collect more data or explore different variables to identify causal relationships")
			return recommendations
		
		# Data quality recommendations
		high_quality_relationships = [r for r in significant_relationships if not r.violations]
		if len(high_quality_relationships) < len(significant_relationships):
			recommendations.append("Address assumption violations to strengthen causal inference")
		
		# Sample size recommendations
		small_sample_relationships = [r for r in significant_relationships if r.sample_size < 100]
		if small_sample_relationships:
			recommendations.append("Increase sample size for more robust causal estimates")
		
		# Practical significance
		practically_significant = [r for r in significant_relationships if r.practical_significance]
		if practically_significant:
			top_practical = max(practically_significant, key=lambda x: x.effect_size)
			recommendations.append(f"Focus on {top_practical.cause_variable} → {top_practical.effect_variable} relationship for maximum impact")
		
		# Model-specific recommendations
		for model in models:
			if model.model_fit.get('r_squared', 0) < 0.3:
				recommendations.append(f"Consider additional variables for {model.outcome_variable} model (low R²)")
