...
class SemanticAnalyzer:
	...
	def __init__(
		...
		self.logger.info(f"SemanticAnalyzer initialized with model: {self.llm_config.model} from centralized config")
	...
	def _initialize_preprocessing(self):
		...
		if HAS_NLTK:
			...
			semantic_stopwords = {
				'now', 'however', 'also', 'therefore', 'thus', 'hence', 'moreover', 
				'furthermore', 'nevertheless', 'although', 'indeed', 'particularly'
			}
		...
		self.domain_concepts = {
			'technical_concepts': {
				'architecture', 'infrastructure', 'system', 'platform', 'framework', 
				'database', 'integration', 'api', 'security', 'scalability', 'performance'
			},
			'project_concepts': {
				'requirement', 'deliverable', 'milestone', 'timeline', 'budget', 'resource',
				'stakeholder', 'objective', 'scope', 'risk', 'quality', 'testing'
			},
			'business_concepts': {
				'strategy', 'value', 'benefit', 'roi', 'cost', 'efficiency', 'compliance',
				'governance', 'vendor', 'procurement', 'contract', 'agreement'
			}
		}
	...
	async def analyze_semantics(
		...
		try:
			...
			if not result.topics and TfidfVectorizer:
				try:
					traditional_topics = await self._traditional_topic_modeling(processed_text, sentences, num_topics)
					result.topics.extend(traditional_topics)
					result.topic_model_type = TopicModelType.LDA if LatentDirichletAllocation else TopicModelType.CLUSTERING
				except Exception as e:
					result.warnings.append(f"Traditional topic modeling failed: {str(e)}")
					self.logger.warning(f"Traditional topic modeling failed: {e}")
			
			# Step 3: Create semantic clusters
			if len(sentences) > 3:
				result.semantic_clusters = await self._create_semantic_clusters(sentences, result.topics)
			...
			if use_ai and aiohttp:
				try:
					...
					result.thematic_summary = result.ai_analysis.get('thematic_summary', '')
			...
			self.logger.info(f"Semantic analysis completed, topics: {len(result.topics)}, "
		...
		output_format = """JSON format:
{
			...
			async with session.post(
				...
				if response.status == 200:
					...
					try:
						...
						for i, topic_data in enumerate(parsed_response.get('topics', [])):
							topic = Topic()
							...
							if topic.keywords:
								...
								keyword_count = sum(text_lower.count(kw.lower()) for kw in topic.keywords)
								word_count = len(text.split())
								topic.document_coverage = min(keyword_count / word_count * 10, 1.0) if word_count > 0 else 0
	...
	async def _traditional_topic_modeling(self, processed_text: str, sentences: List[str], num_topics: int) -> List[Topic]:
				...
				for cluster_id in range(num_topics):
					...
					if cluster_sentences:
						...
						if cluster_tfidf.shape[0] > 0:
							mean_tfidf = np.mean(cluster_tfidf.toarray(), axis=0) if np else [0.5] * len(feature_names)
							top_indices = sorted(range(len(mean_tfidf)), key=lambda i: mean_tfidf[i], reverse=True)[:10]
							top_words = [feature_names[i] for i in top_indices]
	...
	async def _create_semantic_clusters(self, sentences: List[str], topics: List[Topic]) -> List[SemanticCluster]:
		...
		try:
			...
			vectorizer = TfidfVectorizer(max_features=100, stop_words='english' if hasattr(TfidfVectorizer(), 'stop_words') else None)
			tfidf_matrix = vectorizer.fit_transform(sentences)
			...
			if KMeans and num_clusters > 1:
				...
				for cluster_id in range(num_clusters):
					...
					if cluster_sentences:
						...
						cluster.representative_sentence = cluster_sentences[0]  # Use first as representative
						...
						if len(cluster_sentences) > 1:
							...
							if cosine_similarity:
								...
								cluster.coherence_score = float(np.mean(similarities)) if np else 0.5
							else:
						...
						cluster.themes = [topic.name for topic in topics if any(kw in ' '.join(cluster_sentences).lower() for kw in topic.keywords)]
	...
	async def _build_concept_map(self, text: str, topics: List[Topic]) -> ConceptMap:
		...
		try:
			...
			for concept in concept_map.concepts:
				...
				concept_map.concept_weights[concept] = count / total_words if total_words > 0 else 0
			...
			for concept in concept_map.concepts:
				...
				topic_appearances = sum(1 for topic in topics if concept in topic.keywords)
			...
			concept_map.central_concepts = [concept for concept, score in sorted_concepts[:5]]
			...
			for i, concept1 in enumerate(concept_map.central_concepts):
				for concept2 in concept_map.central_concepts[i+1:]:
					...
					sentences = self._split_into_sentences(text)
					cooccurrence = sum(1 for sent in sentences if concept1 in sent.lower() and concept2 in sent.lower())
					
					if cooccurrence > 0:
						relation_type = "co-occurs_with"
						if concept1 in self.domain_concepts.get('technical_concepts', set()) and concept2 in self.domain_concepts.get('business_concepts', set()):
							relation_type = "technical_supports"
						elif concept1 in self.domain_concepts.get('project_concepts', set()) and concept2 in self.domain_concepts.get('business_concepts', set()):
							relation_type = "project_delivers"
						
						concept_map.relationships.append((concept1, relation_type, concept2))
		...
		output_format = """JSON format:
{
	...
	def _calculate_semantic_confidence(self, result: SemanticAnalysisResult) -> float:
		...
		if result.topics:
			avg_topic_confidence = sum(topic.confidence for topic in result.topics) / len(result.topics)
			confidence_factors.append(avg_topic_confidence)
		
		# Cluster quality factor
		if result.semantic_clusters:
			avg_cluster_coherence = sum(cluster.coherence_score for cluster in result.semantic_clusters) / len(result.semantic_clusters)
	...
	def _compile_semantic_statistics(self, result: SemanticAnalysisResult, text: str) -> Dict[str, Any]:
		...
		return {
			...
			'topic_analysis': {
				'avg_topic_weight': sum(topic.weight for topic in result.topics) / len(result.topics) if result.topics else 0,
				'avg_topic_confidence': sum(topic.confidence for topic in result.topics) / len(result.topics) if result.topics else 0,
				'topic_model_type': result.topic_model_type.value
			},
			'clustering_metrics': {
				'avg_cluster_size': sum(cluster.size for cluster in result.semantic_clusters) / len(result.semantic_clusters) if result.semantic_clusters else 0,
				'avg_coherence_score': sum(cluster.coherence_score for cluster in result.semantic_clusters) / len(result.semantic_clusters) if result.semantic_clusters else 0
			},
			'content_metrics': {
				'text_length': len(text),
				'word_count': len(text.split()),
				'unique_concepts_ratio': len(result.concept_map.concepts) / len(set(text.lower().split())) if text else 0
			}
		}
	...
	async def calculate_similarity(
		...
		try:
			if similarity_type == SimilarityMetric.COSINE and TfidfVectorizer and cosine_similarity:
	...
	async def _cosine_similarity_analysis(self, text1: str, text2: str) -> SimilarityAnalysis:
		"""Calculate cosine similarity using TF-IDF vectors"""
		analysis = SimilarityAnalysis()
		analysis.similarity_type = SimilarityMetric.COSINE
		
		vectorizer = TfidfVectorizer(stop_words='english' if hasattr(TfidfVectorizer(), 'stop_words') else None)
	...
	async def _jaccard_similarity_analysis(self, text1: str, text2: str) -> SimilarityAnalysis:
	...
	async def _ai_similarity_analysis(self, text1: str, text2: str) -> SimilarityAnalysis:
...
{{
			...
			async with session.post(
				...
				if response.status == 200:
					...
					try:
						...
						analysis.similarity_score = float(parsed.get('similarity_score', 0.5))
						analysis.semantic_similarity = float(parsed.get('semantic_similarity', analysis.similarity_score))
						analysis.lexical_similarity = float(parsed.get('lexical_similarity', analysis.similarity_score))
						analysis.structural_similarity = float(parsed.get('structural_similarity', analysis.similarity_score))
	...
	async def _simple_similarity_analysis(self, text1: str, text2: str) -> SimilarityAnalysis:
		...
		if words1 or words2:
			intersection = words1 & words2
			analysis.similarity_score = len(intersection) / max(len(words1), len(words2))
			analysis.common_terms = list(intersection)[:10]
	...
	def get_analyzer_info(self) -> Dict[str, Any]:
		...
		return {
			...
			'supported_similarity_metrics': [metric.value for metric in SimilarityMetric],
			'analysis_features': {
				'topic_modeling': True,
