...
try:
	from ...prompting_strategies import create_chain_of_thought_prompt, create_tree_of_thought_prompt, filter_thinking_tags
except ImportError:
	try:
		from ..prompting_strategies import create_chain_of_thought_prompt, create_tree_of_thought_prompt, filter_thinking_tags
	except ImportError:
		...
		def filter_thinking_tags(text: str) -> str:
			if not text or not isinstance(text, str):
				return text
			cleaned = re.sub(r'<think\s*>.*?</think\s*>', '', text, flags=re.DOTALL | re.IGNORECASE)
			return re.sub(r'\n\s*\n\s*\n', '\n\n', cleaned).strip()

try:
	from ...config.llm_config import get_llm_config, LLMTask, LLMConfiguration
except ImportError:
	try:
		from ..config.llm_config import get_llm_config, LLMTask, LLMConfiguration
	except ImportError:
...
class StyleAnalyzer:
	...
	def __init__(
		...
		self.logger.info(f"StyleAnalyzer initialized with model: {self.llm_config.model} from centralized config")
	
	def _initialize_vocabularies(self, custom_vocabularies: Optional[Dict[str, Set[str]]]):
		...
		if 'technical_terms' not in self.vocabularies:
			self.vocabularies['technical_terms'] = {
				'algorithm', 'architecture', 'infrastructure', 'implementation', 'configuration',
				'optimization', 'integration', 'scalability', 'methodology', 'framework',
				'specification', 'protocol', 'interface', 'database', 'application',
				'deployment', 'authentication', 'authorization', 'encryption', 'validation'
			}
		
		# Business terms vocabulary
		if 'business_terms' not in self.vocabularies:
			self.vocabularies['business_terms'] = {
				'stakeholder', 'deliverable', 'milestone', 'requirement', 'objective',
				'strategy', 'initiative', 'investment', 'roi', 'kpi', 'metrics',
				'compliance', 'governance', 'vendor', 'procurement', 'budget',
				'timeline', 'deadline', 'proposal', 'contract', 'agreement'
			}
		...
		if 'action_words' not in self.vocabularies:
			self.vocabularies['action_words'] = {
				...
				'execute', 'deliver', 'achieve', 'provide', 'ensure', 'maintain',
				'optimize', 'enhance', 'improve', 'support', 'manage', 'coordinate',
				'facilitate', 'enable', 'integrate', 'deploy', 'configure', 'validate'
			}
		
		# Certainty indicators
		if 'certainty_words' not in self.vocabularies:
			self.vocabularies['certainty_words'] = {
				'will', 'must', 'shall', 'definitely', 'certainly', 'absolutely',
				'clearly', 'obviously', 'undoubtedly', 'guaranteed', 'proven',
				'established', 'confirmed', 'verified', 'demonstrated', 'ensures'
			}
		...
		if 'uncertainty_words' not in self.vocabularies:
			self.vocabularies['uncertainty_words'] = {
				...
				'approximately', 'roughly', 'estimated', 'expected', 'anticipated'
			}
	...
	def _compile_patterns(self):
		...
		self.patterns = {
			...
			'subordinate_clauses': re.compile(r'\b(?:because|since|although|while|if|when|unless|until)\b', re.IGNORECASE),
			'conjunctions': re.compile(r'\b(?:and|but|or|nor|for|so|yet|however|therefore|moreover|furthermore)\b', re.IGNORECASE),
			...
			'third_person': re.compile(r'\b(?:he|she|it|they|him|her|them|his|hers|its|their|theirs)\b', re.IGNORECASE)
		}
	...
	async def analyze_style(
		...
		try:
			...
			detailed = detailed_analysis if detailed_analysis is not None else self.enable_detailed_analysis
			...
			if use_ai and aiohttp:
				try:
					...
					result.style_recommendations = await self._generate_style_recommendations(text, result)
					result.improvement_suggestions = await self._generate_improvement_suggestions(text, result)
			...
			self.logger.info(f"Style analysis completed, confidence: {result.confidence_score:.3f}")
	...
	async def _calculate_style_metrics(self, text: str) -> StyleMetrics:
		...
		if metrics.paragraph_count > 0:
			metrics.avg_sentences_per_paragraph = metrics.sentence_count / metrics.paragraph_count
		...
		if metrics.word_count > 0:
			...
			metrics.passive_voice_ratio = passive_constructions / metrics.sentence_count if metrics.sentence_count > 0 else 0
			
			subordinate_clauses = len(self.patterns['subordinate_clauses'].findall(text))
			metrics.subordinate_clause_ratio = subordinate_clauses / metrics.sentence_count if metrics.sentence_count > 0 else 0
		...
		if metrics.word_count > 0:
			...
			tech_count = sum(1 for term in self.vocabularies['technical_terms'] if term in text_lower)
			metrics.technical_terms_ratio = tech_count / metrics.word_count
			
			business_count = sum(1 for term in self.vocabularies['business_terms'] if term in text_lower)
			metrics.business_terms_ratio = business_count / metrics.word_count
			
			action_count = sum(1 for term in self.vocabularies['action_words'] if term in text_lower)
	...
	async def _analyze_tone(self, text: str, detailed: bool) -> ToneAnalysis:
		...
		text_lower = text.lower()
		formal_vocab_count = (
			sum(1 for term in self.vocabularies['technical_terms'] if term in text_lower) +
		...
		if len(words) > 0:
			...
			formality_indicators.append(min(formal_vocab_ratio * 10, 1.0))  # Scale to 0-1
		...
		if hasattr(self, '_last_metrics'):
			...
			sentence_formality = min(avg_sentence_length / 20, 1.0)  # Scale with 20 words as baseline
		...
		personal_pronoun_ratio = (first_person_count + second_person_count) / len(words) if len(words) > 0 else 0
		pronoun_formality = 1.0 - min(personal_pronoun_ratio * 20, 1.0)  # Inverse relationship
		formality_indicators.append(pronoun_formality)
		
		# Calculate overall formality score
		analysis.formality_score = sum(formality_indicators) / len(formality_indicators) if formality_indicators else 0.5
		
		# Calculate authority score
		authority_indicators = []
		
		# Certainty vs uncertainty language
		certainty_ratio = sum(1 for word in self.vocabularies['certainty_words'] if word in text_lower) / len(words) if len(words) > 0 else 0
		uncertainty_ratio = sum(1 for word in self.vocabularies['uncertainty_words'] if word in text_lower) / len(words) if len(words) > 0 else 0
		
		authority_from_certainty = min(certainty_ratio * 20, 1.0) - min(uncertainty_ratio * 10, 0.5)
		authority_indicators.append(max(authority_from_certainty, 0))
		
		# Action words (indicate decisiveness)
		action_ratio = sum(1 for word in self.vocabularies['action_words'] if word in text_lower) / len(words) if len(words) > 0 else 0
		authority_indicators.append(min(action_ratio * 15, 1.0))
		
		analysis.authority_score = sum(authority_indicators) / len(authority_indicators) if authority_indicators else 0.5
		...
		if sentence_count > 0:
			punctuation_emotion = (question_count + exclamation_count * 2) / sentence_count
			emotion_indicators.append(min(punctuation_emotion, 1.0))
		
		analysis.emotion_score = sum(emotion_indicators) / len(emotion_indicators) if emotion_indicators else 0.2
		...
		persuasive_indicators.append(min(you_count / len(words) * 30, 1.0) if len(words) > 0 else 0)
		
		# Action words (call to action)
		persuasive_indicators.append(min(action_ratio * 10, 1.0))
		
		analysis.persuasiveness_score = sum(persuasive_indicators) / len(persuasive_indicators) if persuasive_indicators else 0.3
		...
		tone_scores = {
			...
			WritingTone.TECHNICAL: min(formal_vocab_ratio * 8, 1.0) if len(words) > 0 else 0,
			WritingTone.CONVERSATIONAL: 1.0 - analysis.formality_score + analysis.emotion_score * 0.5
		}
	...
	async def _analyze_writing_style(self, text: str, detailed: bool) -> StyleAnalysis:
		...
		descriptive_patterns = re.findall(r'\b(?:detailed|comprehensive|extensive|thorough|specific|precise)\b', text.lower())
		adjective_pattern = re.compile(r'\b(?:\w+ly\s+\w+|\w+(?:ive|ous|ful|ing|ed)\s+\w+)\b', re.IGNORECASE)
		adjectives = len(adjective_pattern.findall(text))
		analysis.descriptiveness = min((len(descriptive_patterns) + adjectives * 0.1) / word_count * 100, 1.0)
		
		# Argumentativeness (reasoning words, evidence presentation)
		argument_words = ['therefore', 'because', 'since', 'however', 'moreover', 'furthermore', 'consequently', 'thus', 'hence']
		...
		tech_terms = sum(1 for term in self.vocabularies['technical_terms'] if term in text.lower())
		analysis.technicality = min(tech_terms / word_count * 20, 1.0)
		
		# Clarity (sentence structure, direct language)
		avg_sentence_length = word_count / max(len(self.patterns['sentences'].split(text)), 1)
		clarity_score = 1.0 - min(avg_sentence_length / 30, 0.7)  # Shorter sentences = clearer
		...
		filler_words = ['very', 'really', 'quite', 'rather', 'somewhat', 'actually', 'basically', 'essentially']
		filler_count = sum(text.lower().count(word) for word in filler_words)
		conciseness_penalty = min(filler_count / word_count * 10, 0.4)
		analysis.conciseness = max(1.0 - conciseness_penalty - (avg_sentence_length / 40), 0.1)
		
		# Determine primary style based on characteristics
		style_scores = {
			WritingStyle.TECHNICAL_SPECIFICATION: analysis.technicality * 0.7 + analysis.clarity * 0.3,
			WritingStyle.ARGUMENTATIVE: analysis.argumentativeness * 0.8 + analysis.clarity * 0.2,
			WritingStyle.ANALYTICAL: analysis.argumentativeness * 0.4 + analysis.technicality * 0.4 + analysis.clarity * 0.2,
			WritingStyle.DESCRIPTIVE: analysis.descriptiveness * 0.8 + analysis.clarity * 0.2,
			WritingStyle.EXPOSITORY: analysis.clarity * 0.5 + analysis.conciseness * 0.3 + analysis.argumentativeness * 0.2,
			WritingStyle.INSTRUCTIONAL: analysis.clarity * 0.6 + analysis.conciseness * 0.4
		}
		...
		output_format = """JSON format:
{
				...
				if response.status == 200:
					...
					try:
						...
						return {
							...
							"tone_assessment": {"primary_tone": "analysis_available", "tone_confidence": 0.7},
							"recommendations": ["Review AI analysis in raw_response field"]
						}
				else:
					raise Exception(f"Ollama API error: {response.status}")
	
	async def _generate_style_recommendations(self, text: str, analysis: StyleAnalysisResult) -> List[str]:
		...
		if analysis.metrics.flesch_reading_ease and analysis.metrics.flesch_reading_ease < 30:
			recommendations.append("Consider simplifying sentence structure for better readability (current Flesch score indicates difficult reading level)")
		
		if analysis.metrics.avg_words_per_sentence > 25:
			recommendations.append(f"Break up long sentences to improve clarity (average {analysis.metrics.avg_words_per_sentence:.1f} words per sentence)")
		
		# Tone recommendations with contextual awareness
		if analysis.tone_analysis.formality_score < 0.3 and analysis.metrics.business_terms_ratio > 0.05:
			recommendations.append("Maintain consistent formal tone throughout business content (detected business terminology with informal language)")
		
		if analysis.tone_analysis.authority_score < 0.4:
			recommendations.append("Use more decisive language and reduce uncertainty markers to increase authority")
		...
		if analysis.style_analysis.clarity < 0.6:
			...
			if clarity_issues:
				recommendations.append(f"Improve clarity by addressing: {', '.join(clarity_issues)}")
			else:
				recommendations.append("Improve clarity by using more direct language and concrete examples")
		
		if analysis.metrics.passive_voice_ratio > 0.3:
			recommendations.append(f"Reduce passive voice usage for more engaging writing (currently {analysis.metrics.passive_voice_ratio:.1%} of content)")
		
		# Advanced vocabulary recommendations
		if analysis.metrics.unique_words_ratio < 0.4:
			recommendations.append(f"Expand vocabulary to avoid repetitive language (unique word ratio: {analysis.metrics.unique_words_ratio:.1%})")
		...
		if analysis.style_analysis.primary_style.value == "technical_specification":
			if analysis.metrics.technical_terms_ratio < 0.03:
				recommendations.append("Include more specific technical terminology for technical specification style")
		
		if analysis.style_analysis.primary_style.value == "persuasive":
			if analysis.metrics.second_person_ratio < 0.01:
				recommendations.append("Consider using direct address (you/your) to enhance persuasive impact")
		
		return recommendations
	
	async def _generate_improvement_suggestions(self, text: str, analysis: StyleAnalysisResult) -> List[str]:
		...
		if analysis.metrics.paragraph_count < 3 and len(text) > 500:
			suggestions.append("Break content into multiple paragraphs for better organization and readability")
		
		if analysis.metrics.sentence_count > 0:
			if analysis.metrics.avg_sentences_per_paragraph > 8:
				suggestions.append("Consider shorter paragraphs (currently averaging 8+ sentences per paragraph)")
		
		# Step 2: Engagement and readability improvements
		if analysis.metrics.question_ratio == 0 and analysis.tone_analysis.persuasiveness_score > 0.6:
			suggestions.append("Consider adding rhetorical questions to engage readers and enhance persuasive impact")
		
		if analysis.metrics.exclamation_ratio == 0 and analysis.tone_analysis.emotion_score < 0.2:
			if analysis.style_analysis.primary_style in [WritingStyle.PERSUASIVE, WritingStyle.ARGUMENTATIVE]:
				suggestions.append("Add occasional emphasis to strengthen emotional connection with readers")
		
		# Step 3: Style-specific improvements with reasoning
		if analysis.style_analysis.primary_style == WritingStyle.TECHNICAL_SPECIFICATION:
			if analysis.metrics.technical_terms_ratio < 0.03:
				suggestions.append("Include more specific technical terminology and precise specifications for technical content")
			if analysis.style_analysis.clarity < 0.7:
				suggestions.append("Enhance technical clarity with step-by-step explanations and concrete examples")
		
		if analysis.style_analysis.primary_style == WritingStyle.ARGUMENTATIVE:
			if analysis.metrics.conjunction_usage_ratio < 0.02:
				suggestions.append("Strengthen logical connections between ideas using more transitional phrases")
			if analysis.tone_analysis.authority_score < 0.6:
				suggestions.append("Support arguments with more definitive language and evidence-based statements")
		...
		if analysis.metrics.business_terms_ratio > 0.05:
			if analysis.tone_analysis.formality_score < 0.6:
				suggestions.append("Maintain consistent professional tone throughout business content")
			if analysis.metrics.action_words_ratio < 0.02:
				suggestions.append("Include more action-oriented language to drive business outcomes")
		
		# Step 5: Vocabulary and language improvements
		if analysis.metrics.modal_verbs_ratio > 0.05:
			suggestions.append("Consider reducing modal verb usage (might, could, should) for more direct communication")
		
		if analysis.metrics.uncertainty_words_ratio > analysis.metrics.certainty_words_ratio:
			suggestions.append("Balance uncertain language with more confident statements to enhance credibility")
		
		# Step 6: Audience-specific suggestions
		if analysis.tone_analysis.formality_score > 0.8 and analysis.tone_analysis.emotion_score < 0.3:
			suggestions.append("Balance formal tone with engaging elements to maintain reader interest")
	...
	def _calculate_overall_confidence(self, result: StyleAnalysisResult) -> float:
		...
		if result.ai_analysis and 'tone_assessment' in result.ai_analysis:
			ai_confidence = result.ai_analysis['tone_assessment'].get('tone_confidence', 0.7)
