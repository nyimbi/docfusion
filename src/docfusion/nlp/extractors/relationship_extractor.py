...
class RelationshipExtractor:
		...
		if self.config['use_spacy_deps'] and HAS_SPACY_SUPPORT:
			try:
				...
				if 'parser' not in self.nlp_model.pipe_names:
					self.logger.warning("spaCy model does not include dependency parser")
				else:
					self.dependency_matcher = DependencyMatcher(self.nlp_model.vocab)
					self.matcher = Matcher(self.nlp_model.vocab)
					self.logger.info(f"spaCy dependency parser loaded with '{self.config['spacy_model']}'")
	...
	def _compile_relationship_patterns(self):
		...
		tech_org_pattern = [
			...
			},
			{
				...
				"REL_OP": ">",
				"RIGHT_ID": "uses",
				"RIGHT_ATTRS": {"LEMMA": {"IN": ["use", "adopt", "implement", "deploy"]}}
			},
			{
				"LEFT_ID": "uses",
				"REL_OP": ">",
		...
		project_person_pattern = [
			...
			},
			{
				...
				"REL_OP": ">",
				"RIGHT_ID": "managed",
				"RIGHT_ATTRS": {"LEMMA": {"IN": ["manage", "lead", "direct", "oversee"]}}
			},
			{
				"LEFT_ID": "managed",
				"REL_OP": ">",
	...
	async def extract_relationships(
		...
		try:
			...
			if (use_ai or self.config['use_ai_enhancement']) and len(text) < 75000:
				ai_relationships, ai_analysis = await self._extract_with_ai(text, entities)
			...
			for relationship in filtered_relationships:
				if not relationship.context:
					relationship.context = self._extract_relationship_context(text, relationship)
			
			# Build relationship graph
			relationship_graph = None
			if self.config['build_relationship_graph']:
				relationship_graph = self._build_relationship_graph(filtered_relationships)
			
			# Detect clusters if requested
			if self.config['detect_relationship_clusters'] and relationship_graph:
				relationship_graph.clusters = self._detect_relationship_clusters(relationship_graph)
			...
			self.logger.info(f"Relationships extracted successfully, time: {result.processing_time:.2f}s")
	...
	async def _extract_with_dependencies(self, text: str) -> List[Relationship]:
		...
		if self.dependency_matcher:
			...
			for match_id, token_ids in matches:
				...
				relationship = self._create_relationship_from_dependency_match(label, tokens, doc)
				if relationship:
	...
	def _create_relationship_from_dependency_match(self, label: str, tokens: List[Token], doc: Doc) -> Optional[Relationship]:
		...
		if label == "PERSON_WORKS_ORG":
			...
			verb_token = next((t for t in tokens if t.lemma_ in ["work", "employ", "join"]), None)
			...
			org_token = next((t for t in tokens if t.ent_type_ == "ORG"), None)
			tech_token = next((t for t in tokens if t.ent_type_ in ["PRODUCT", "ORG"]), None)
			verb_token = next((t for t in tokens if t.lemma_ in ["use", "adopt", "implement", "deploy"]), None)
			...
			if token.dep_ == "ROOT" and token.pos_ == "VERB":
				...
				for subj in subjects:
					for obj in objects:
						...
						relationship = self._create_relationship_from_svo(subj, token, obj, sent)
						if relationship:
							relationships.append(relationship)
		
		return relationships
	
	def _create_relationship_from_svo(self, subject: Token, verb: Token, obj: Token, sent: Span) -> Optional[Relationship]:
		...
		subj_ent = subject.ent_type_ if subject.ent_type_ else self._classify_token_entity_type(subject)
		obj_ent = obj.ent_type_ if obj.ent_type_ else self._classify_token_entity_type(obj)
		
		# Map to relationship types
		relationship_type = self._map_svo_to_relationship_type(subj_ent, verb_lemma, obj_ent)
		...
		if relationship_type:
			return Relationship(
				...
				end_char=max(subject.idx + len(subject.text), verb.idx + len(verb.text), obj.idx + len(obj.text)),
	...
	def _classify_token_entity_type(self, token: Token) -> str:
		...
		# Technical terms
		if text_lower in ['system', 'application', 'software', 'platform', 'api', 'service']:
			return "TECH"
		
		# Business terms
		if text_lower in ['project', 'initiative', 'program', 'requirement', 'deliverable']:
			return "PROJECT"
	...
	def _map_svo_to_relationship_type(self, subj_type: str, verb: str, obj_type: str) -> Optional[RelationshipType]:
		"""Map subject-verb-object pattern to relationship type"""
		# Person-Organization relationships
		if subj_type == "PERSON" and verb in ["work", "join", "employ"] and obj_type == "ORG":
		...
		# Project relationships
		if subj_type == "PROJECT" and verb in ["require", "need"] and obj_type in ["TECH", "PRODUCT"]:
	...
	async def _extract_with_patterns(self, text: str) -> List[Relationship]:
		...
		for match_id, start, end in matches:
			...
			relationship = self._create_relationship_from_pattern_match(label, span, doc)
			if relationship:
				relationships.append(relationship)
	...
	def _create_relationship_from_pattern_match(self, label: str, span: Span, doc: Doc) -> Optional[Relationship]:
	...
	async def _extract_with_ai(self, text: str, entities: Optional[List[Any]]) -> Tuple[List[Relationship], Dict[str, Any]]:
		...
		try:
			...
			for chunk_idx, chunk in enumerate(chunks):
				chunk_relationships, chunk_analysis = await self._analyze_chunk_for_relationships(
			...
			if len(text) < self.config['ai_chunk_size']:
				global_analysis = await self._perform_global_relationship_analysis(text, entities)
	...
	async def _analyze_chunk_for_relationships(self, chunk: str, chunk_idx: int, entities: Optional[List[Any]]) -> Tuple[List[Relationship], Dict[str, Any]]:
		"""Analyze chunk for relationships using AI"""
		entity_context = ""
		if entities:
			entity_list = [f"- {entity.text} ({entity.entity_type.value})" for entity in entities[:15]]
...
{{
  ...
  "analysis": {{
		...
		try:
			...
			if response.status_code == 200:
				...
				try:
					...
					relationships = self._convert_ai_relationships(ai_data.get('relationships', []), chunk)
					analysis = ai_data.get('analysis', {})
					analysis['raw_response'] = response_text
					return relationships, analysis
				
				except json.JSONDecodeError:
					relationships = self._parse_ai_text_relationships(response_text, chunk)
	...
	async def _perform_global_relationship_analysis(self, text: str, entities: Optional[List[Any]]) -> Dict[str, Any]:
...
{{
  ...
  "relationship_gaps": ["Missing clear ownership relationships", "Unclear technical dependencies"]
	...
	def _convert_ai_relationships(self, ai_relationships: List[Dict[str, Any]], chunk: str) -> List[Relationship]:
	...
	def _parse_ai_text_relationships(self, response_text: str, chunk: str) -> List[Relationship]:
		...
		for line in lines:
			...
			if '->' in line or 'relates to' in line.lower() or 'connected to' in line.lower():
				parts = line.split('->')
				if len(parts) == 2:
	...
	def _relationships_similar(self, rel1: Relationship, rel2: Relationship) -> bool:
		...
		return (
			rel1.relationship_type == rel2.relationship_type and
			self._entities_similar(rel1.source_entity, rel2.source_entity) and
			self._entities_similar(rel1.target_entity, rel2.target_entity)
		)
	...
	def _extract_relationship_context(self, text: str, relationship: Relationship) -> str:
	...
	def _build_relationship_graph(self, relationships: List[Relationship]) -> RelationshipGraph:
		...
		statistics = {
			...
			'average_connections_per_entity': len(relationships) / max(len(entities), 1),
			'most_connected_entities': self._find_most_connected_entities(adjacency_matrix),
			'relationship_type_distribution': self._get_relationship_type_distribution(relationships)
		}
	...
	def _find_most_connected_entities(self, adjacency_matrix: Dict[str, List[str]]) -> List[Tuple[str, int]]:
	...
	def _calculate_relationship_statistics(self, relationships: List[Relationship]) -> Dict[str, Any]:
		...
		return {
			...
			'relationships_with_evidence': len([r for r in relationships if r.evidence]),
			'dependency_parsing_relationships': len([r for r in relationships if r.dependency_path])
		}
	...
	async def analyze_relationship_patterns(self, relationships: List[Relationship]) -> Dict[str, Any]:
  ...
  "gaps": ["Missing timeline relationships", "Unclear ownership structures"]
	...
	def get_extractor_info(self) -> Dict[str, Any]:
		...
		return {
			...
			'extraction_methods': ['dependency_parsing', 'pattern_matching', 'ai_enhancement'],
			'spacy_available': HAS_SPACY_SUPPORT,
			'dependency_parsing_available': bool(self.nlp_model and 'parser' in self.nlp_model.pipe_names),
...
# Factory function
def create_relationship_extractor(config: Optional[Dict[str, Any]] = None) -> RelationshipExtractor:
