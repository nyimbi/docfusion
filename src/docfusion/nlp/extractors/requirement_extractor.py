...
class RequirementExtractor:
	...
	def _get_default_config(self) -> Dict[str, Any]:
		...
		return {
			...
			'mandatory_indicators': [
				...
				'needs to', 'has to', 'is required to', 'obligated to'
			],
			'optional_indicators': [
				'may', 'should', 'could', 'preferred', 'desirable',
				'nice to have', 'optional', 'if possible'
			],
			'compliance_indicators': [
				'comply with', 'accordance with', 'conform to', 'meet standards',
			...
			'requirement_sections': [
				...
				'deliverables', 'objectives', 'criteria', 'standards'
			],
	...
	def _compile_patterns(self):
		...
		mandatory_words = '|'.join(re.escape(word) for word in self.config['mandatory_indicators'])
		self.mandatory_pattern = re.compile(
			rf'\b(?:{mandatory_words})\b.*?(?:\.|;|$)',
		...
		self.optional_pattern = re.compile(
			rf'\b(?:{optional_words})\b.*?(?:\.|;|$)',
			re.IGNORECASE | re.MULTILINE
		)
		
		# Compliance patterns
		compliance_words = '|'.join(re.escape(word) for word in self.config['compliance_indicators'])
		self.compliance_pattern = re.compile(
			rf'\b(?:{compliance_words})\b.*?(?:\.|;|$)',
			re.IGNORECASE | re.MULTILINE
		)
	...
	def _load_vocabularies(self):
		...
		self.technical_keywords = {
			'software', 'application', 'system', 'platform', 'database',
			'api', 'interface', 'integration', 'architecture', 'framework',
			'security', 'authentication', 'authorization', 'encryption',
			'performance', 'scalability', 'availability', 'reliability',
			'backup', 'recovery', 'monitoring', 'logging', 'testing'
		}
		
		self.business_keywords = {
			'process', 'workflow', 'business rule', 'governance',
			'compliance', 'audit', 'reporting', 'analytics',
			'user experience', 'training', 'support', 'maintenance',
			'budget', 'cost', 'timeline', 'deliverable', 'milestone'
		}
		
		self.compliance_keywords = {
			'gdpr', 'hipaa', 'sox', 'pci', 'iso', 'nist', 'cmmi',
			'regulation', 'standard', 'policy', 'procedure',
			'certification', 'accreditation', 'audit'
		}
	...
	async def extract_requirements(
		...
		try:
			...
			if (use_ai or self.config['use_ai_enhancement']) and len(text) < 100000:
				ai_reqs, doc_analysis = await self._ai_enhanced_extraction(text, document_type)
			...
			analyzed_requirements = await self._analyze_requirements(merged_requirements, text)
			...
			self.logger.info(f"Requirements extracted successfully, time: {result.processing_time:.2f}s")
	...
	async def _keyword_based_extraction(self, text: str) -> List[Requirement]:
		...
		for sentence in sentences:
			...
			compliance_score = sum(1 for keyword in self.compliance_keywords 
									if keyword.lower() in sentence.lower())
			
			# Determine if sentence contains requirements
			if tech_score >= 2 or business_score >= 2 or compliance_score >= 1:
				req_type = RequirementType.TECHNICAL if tech_score > business_score else RequirementType.BUSINESS
				if compliance_score > 0:
					req_type = RequirementType.COMPLIANCE
				
				priority = self._determine_priority_from_text(sentence)
				confidence = min(0.9, 0.3 + (tech_score + business_score + compliance_score) * 0.1)
	...
	async def _extract_from_section(self, section_text: str, section_name: str, offset: int) -> List[Requirement]:
	...
	async def _ai_enhanced_extraction(self, text: str, document_type: Optional[str]) -> Tuple[List[Requirement], Dict[str, Any]]:
		...
		try:
			...
			if len(text) < self.config['ai_chunk_size']:
				global_analysis = await self._perform_global_requirement_analysis(text, document_type)
	...
	async def _analyze_chunk_for_requirements(self, chunk: str, chunk_idx: int, document_type: Optional[str]) -> Tuple[List[Requirement], Dict[str, Any]]:
...
{{
  ...
  "document_insights": {{
		...
		try:
			...
			if response.status_code == 200:
				...
				try:
					...
					requirements = self._convert_ai_requirements(ai_data.get('requirements', []), chunk)
					...
					requirements = self._parse_ai_text_requirements(response_text, chunk)
	...
	async def _perform_global_requirement_analysis(self, text: str, document_type: Optional[str]) -> Dict[str, Any]:
...
{{
  ...
  "overall_assessment": "Well-structured RFP with clear requirements"
	...
	def _convert_ai_requirements(self, ai_requirements: List[Dict[str, Any]], chunk: str) -> List[Requirement]:
		...
		for ai_req in ai_requirements:
			try:
				req = Requirement(
					...
					requirement_type=self._map_ai_type_to_requirement_type(ai_req.get('type', 'functional')),
					priority=self._map_ai_priority_to_requirement_priority(ai_req.get('priority', 'essential')),
					clarity=self._map_ai_clarity_to_requirement_clarity(ai_req.get('clarity', 'clear')),
	...
	def _parse_ai_text_requirements(self, response_text: str, chunk: str) -> List[Requirement]:
		...
		for line in lines:
			...
			if any(indicator in line.lower() for indicator in ['requirement:', 'must', 'shall', 'should']):
	...
	def _map_ai_priority_to_requirement_priority(self, ai_priority: str) -> RequirementPriority:
	...
	def _map_ai_clarity_to_requirement_clarity(self, ai_clarity: str) -> RequirementClarity:
		...
		return clarity_mapping.get(ai_clarity.lower(), RequirementClarity.SOMEWHAT_CLEAR)
	...
	async def _analyze_requirements(self, requirements: List[Requirement], full_text: str) -> List[Requirement]:
	...
	def _group_requirements(self, requirements: List[Requirement]) -> List[RequirementGroup]:
		"""Group related requirements"""
		groups = []
		...
		for req_type, req_ids in type_groups.items():
			if len(req_ids) > 1:  # Only create groups with multiple requirements
				...
				req_priorities = [req.priority for req in requirements if req.id in req_ids]
				group_priority = min(req_priorities) if req_priorities else RequirementPriority.ESSENTIAL
	...
	def _calculate_statistics(self, requirements: List[Requirement]) -> Dict[str, Any]:
		...
		return {
			...
			'requirements_with_measurements': len([r for r in requirements if r.measurement_criteria]),
			'compliance_requirements': len([r for r in requirements if r.requirement_type == RequirementType.COMPLIANCE]),
			'mandatory_requirements': len([r for r in requirements if r.priority == RequirementPriority.MANDATORY]),
			'unclear_requirements': len([r for r in requirements if r.clarity in [RequirementClarity.AMBIGUOUS, RequirementClarity.UNCLEAR]])
		}
	...
	def _determine_priority_from_text(self, text: str) -> RequirementPriority:
		...
		elif any(word in text_lower for word in ['preferred', 'desirable', 'nice to have']):
	...
	def _assess_requirement_clarity(self, text: str) -> RequirementClarity:
		...
		unclear_indicators = ['tbd', 'tba', 'appropriate', 'reasonable', 'sufficient', 'adequate']
	...
	def _extract_keywords_from_sentence(self, text: str) -> List[str]:
		...
		all_keywords = self.technical_keywords.union(self.business_keywords).union(self.compliance_keywords)
	...
	def _determine_section_context(self, requirement: Requirement, full_text: str) -> Optional[str]:
		...
		for line in reversed(lines[-20:]):  # Check last 20 lines
			line = line.strip()
			if any(section in line.lower() for section in self.config['requirement_sections']):
				return line
	...
	async def identify_missing_requirements(self, text: str, domain: str = "general") -> Dict[str, Any]:
  ...
  "assessment": "Brief assessment of requirement completeness"
	...
	def get_extractor_info(self) -> Dict[str, Any]:
		...
		return {
			...
			'supported_priorities': [priority.value for priority in RequirementPriority],
			'extraction_methods': ['pattern_matching', 'keyword_detection', 'section_analysis', 'ai_enhancement'],
...
# Factory function
def create_requirement_extractor(config: Optional[Dict[str, Any]] = None) -> RequirementExtractor:
