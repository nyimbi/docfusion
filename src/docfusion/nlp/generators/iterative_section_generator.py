...
class IterativeSectionGenerator:
	...
	def __init__(
		...
		self.logger.info(f"IterativeSectionGenerator initialized with model: {ollama_model}")
	...
	def _initialize_narrative_templates(self):
		...
		self.narrative_templates = {
			'intro': {
				...
				'psychology_focus': [PsychologyBias.ANCHORING, PsychologyBias.EMOTIONAL_APPEAL],
			...
			'problem_identification': {
				...
				'psychology_focus': [PsychologyBias.PROBLEM_AGITATION, PsychologyBias.LOSS_AVERSION],
				'content_structure': ['current_state_analysis', 'pain_point_quantification', 'consequence_of_inaction'],
				'transition_out': 'Bridge to solution introduction'
			},
			
			'solution_presentation': {
				'purpose': 'Present solution and build credibility',
				'psychology_focus': [PsychologyBias.AUTHORITY, PsychologyBias.SOCIAL_PROOF],
				'content_structure': ['solution_overview', 'capabilities_demonstration', 'differentiation'],
				'transition_out': 'Connect to implementation and benefits'
			},
			
			'benefit_realization': {
				'purpose': 'Demonstrate value and ROI',
				'psychology_focus': [PsychologyBias.ANCHORING, PsychologyBias.RECIPROCITY],
				'content_structure': ['quantified_benefits', 'roi_analysis', 'success_stories'],
			...
			'credibility_building': {
				...
				'psychology_focus': [PsychologyBias.AUTHORITY, PsychologyBias.SOCIAL_PROOF],
				'content_structure': ['team_expertise', 'past_successes', 'methodology_strength'],
				'transition_out': 'Connect to partnership and next steps'
			},
			
			'call_to_action': {
				'purpose': 'Drive decision and commitment',
				'psychology_focus': [PsychologyBias.SCARCITY, PsychologyBias.RECIPROCITY],
				'content_structure': ['clear_next_steps', 'urgency_drivers', 'partnership_invitation'],
	...
	def _initialize_repetition_prevention(self):
		...
		self.tracked_phrases = {
			'transition_phrases': [
				...
				"Building on this", "Taking this further", "Expanding on this"
			],
			'emphasis_phrases': [
				"Importantly", "Significantly", "Notably", "Crucially",
				"Key to success", "Essential for", "Critical to"
			],
			'benefit_phrases': [
				"This results in", "This leads to", "The outcome is",
				"Organizations achieve", "Clients realize", "The impact includes"
			]
		}
	...
	async def generate_complete_document(
		...
		try:
			...
			self.logger.info(f"Starting iterative generation of {len(ordered_elements)} sections")
			...
			for element in ordered_elements:
				try:
					...
					if section_content:
						...
						pipeline.total_repetition_retries += section_content.retries_due_to_repetition
						...
						self.logger.info(f"Generated section: {section_content.title} ({section_content.word_count} words)")
					else:
						result.sections_failed += 1
						result.warnings.append(f"Failed to generate content for section: {element.title}")
				
				except Exception as e:
					result.sections_failed += 1
					result.errors.append(f"Error generating section '{element.title}': {str(e)}")
			...
			self.logger.info(f"Document generation completed: {result.sections_generated} sections, "
	...
	def _determine_generation_order(self, elements: List[OutlineElement]) -> List[OutlineElement]:
		...
		if self.generation_strategy == ContentGenerationStrategy.SEQUENTIAL:
			...
			return sorted(all_elements, key=lambda x: priority_order.get(x.priority, 2), reverse=True)
	...
	def _flatten_outline_elements(self, elements: List[OutlineElement]) -> List[OutlineElement]:
	...
	def _optimize_for_psychology_flow(self, elements: List[OutlineElement]) -> List[OutlineElement]:
		...
		for element in elements:
			...
			if any(word in title_lower for word in ['summary', 'overview', 'introduction']):
				intro_elements.append(element)
			elif any(word in title_lower for word in ['problem', 'challenge', 'need', 'pain', 'current']):
				problem_elements.append(element)
			elif any(word in title_lower for word in ['solution', 'approach', 'method', 'strategy']):
				solution_elements.append(element)
			elif any(word in title_lower for word in ['team', 'experience', 'expertise', 'credential', 'about']):
				credibility_elements.append(element)
			elif any(word in title_lower for word in ['benefit', 'value', 'roi', 'impact', 'result']):
				benefit_elements.append(element)
			elif any(word in title_lower for word in ['next', 'timeline', 'implementation', 'conclusion']):
				action_elements.append(element)
			else:
				other_elements.append(element)
		
		# Order for optimal persuasive flow: intro -> problem -> solution -> benefits -> credibility -> action
		ordered = (intro_elements + problem_elements + solution_elements + 
				  benefit_elements + credibility_elements + other_elements + action_elements)
		
		return ordered
	
	def _resolve_dependencies(self, elements: List[OutlineElement]) -> List[OutlineElement]:
		...
		for attempt in range(self.max_retries_per_section):
			try:
				...
				section_content = SectionContent(
					...
					generation_time=asyncio.get_event_loop().time() - section_start_time,
					model_calls=1,
					retries_due_to_repetition=attempt
				)
				
				# Check for repetition against existing content
				repetition_score = await self._assess_repetition_risk(section_content, pipeline)
				...
				if repetition_score < 0.7:  # Threshold for acceptable repetition
					await self._enhance_section_content(section_content, element, pipeline)
					return section_content
				
				else:
					self.logger.warning(f"High repetition risk ({repetition_score:.3f}) for section {element.title}, retrying...")
					continue
			
			except Exception as e:
				self.logger.error(f"Error generating section content (attempt {attempt + 1}): {e}")
				continue
		
		# If all retries failed
		self.logger.error(f"Failed to generate acceptable content for section: {element.title}")
		return None
	
	def _determine_narrative_role(self, element: OutlineElement, pipeline: ContentPipeline) -> str:
		...
		if any(word in title_lower for word in ['summary', 'overview']) and position == 0:
			return 'intro'
		elif any(word in title_lower for word in ['problem', 'challenge', 'need', 'current']):
		...
		elif any(word in title_lower for word in ['next', 'timeline', 'conclusion']) or position > total_sections * 0.8:
	...
	async def _build_section_prompt(
		...
		section_context = self._build_section_context(element, outline, pipeline, context)
		
		# Get repetition prevention instructions
		repetition_instructions = self._build_repetition_prevention_instructions(pipeline, element)
...
Generate the section content now:"""
	...
	def _get_section_reasoning_steps(self, element: OutlineElement, narrative_role: str) -> List[str]:
	...
	def _build_section_context(
		...
		if pipeline.sections:
			...
			for prev_section in pipeline.sections[-2:]:  # Last 2 sections for context
				context_parts.append(f"- {prev_section.title}: {prev_section.content[:200]}...")
	...
	def _build_repetition_prevention_instructions(
		...
		if pipeline.used_concepts:
			...
			instructions.append(f"**Avoid repeating these concepts**: {', '.join(concepts_list)}")
		...
		if used_biases and element.psychology_biases:
			available_biases = [bias for bias in element.psychology_biases if bias not in used_biases]
			if available_biases:
				bias_names = [bias.value for bias in available_biases]
				instructions.append(f"**Emphasize these fresh biases**: {', '.join(bias_names)}")
		
		instructions.append("**Generate unique content** that advances the narrative without repeating previous points.")
		
		return "\\n".join(instructions)
	
	def _build_psychology_guidance(self, element: OutlineElement, narrative_role: str) -> str:
		...
		for bias in element.psychology_biases:
			if bias.value in self.psychology_applications:
				...
				if app['intro_phrases']:
					...
					guidance_parts.append(f"- Consider openings like: {' OR '.join(phrases)}")
	...
	def _get_section_examples(self, element: OutlineElement, narrative_role: str) -> List[Dict[str, str]]:
	...
	async def _call_generation_api(self, prompt: str, element: OutlineElement) -> Optional[str]:
	...
	async def _assess_repetition_risk(self, section_content: SectionContent, pipeline: ContentPipeline) -> float:
		...
		elif self.repetition_prevention == RepetitionPreventionMode.CONTENT_FINGERPRINTING:
			return self._assess_content_fingerprints(section_content, pipeline)
		
		return 0.0  # No repetition checking
	
	def _assess_keyword_repetition(self, section_content: SectionContent, pipeline: ContentPipeline) -> float:
		...
		phrase_overlap = sum(1 for phrase in pipeline.used_phrases 
	...
	def _assess_semantic_similarity(self, section_content: SectionContent, pipeline: ContentPipeline) -> float:
		...
		new_words = set(word.lower().strip('.,!?;:') for word in section_content.content.split()
						if word.lower() not in stop_words and len(word) > 3)
		
		# Compare with all previous sections
		max_similarity = 0.0
		for prev_section in pipeline.sections:
			prev_words = set(word.lower().strip('.,!?;:') for word in prev_section.content.split()
							if word.lower() not in stop_words and len(word) > 3)
	...
	async def _assess_ai_repetition_review(self, section_content: SectionContent, pipeline: ContentPipeline) -> float:
...
Return only a single number between 0.0 and 1.0:"""
			...
			async with aiohttp.ClientSession(timeout=timeout) as session:
				async with session.post(
					...
					if response.status == 200:
						...
						import re
						score_match = re.search(r'\\b(0\\.\\d+|1\\.0|0)\\b', response_text)
						if score_match:
	...
	def _assess_content_fingerprints(self, section_content: SectionContent, pipeline: ContentPipeline) -> float:
	...
	async def _enhance_section_content(
		...
		section_content.key_concepts = self._extract_key_concepts(section_content.content)
		
		# Record psychology biases used
		section_content.psychology_biases_used = element.psychology_biases.copy()
		
		# Extract value propositions
		section_content.unique_value_propositions = self._extract_value_propositions(section_content.content)
		
		# Assess quality scores
		section_content.psychology_integration_score = self._assess_psychology_integration(section_content, element)
		section_content.coherence_score = self._assess_section_coherence(section_content)
		section_content.persuasiveness_score = self._assess_persuasiveness(section_content)
		
		# Generate transitions
		section_content.transition_in = self._generate_transition_in(section_content, pipeline)
		section_content.transition_out = self._generate_transition_out(section_content, element)
	...
	def _extract_key_concepts(self, content: str) -> List[str]:
		...
		technical_terms = re.findall(r'\\b\\w*(?:system|platform|solution|framework|methodology|approach)\\w*\\b', content, re.IGNORECASE)
	...
	def _extract_value_propositions(self, content: str) -> List[str]:
		...
		value_indicators = [
			'benefit', 'advantage', 'improvement', 'increase', 'reduce', 'save', 
			'roi', 'return', 'efficiency', 'cost', 'revenue', 'profit'
		]
	...
	def _assess_psychology_integration(self, section_content: SectionContent, element: OutlineElement) -> float:
	...
	def _assess_persuasiveness(self, section_content: SectionContent) -> float:
		...
		persuasive_indicators = [
			'proven', 'results', 'success', 'achieve', 'improve', 'increase',
			'reduce', 'save', 'roi', 'benefit', 'advantage', 'unique', 'exclusive',
			'leading', 'expert', 'industry', 'award', 'recognized'
		]
	...
	def _generate_transition_in(self, section_content: SectionContent, pipeline: ContentPipeline) -> str:
	...
	def _generate_transition_out(self, section_content: SectionContent, element: OutlineElement) -> str:
	...
	def _update_content_tracking(self, pipeline: ContentPipeline, section_content: SectionContent):
		"""Update pipeline tracking for repetition prevention"""
		
		# Add key concepts to tracking
		pipeline.used_concepts.update(concept.lower() for concept in section_content.key_concepts)
		
		# Add value propositions to tracking
		pipeline.used_phrases.update(prop.lower() for prop in section_content.unique_value_propositions)
	...
	async def _post_process_content(self, pipeline: ContentPipeline, outline: DocumentOutline):
		"""Post-process generated content for final optimization"""
		
		# Calculate pipeline statistics
		pipeline.total_generation_time = sum(section.generation_time for section in pipeline.sections)
	...
	async def _ensure_document_coherence(self, pipeline: ContentPipeline, outline: DocumentOutline):
		...
		if pipeline.sections:
			coherence_scores = [section.coherence_score for section in pipeline.sections]
			pipeline.overall_coherence = sum(coherence_scores) / len(coherence_scores)
		else:
			pipeline.overall_coherence = 0.0
	
	async def _assess_pipeline_quality(self, result: IterativeGenerationResult, pipeline: ContentPipeline):
		...
		for section in pipeline.sections:
			...
			section_quality = (section.coherence_score + section.persuasiveness_score) / 2
			quality_scores.append(section_quality)
			
			psychology_scores.append(section.psychology_integration_score)
			repetition_scores.append(1.0 - section.repetition_risk_score)  # Invert repetition risk
...
# Factory function
def create_iterative_section_generator(config: Optional[Dict[str, Any]] = None) -> IterativeSectionGenerator:
