...
class StyleTransformer:
    ...
    async def transform_tone(
        ...
        try:
            ...
            if result.primary_result:
                transformed_scores = await self._analyze_text_style(
                    result.primary_result.transformed_text
                )
                ...
                result.primary_result.style_delta = self._calculate_style_delta(
            ...
            result.transformation_suggestions = (
                await self._generate_transformation_suggestions(result)
            )
    ...
    async def transform_to_style_profile(
        ...
        try:
            ...
            transformations_needed = self._plan_style_transformations(
            ...
            if transformation_chain:
                ...
                final_result.transformation_type = (
                    ToneTransformation.MORE_PROFESSIONAL
                )  # Generic
                final_result.original_style_scores = current_scores
                final_result.transformed_style_scores = await self._analyze_text_style(
                    current_text
                )
    ...
    async def _analyze_text_style(self, text: str) -> Dict[str, float]:
        ...
        informal_count = sum(
            1 for indicator in informal_indicators if indicator in text
        )
        ...
        scores["technicality"] = (
            min(technical_count / len(words) * 100, 1.0) if words else 0
        )
        ...
        scores["persuasiveness"] = (
            min(persuasive_count / len(words) * 50, 1.0) if words else 0
        )
        ...
        direct_count = sum(
            1 for indicator in direct_indicators if indicator in text_lower
        )
        indirect_count = sum(
            1 for indicator in indirect_indicators if indicator in text_lower
        )
    ...
    def _calculate_style_delta(
        self, original: Dict[str, float], transformed: Dict[str, float]
    ) -> Dict[str, float]:
    ...
    def _plan_style_transformations(
        self, current_scores: Dict[str, float], target_profile: StyleProfile
    ) -> List[Tuple[ToneTransformation, float]]:
                    ...
                    if attribute == "formality":
                        if difference > 0:
                            transformations.append(
                                (ToneTransformation.MORE_FORMAL, abs(difference))
                            )
                        else:
                            transformations.append(
                                (ToneTransformation.LESS_FORMAL, abs(difference))
                            )

                    elif attribute == "technicality":
                        if difference > 0:
                            transformations.append(
                                (ToneTransformation.MORE_TECHNICAL, abs(difference))
                            )
                        else:
                            transformations.append(
                                (ToneTransformation.LESS_TECHNICAL, abs(difference))
                            )

                    elif attribute == "confidence":
                        if difference > 0:
                            transformations.append(
                                (ToneTransformation.MORE_CONFIDENT, abs(difference))
                            )

                    elif attribute == "persuasiveness":
                        if difference > 0:
                            transformations.append(
                                (ToneTransformation.MORE_PERSUASIVE, abs(difference))
                            )

                    elif attribute == "directness":
                        if difference > 0:
                            transformations.append(
                                (ToneTransformation.MORE_DIRECT, abs(difference))
                            )
        ...
        return transformations[
            :3
        ]  # Limit to 3 transformations to avoid over-processing
    ...
    async def _ai_transform_tone(
        ...
        structure_instruction = (
            ...
            else "Feel free to reorganize for better flow."
        )
...
Rewritten text:"""
            ...
            async with (
                ...
                if response.status == 200:
                    ...
                    result.confidence_score = (
                        0.8  # High confidence for AI transformations
                    )
    ...
    async def _rule_based_transform(
        self, text: str, transformation: ToneTransformation, intensity: float
    ) -> TransformationResult:
    ...
    async def _validate_transformation_quality(self, result: StyleTransformationResult):
        ...
        length_change = (
            abs(len(transformed) - len(original)) / len(original) if original else 0
        )
        ...
        if result.content_preservation < 0.6:
            result.warnings.append(
                "Significant content changes detected - meaning may be altered"
            )

        if result.style_consistency < 0.5:
            result.warnings.append(
                "Style changes may not align with intended transformation"
            )

        if result.transformation_quality < 0.6:
            result.warnings.append(
                "Overall transformation quality is below recommended threshold"
            )

    async def _generate_transformation_suggestions(
        self, result: StyleTransformationResult
    ) -> List[str]:
        ...
        if result.transformation_quality < 0.7:
            suggestions.append(
                "Consider adjusting transformation intensity for better results"
            )
        ...
        if result.primary_result.style_delta:
            for attribute, delta in result.primary_result.style_delta.items():
                if abs(delta) < 0.1:
                    suggestions.append(
                        f"Consider stronger {attribute} adjustment for more noticeable change"
                    )
                elif abs(delta) > 0.8:
                    suggestions.append(
                        f"{attribute.capitalize()} change may be too extreme"
                    )
        ...
        if transformation == ToneTransformation.MORE_FORMAL:
            suggestions.append(
                "Consider expanding contractions and using more formal vocabulary"
            )
        elif transformation == ToneTransformation.MORE_PERSUASIVE:
            suggestions.append(
                "Add benefit-focused language and compelling calls to action"
            )
        elif transformation == ToneTransformation.MORE_CONFIDENT:
            suggestions.append(
                "Remove hedging words and use more definitive statements"
            )

        return suggestions[:5]  # Limit to 5 suggestions

    def _compile_transformation_statistics(
        self, result: StyleTransformationResult, original_text: str
    ) -> Dict[str, Any]:
        ...
        if result.primary_result:
            stats["text_changes"] = {
                ...
                "length_change_ratio": len(result.primary_result.transformed_text)
                / len(original_text)
                if original_text
                else 1.0,
    ...
    def get_transformer_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_transformations": [
                transform.value for transform in ToneTransformation
            ],
