...
class DocumentSummarizer:
	...
	def _initialize_key_phrases(self):
		...
		self.importance_indicators = {
			'high': [
				'key finding', 'main result', 'primary objective', 'critical issue',
				'important factor', 'significant impact', 'major benefit', 'core requirement',
				'essential element', 'fundamental principle', 'primary goal', 'main purpose'
			],
			'medium': [
				'notable', 'significant', 'important', 'relevant', 'substantial', 'considerable',
				'effective', 'successful', 'valuable', 'beneficial', 'useful', 'practical'
			],
			'structural': [
				'in conclusion', 'to summarize', 'in summary', 'overall', 'finally',
				'first', 'second', 'third', 'initially', 'subsequently', 'therefore',
	...
	async def summarize_document(
		...
		try:
			...
			if len(sentences) < 2:
				result.warnings.append("Document too short for meaningful summarization")
			...
			if summary_type == SummaryType.EXTRACTIVE or (not self.use_ai_enhancement and not aiohttp):
				await self._generate_extractive_summary(result, sentence_scores, length, focus, custom_word_limit)
			
			elif summary_type == SummaryType.ABSTRACTIVE and self.use_ai_enhancement and aiohttp:
				try:
					await self._generate_abstractive_summary(result, text, length, focus, custom_word_limit)
				...
				await self._generate_hybrid_summary(result, text, sentence_scores, length, focus, custom_word_limit)
				if self.use_ai_enhancement and aiohttp:
					result.model_calls += 1
			
			elif summary_type == SummaryType.BULLET_POINTS:
				await self._generate_bullet_summary(result, text, sentence_scores, length, focus)
				if self.use_ai_enhancement and aiohttp:
					result.model_calls += 1
			
			elif summary_type == SummaryType.EXECUTIVE:
				await self._generate_executive_summary(result, text, length, custom_word_limit)
				if self.use_ai_enhancement and aiohttp:
					result.model_calls += 1
			...
			if not result.key_points:
				result.key_points = await self._extract_key_points_from_summary(result.summary_text)
			...
			self.logger.info(f"Document summarization completed: {summary_type.value}, "
	...
	async def _score_sentences(self, sentences: List[str], full_text: str) -> List[SentenceScore]:
		...
		if TfidfVectorizer and len(sentences) > 1:
			try:
				...
				for i, sentence in enumerate(sentences):
					tfidf_scores[i] = float(np.mean(tfidf_matrix[i].toarray())) if np else 0.5
			except Exception as e:
		...
		for i, sentence in enumerate(sentences):
			...
			for importance_level, phrases in self.importance_indicators.items():
				weight = {'high': 1.0, 'medium': 0.6, 'structural': 0.4}.get(importance_level, 0.3)
			...
			if TfidfVectorizer and cosine_similarity:
				...
				score.similarity_score = min(overlap / len(sentence_words), 1.0) if sentence_words else 0
	...
	async def _extract_named_entities(self, text: str) -> List[str]:
		...
		org_patterns = re.findall(r'\b[A-Z][a-z]+(?:\s+[A-Z&][a-z]*)*(?:\s+(?:Inc|LLC|Corp|Ltd|Co)\.?)\b', text)
	...
	async def _generate_extractive_summary(
		...
		for score in filtered_scores:
			...
			if current_word_count >= target_words * 0.9:  # Stop at 90% to avoid going over
				break
	...
	def _apply_focus_filter(self, sentence_scores: List[SentenceScore], focus: SummaryFocus) -> List[SentenceScore]:
		"""Filter sentences based on summary focus"""
		focus_keywords = {
			SummaryFocus.METHODOLOGY: [
				'approach', 'method', 'process', 'procedure', 'technique', 'strategy',
				'implementation', 'framework', 'system', 'workflow'
			],
			SummaryFocus.RESULTS: [
				'result', 'outcome', 'finding', 'conclusion', 'achievement', 'success',
				'performance', 'metric', 'measurement', 'data'
			],
			SummaryFocus.RECOMMENDATIONS: [
				'recommend', 'suggest', 'propose', 'should', 'must', 'advise',
				'next steps', 'action', 'improvement', 'enhance'
			],
			SummaryFocus.TECHNICAL_DETAILS: [
				'technical', 'system', 'architecture', 'configuration', 'specification',
				'implementation', 'code', 'software', 'hardware', 'infrastructure'
			],
			SummaryFocus.BUSINESS_VALUE: [
				'value', 'benefit', 'roi', 'cost', 'profit', 'efficiency', 'improvement',
				'advantage', 'opportunity', 'impact', 'business'
			]
		}
...
Summary:"""
		...
		async with aiohttp.ClientSession(timeout=timeout) as session:
			async with session.post(
				...
				if response.status == 200:
					...
					result.tokens_used = len(prompt.split()) * 1.3 + len(summary_text.split()) * 1.3
	...
	async def _generate_hybrid_summary(
		...
		await self._generate_extractive_summary(temp_result, sentence_scores, length, focus, custom_word_limit)
				...
				async with aiohttp.ClientSession(timeout=timeout) as session:
					async with session.post(
						...
						if response.status == 200:
							...
							segment.segment_type = "content"
							segment.importance = 1.0
							segment.source_sentences = [score.sentence for score in sentence_scores[:5]]
							result.summary_segments = [segment]
...
Bullet points:"""
				...
				await self._generate_extractive_bullets(result, sentence_scores, num_bullets)
		else:
			await self._generate_extractive_bullets(result, sentence_scores, num_bullets)
	
	async def _generate_extractive_bullets(self, result: SummaryResult, sentence_scores: List[SentenceScore], num_bullets: int):
	...
	async def _generate_executive_summary(
		...
		if not self.use_ai_enhancement or not aiohttp:
			...
			temp_result = SummaryResult()
			sentence_scores = await self._score_sentences(self._extract_sentences(text), text)
			await self._generate_extractive_summary(temp_result, sentence_scores, length, None, custom_word_limit)
			result.summary_text = temp_result.summary_text
			result.word_count = temp_result.word_count
			result.summary_segments = temp_result.summary_segments
			return
...
Executive Summary:"""
	...
	async def _assess_summary_quality(self, result: SummaryResult, original_text: str):
		...
		if result.key_topics:
			topics_in_summary = sum(1 for topic in result.key_topics if topic in result.summary_text.lower())
			informativeness_factors.append(topics_in_summary / len(result.key_topics))
		
		# Check for named entities
		if result.named_entities:
			entities_in_summary = sum(1 for entity in result.named_entities if entity in result.summary_text)
			informativeness_factors.append(entities_in_summary / len(result.named_entities))
		
		if informativeness_factors:
			result.informativeness = sum(informativeness_factors) / len(informativeness_factors)
		else:
	...
	async def _extract_key_points_from_summary(self, summary_text: str) -> List[str]:
		...
		for sentence in sentences:
			...
			if any(phrase in sentence_lower for phrase in ['key', 'important', 'critical', 'main', 'primary']):
				importance_score += 2
			
			if any(phrase in sentence_lower for phrase in ['significant', 'major', 'essential', 'fundamental']):
				importance_score += 1
			
			# Check for structural indicators
			if any(phrase in sentence_lower for phrase in ['first', 'second', 'finally', 'in conclusion']):
				importance_score += 1
	...
	def _compile_summarization_statistics(self, result: SummaryResult, original_text: str) -> Dict[str, Any]:
	...
	async def create_multi_perspective_summary(
		...
		tasks = {perspective.value: summarize_perspective(perspective) for perspective in perspectives}
		perspective_results = await asyncio.gather(*tasks.values(), return_exceptions=True)
		
		# Combine results
		for i, (perspective_name, result) in enumerate(zip(tasks.keys(), perspective_results)):
			if isinstance(result, Exception):
				error_result = SummaryResult()
				error_result.errors.append(f"Perspective summarization failed: {str(result)}")
	...
	def get_summarizer_info(self) -> Dict[str, Any]:
		...
		return {
			...
			'features': {
				...
				'abstractive_summarization': self.use_ai_enhancement and aiohttp is not None,
...
# Factory function
def create_document_summarizer(config: Optional[Dict[str, Any]] = None) -> DocumentSummarizer:
