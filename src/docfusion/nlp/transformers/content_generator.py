...
class ContentGenerator:
	...
	def __init__(
		...
		# If a centralized LLMConfiguration is provided, its fields override the defaults.
		if llm_config is not None:
			ollama_base_url = getattr(llm_config, "base_url", ollama_base_url)
			ollama_model = getattr(llm_config, "model", ollama_model)
			ollama_timeout = getattr(llm_config, "timeout", ollama_timeout)
			max_retries = getattr(llm_config, "max_retries", max_retries)
