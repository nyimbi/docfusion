...
class ContentGenerator:
	...
	def __init__(
		...
		self.logger.info(f"ContentGenerator initialized with model: {ollama_model} (advanced prompting: {use_advanced_prompting})")
	...
	def _initialize_templates(self):
		...
		self.content_templates = {
			...
			ContentType.EXECUTIVE_SUMMARY: {
				'structure': ['project_overview', 'key_benefits', 'approach_summary', 'expected_outcomes', 'call_to_action'],
				'length_guidance': 'medium',
				'key_elements': ['value_proposition', 'differentiators', 'roi_summary', 'next_steps'],
			...
			ContentType.EXECUTIVE_OVERVIEW: {
				'structure': ['situation_summary', 'proposed_solution', 'business_impact', 'investment_required'],
				'length_guidance': 'short',
				'key_elements': ['strategic_alignment', 'competitive_advantage', 'success_metrics'],
				'style_preference': GenerationStyle.C_LEVEL
			},
			ContentType.PROJECT_SUMMARY: {
				'structure': ['project_purpose', 'scope_overview', 'deliverables', 'timeline', 'success_criteria'],
				'length_guidance': 'medium',
				'key_elements': ['objectives', 'outcomes', 'stakeholders'],
				'style_preference': GenerationStyle.PROFESSIONAL
			},
			ContentType.BUSINESS_CASE: {
				'structure': ['problem_definition', 'proposed_solution', 'financial_analysis', 'risk_assessment', 'recommendation'],
				'length_guidance': 'long',
				'key_elements': ['cost_benefit_analysis', 'roi_calculation', 'implementation_roadmap'],
				'style_preference': GenerationStyle.ANALYTICAL
			},
			ContentType.VALUE_PROPOSITION: {
				'structure': ['customer_problem', 'unique_solution', 'proven_benefits', 'competitive_differentiation'],
				'length_guidance': 'short',
			...
			ContentType.TECHNICAL_OVERVIEW: {
				'structure': ['technology_landscape', 'architecture_design', 'implementation_approach', 'technical_benefits'],
				'length_guidance': 'long',
				'key_elements': ['specifications', 'methodologies', 'standards_compliance', 'scalability'],
			...
			ContentType.TECHNICAL_SPECIFICATION: {
				'structure': ['functional_specifications', 'non_functional_requirements', 'interface_specifications', 'data_requirements'],
				'length_guidance': 'detailed',
				'key_elements': ['performance_criteria', 'security_requirements', 'compliance_standards'],
				'style_preference': GenerationStyle.SPECIFICATION
			},
			ContentType.ARCHITECTURE_OVERVIEW: {
				'structure': ['architecture_principles', 'system_components', 'integration_patterns', 'scalability_design'],
				'length_guidance': 'long',
				'key_elements': ['design_patterns', 'technology_choices', 'performance_considerations'],
				'style_preference': GenerationStyle.ENGINEERING
			},
			ContentType.SECURITY_APPROACH: {
				'structure': ['security_framework', 'threat_analysis', 'protection_mechanisms', 'compliance_measures'],
				'length_guidance': 'long',
				'key_elements': ['access_control', 'data_protection', 'monitoring', 'incident_response'],
				'style_preference': GenerationStyle.TECHNICAL_DETAILED
			},
			
			# Methodology & Approach
			ContentType.APPROACH_METHODOLOGY: {
				'structure': ['methodology_overview', 'phase_breakdown', 'deliverables_mapping', 'quality_framework'],
				'length_guidance': 'long',
				'key_elements': ['best_practices', 'risk_mitigation', 'success_metrics', 'continuous_improvement'],
				'style_preference': GenerationStyle.CONSULTATIVE
			},
			ContentType.AGILE_APPROACH: {
				'structure': ['agile_principles', 'sprint_methodology', 'team_structure', 'delivery_cadence'],
				'length_guidance': 'medium',
				'key_elements': ['scrum_framework', 'user_stories', 'retrospectives', 'continuous_delivery'],
				'style_preference': GenerationStyle.COLLABORATIVE
			},
			ContentType.QUALITY_ASSURANCE: {
				'structure': ['qa_framework', 'testing_strategy', 'quality_metrics', 'continuous_monitoring'],
				'length_guidance': 'medium',
				'key_elements': ['test_automation', 'defect_prevention', 'performance_testing'],
				'style_preference': GenerationStyle.TECHNICAL
			},
			ContentType.TESTING_STRATEGY: {
				'structure': ['test_approach', 'test_levels', 'automation_strategy', 'defect_management'],
				'length_guidance': 'long',
				'key_elements': ['unit_testing', 'integration_testing', 'user_acceptance_testing'],
				'style_preference': GenerationStyle.TECHNICAL_DETAILED
			},
			
			# Project Management
			ContentType.PROJECT_TIMELINE: {
				'structure': ['project_phases', 'major_milestones', 'critical_dependencies', 'delivery_schedule'],
				'length_guidance': 'medium',
				'key_elements': ['deadlines', 'resource_allocation', 'risk_buffers', 'go_live_dates'],
				'style_preference': GenerationStyle.PROFESSIONAL
			},
			ContentType.WORK_BREAKDOWN: {
				'structure': ['work_packages', 'task_hierarchy', 'effort_estimates', 'resource_assignments'],
				'length_guidance': 'long',
			...
			ContentType.RESOURCE_ALLOCATION: {
				'structure': ['resource_requirements', 'skill_matrix', 'allocation_timeline', 'optimization_strategy'],
				'length_guidance': 'medium',
				'key_elements': ['team_composition', 'utilization_rates', 'capacity_planning'],
				'style_preference': GenerationStyle.ANALYTICAL
			},
			ContentType.COMMUNICATION_PLAN: {
				'structure': ['stakeholder_mapping', 'communication_matrix', 'reporting_schedule', 'escalation_procedures'],
				'length_guidance': 'medium',
				'key_elements': ['meeting_cadence', 'status_reporting', 'change_notifications'],
				'style_preference': GenerationStyle.PROFESSIONAL
			},
			
			# Risk & Compliance
			ContentType.RISK_ASSESSMENT: {
				'structure': ['risk_identification', 'probability_analysis', 'impact_assessment', 'mitigation_planning'],
				'length_guidance': 'long',
				'key_elements': ['risk_register', 'contingency_plans', 'monitoring_procedures'],
				'style_preference': GenerationStyle.ANALYTICAL
			},
			ContentType.RISK_MANAGEMENT: {
				'structure': ['risk_framework', 'identification_process', 'assessment_methodology', 'response_strategies'],
				'length_guidance': 'long',
				'key_elements': ['risk_appetite', 'escalation_criteria', 'review_cycles'],
				'style_preference': GenerationStyle.CONSULTATIVE
			},
			ContentType.COMPLIANCE_APPROACH: {
				'structure': ['regulatory_landscape', 'compliance_framework', 'control_mechanisms', 'audit_readiness'],
				'length_guidance': 'long',
				'key_elements': ['standards_adherence', 'documentation_requirements', 'reporting_obligations'],
				'style_preference': GenerationStyle.FORMAL
			},
			
			# Financial Content
			ContentType.COST_BREAKDOWN: {
				'structure': ['cost_categories', 'resource_costs', 'infrastructure_costs', 'contingency_provisions'],
				'length_guidance': 'medium',
				'key_elements': ['labor_costs', 'technology_costs', 'operational_expenses'],
				'style_preference': GenerationStyle.ANALYTICAL
			},
			ContentType.ROI_ANALYSIS: {
				'structure': ['investment_summary', 'benefit_quantification', 'payback_calculation', 'sensitivity_analysis'],
				'length_guidance': 'long',
				'key_elements': ['cost_savings', 'revenue_generation', 'productivity_gains'],
				'style_preference': GenerationStyle.FINANCIAL
			},
			ContentType.FINANCIAL_BENEFITS: {
				'structure': ['direct_savings', 'indirect_benefits', 'revenue_opportunities', 'long_term_value'],
				'length_guidance': 'medium',
				'key_elements': ['quantified_benefits', 'realization_timeline', 'measurement_approach'],
				'style_preference': GenerationStyle.BENEFIT_DRIVEN
			},
			
			# Team & Resources
			ContentType.TEAM_OVERVIEW: {
				'structure': ['team_composition', 'key_roles', 'experience_highlights', 'collaboration_model'],
				'length_guidance': 'medium',
				'key_elements': ['expertise_areas', 'past_successes', 'availability'],
				'style_preference': GenerationStyle.PROFESSIONAL
			},
			ContentType.KEY_PERSONNEL: {
				'structure': ['leadership_team', 'subject_matter_experts', 'key_contributors', 'advisory_support'],
				'length_guidance': 'long',
				'key_elements': ['qualifications', 'relevant_experience', 'role_responsibilities'],
				'style_preference': GenerationStyle.CONSULTATIVE
			},
			ContentType.ROLES_RESPONSIBILITIES: {
				'structure': ['organizational_structure', 'role_definitions', 'accountability_matrix', 'decision_authority'],
				'length_guidance': 'long',
			...
			ContentType.SOLUTION_OVERVIEW: {
				'structure': ['solution_architecture', 'key_capabilities', 'integration_approach', 'user_experience'],
				'length_guidance': 'long',
				'key_elements': ['functional_features', 'technical_specifications', 'deployment_model'],
				'style_preference': GenerationStyle.SOLUTION_FOCUSED
			},
			ContentType.BUSINESS_BENEFITS: {
				'structure': ['operational_improvements', 'cost_reductions', 'revenue_enhancements', 'strategic_advantages'],
				'length_guidance': 'medium',
				'key_elements': ['quantified_benefits', 'realization_timeline', 'success_metrics'],
				'style_preference': GenerationStyle.BENEFIT_DRIVEN
			},
			ContentType.COMPETITIVE_ADVANTAGES: {
				'structure': ['market_differentiation', 'unique_capabilities', 'proven_track_record', 'innovation_leadership'],
				'length_guidance': 'medium',
				'key_elements': ['differentiators', 'competitive_positioning', 'value_drivers'],
				'style_preference': GenerationStyle.COMPETITIVE
			},
			
			# Implementation & Deployment
			ContentType.IMPLEMENTATION_PLAN: {
				'structure': ['implementation_strategy', 'phase_approach', 'resource_mobilization', 'success_criteria'],
				'length_guidance': 'long',
				'key_elements': ['deployment_phases', 'change_management', 'training_program'],
				'style_preference': GenerationStyle.COMPREHENSIVE
			},
			ContentType.DEPLOYMENT_STRATEGY: {
				'structure': ['deployment_approach', 'rollout_phases', 'cutover_planning', 'rollback_procedures'],
				'length_guidance': 'long',
				'key_elements': ['go_live_strategy', 'user_adoption', 'support_model'],
				'style_preference': GenerationStyle.TECHNICAL
			},
			ContentType.MIGRATION_APPROACH: {
				'structure': ['migration_strategy', 'data_migration', 'system_cutover', 'validation_procedures'],
				'length_guidance': 'long',
				'key_elements': ['migration_tools', 'data_integrity', 'downtime_minimization'],
				'style_preference': GenerationStyle.TECHNICAL_DETAILED
			},
			
			# Experience & Credentials
			ContentType.COMPANY_BACKGROUND: {
				'structure': ['company_overview', 'core_competencies', 'market_presence', 'value_philosophy'],
				'length_guidance': 'medium',
				'key_elements': ['history', 'capabilities', 'differentiators'],
				'style_preference': GenerationStyle.PROFESSIONAL
			},
			ContentType.RELEVANT_EXPERIENCE: {
				'structure': ['project_portfolio', 'industry_experience', 'technology_expertise', 'success_metrics'],
				'length_guidance': 'long',
				'key_elements': ['case_studies', 'client_results', 'lessons_learned'],
				'style_preference': GenerationStyle.CONSULTATIVE
			},
			ContentType.CASE_STUDIES: {
				'structure': ['client_situation', 'solution_delivered', 'implementation_approach', 'results_achieved'],
				'length_guidance': 'long',
			...
			ContentType.FAQ_SECTION: {
				'structure': ['common_questions', 'detailed_answers', 'additional_resources', 'contact_information'],
				'length_guidance': 'medium',
			...
			ContentType.SCOPE_OF_WORK: {
				'structure': ['project_objectives', 'deliverables_list', 'activities_breakdown', 'acceptance_criteria'],
				'length_guidance': 'long',
				'key_elements': ['work_products', 'timelines', 'responsibilities'],
				'style_preference': GenerationStyle.FORMAL
			},
			ContentType.SUCCESS_CRITERIA: {
				'structure': ['success_definition', 'measurable_outcomes', 'acceptance_criteria', 'validation_approach'],
				'length_guidance': 'medium',
	...
	def _initialize_style_guides(self):
		...
		self.style_guides = {
			...
			GenerationStyle.EXECUTIVE: {
				...
				'key_phrases': ['strategic', 'value creation', 'competitive advantage', 'roi'],
			...
			GenerationStyle.CONSULTATIVE: {
				...
				'key_phrases': ['recommend', 'best practice', 'proven approach', 'insight'],
			...
			GenerationStyle.TECHNICAL_DETAILED: {
				...
				'key_phrases': ['specification', 'architecture', 'framework', 'methodology'],
			...
			GenerationStyle.INNOVATION_FOCUSED: {
				...
				'key_phrases': ['innovative', 'creative', 'breakthrough', 'cutting-edge'],
	...
	async def generate_content(
		...
		try:
			...
			prompt = await self._build_generation_prompt(content_type, context, style, format)
			...
			result.total_tokens_used += primary_content.prompt_tokens + primary_content.completion_tokens
			...
			if generate_alternatives and result.primary_content.quality_score > 0.6:
				...
				for alt in alternatives:
					result.total_tokens_used += alt.prompt_tokens + alt.completion_tokens
			
			# Step 4: Quality assessment
			await self._assess_content_quality(result, context)
			
			# Step 5: Generate suggestions and improvements
			result.suggestions = await self._generate_improvement_suggestions(result, context)
			
			# Step 6: Compile statistics
			result.statistics = self._compile_generation_statistics(result, context)
			
			result.success = len(result.errors) == 0 and result.primary_content is not None
			result.processing_time = asyncio.get_event_loop().time() - start_time
			
			self.logger.info(f"Content generation completed: {content_type.value}, "
							f"quality: {result.primary_content.quality_score:.3f}" if result.primary_content else "failed")
	...
	async def _build_generation_prompt(
		...
		if self.use_advanced_prompting and self.prompt_builder:
			...
			prompt.main_instruction = self._build_cot_prompt(content_type, context, style, format)
		else:
			# Fallback to basic prompting
			prompt.main_instruction = self._build_basic_prompt(content_type, context, style, format)
	...
	def _build_context_string(self, context: GenerationContext) -> str:
		...
		if context.key_points:
			context_parts.append(f"Key points to address: {', '.join(context.key_points)}")
		
		if context.requirements:
			context_parts.append(f"Requirements: {', '.join(context.requirements)}")
		
		if context.existing_content:
			context_parts.append(f"Existing content context: {context.existing_content[:500]}")
	...
	def _get_output_format_instructions(self, format: ContentFormat) -> str:
		...
		return format_instructions.get(format, "Use appropriate formatting for the content")
...
Please generate the content now:"""
		...
		if style == GenerationStyle.TECHNICAL:
			generation_params['temperature'] = 0.3  # More deterministic for technical content
		elif style == GenerationStyle.PERSUASIVE:
			generation_params['temperature'] = 0.8  # More creative for persuasive content
		...
		async with aiohttp.ClientSession(timeout=timeout) as session:
			async with session.post(
				...
				if response.status == 200:
					...
					if self.use_advanced_prompting and 'deepseek-r1' in self.ollama_model:
					...
					content.estimated_reading_time = content.word_count / 200  # Average reading speed
					
					# Estimate token usage (rough approximation)
					content.prompt_tokens = len(full_prompt.split()) * 1.3  # Rough token estimate
					content.completion_tokens = len(generated_text.split()) * 1.3
					
					# Basic quality assessment
					content.quality_score = await self._assess_single_content_quality(generated_text, context)
					content.style_score = await self._assess_style_consistency(generated_text, style)
	...
	async def _generate_alternatives(
		...
		for i in range(min(num_alternatives, len(variation_prompts))):
			try:
				...
				varied_prompt.main_instruction = f"{variation_prompts[i]}\n\n{prompt.main_instruction}"
	...
	async def _assess_content_quality(self, result: ContentGenerationResult, context: GenerationContext):
		...
		if target_range[0] <= word_count <= target_range[1]:
			...
			result.completeness_score = max(1.0 - (word_count - target_range[1]) / target_range[1] * 0.5, 0.5)
	...
	def _get_target_word_range(self, context: GenerationContext) -> tuple:
		...
		if context.word_count_target:
			...
			return (context.word_count_target - variance, context.word_count_target + variance)
	...
	async def _assess_single_content_quality(self, content: str, context: GenerationContext) -> float:
		...
		if target_range[0] <= word_count <= target_range[1]:
			...
			if word_count < target_range[0]:
				...
				quality_factors.append(max(1.0 - (word_count - target_range[1]) / target_range[1] * 0.3, 0.6))
		...
		if context.key_points:
			relevant_points = sum(1 for point in context.key_points if point.lower() in content_lower)
	...
	async def _assess_style_consistency(self, content: str, style: GenerationStyle) -> float:
		...
		if style == GenerationStyle.PROFESSIONAL:
			...
			professional_indicators = ['deliver', 'provide', 'ensure', 'develop', 'implement', 'achieve']
			indicator_count = sum(1 for indicator in professional_indicators if indicator in content_lower)
			style_score += min(indicator_count * 0.05, 0.2)
		
		elif style == GenerationStyle.TECHNICAL:
			# Check for technical precision
			technical_indicators = ['system', 'process', 'method', 'approach', 'technology', 'solution']
			indicator_count = sum(1 for indicator in technical_indicators if indicator in content_lower)
			style_score += min(indicator_count * 0.05, 0.2)
		
		elif style == GenerationStyle.PERSUASIVE:
			# Check for persuasive elements
			persuasive_indicators = ['benefit', 'advantage', 'opportunity', 'value', 'improvement', 'success']
			indicator_count = sum(1 for indicator in persuasive_indicators if indicator in content_lower)
			style_score += min(indicator_count * 0.05, 0.2)
	...
	async def _generate_improvement_suggestions(self, result: ContentGenerationResult, context: GenerationContext) -> List[str]:
		...
		if word_count < target_range[0]:
			suggestions.append(f"Consider expanding content (currently {word_count} words, target: {target_range[0]}-{target_range[1]})")
		elif word_count > target_range[1]:
			suggestions.append(f"Consider condensing content (currently {word_count} words, target: {target_range[0]}-{target_range[1]})")
		...
		if result.relevance_score < 0.7:
			suggestions.append("Better address the specified requirements and key points")
		...
		if context.key_points:
			content_lower = content.lower()
			unaddressed_points = [point for point in context.key_points if point.lower() not in content_lower]
			
			if unaddressed_points:
				suggestions.append(f"Address missing key points: {', '.join(unaddressed_points[:3])}")
	...
	def _compile_generation_statistics(self, result: ContentGenerationResult, context: GenerationContext) -> Dict[str, Any]:
...
# Factory function
def create_content_generator(config: Optional[Dict[str, Any]] = None) -> ContentGenerator:
