...
class ConflictDetector:
    ...
    async def detect_semantic_conflicts(
        self, content_a: str, content_b: str, author_a: str = "", author_b: str = ""
        ...
        return await self._detect_semantic_conflicts(
            content_a, content_b, author_a, author_b
        )
    ...
    async def detect_structural_conflicts(
        ...
        """
        return await self._detect_structural_conflicts(
            content_a, content_b, author_a, author_b
        )
    ...
    async def _are_conflicting_entities(
        self, entity_a: SemanticEntity, entity_b: SemanticEntity
    ) -> bool:
                ...
                if re.search(pattern_a, sent_a, re.IGNORECASE):
                    for sent_b in sentences_b:
                        if re.search(pattern_b, sent_b, re.IGNORECASE):
                            ...
                            common_words = set(sent_a.lower().split()) & set(
                                sent_b.lower().split()
                            )
    ...
    async def _calculate_conflict_statistics(
        ...
        for conflict in result.conflicts:
            ...
            result.conflicts_by_type[conflict_type] = (
                result.conflicts_by_type.get(conflict_type, 0) + 1
            )

        # Count by severity
        for conflict in result.conflicts:
            severity = conflict.severity.value
            result.conflicts_by_severity[severity] = (
                result.conflicts_by_severity.get(severity, 0) + 1
            )

    async def _generate_assessment_and_recommendations(
        self, result: ConflictAnalysisResult
    ) -> None:
        """Generate overall assessment and resolution recommendations."""
        if not result.conflicts:
            result.overall_assessment = (
                "No conflicts detected. Documents can be merged automatically."
            )
