...
from ..editing.collaborative_editor import (
	CollaborativeEditor, CollaborativeSession, Operation, User, DocumentState
)
from ..versioning.version_manager import (
	VersionManager, DocumentCommit, DocumentBranch, DocumentHistory
)
from ..conflicts.conflict_detector import (
	ConflictDetector, DetectedConflict, ConflictAnalysisResult
)
from ..conflicts.conflict_resolver import (
	ConflictResolver, ConflictResolutionResult, ResolutionStrategy
)
...
class CollaborationIntegrator:
	...
	def __init__(
		...
		self.storage_path = Path(storage_path) if storage_path else Path.cwd() / "collaborative_docs"
		...
		async with self._lock:
			...
			if doc.collaboration_settings.version_control_enabled:
				version_manager = VersionManager(
					doc.document_id,
					str(self.storage_path / doc.document_id)
				)
	...
	async def add_user_to_document(
		...
		if doc.collaborative_editor_active and document_id in self.collaborative_sessions:
			collab_session = self.collaborative_sessions[document_id]
			editor = await collab_session.get_session(document_id)
			if editor:
				await editor.add_user(user_id, user_name, permissions=session.permissions)
	...
	async def edit_document(
		...
		try:
			...
			await self._handle_document_sync(document_id, doc.collaboration_settings.sync_strategy)
	...
	async def resolve_conflicts(
		...
		conflicts = await self.conflict_detector.detect_conflicts(
			current_content, collab_content,
			author_a="current", author_b="collaborative",
			document_a_id=document_id, document_b_id=document_id
		)
		...
		if auto_resolve and conflicts.conflicts:
			...
			if resolution_result.merge_successful:
				...
				doc.total_conflicts_resolved += len(resolution_result.resolved_conflicts)
				doc.last_modified = datetime.now()
	...
	async def merge_document_branch(
		...
		merge_success, conflicts = await version_manager.merge_branch(
			user_id, source_branch, target_branch,
	...
	async def get_active_users(self, document_id: str) -> List[UserSession]:
		...
		doc = self.documents[document_id]
		active_sessions = [
			session for session in doc.active_sessions.values()
			if session.is_online and 
			(datetime.now() - session.last_activity).total_seconds() < 300  # 5 min timeout
		]
	...
	async def get_document_status(self, document_id: str) -> Dict[str, Any]:
		...
		status = {
			...
			"user_list": [{"user_id": u.user_id, "name": u.user_name} for u in active_users],
	...
	async def _get_version_manager_content(self, document_id: str) -> str:
		...
		if version_manager:
			...
			if main_branch and main_branch.head_commit_id:
				return await version_manager.get_document_at_commit(main_branch.head_commit_id)
	...
	async def _create_document_snapshot(self, doc: CollaborativeDocument) -> None:
		...
		snapshot = DocumentSnapshot(
			...
			active_users=[s.user_id for s in doc.active_sessions.values() if s.is_online]
		)
		...
		if len(self.document_snapshots[doc.document_id]) > 100:
			self.document_snapshots[doc.document_id] = self.document_snapshots[doc.document_id][-100:]
	...
	async def _update_performance_metrics(
		...
		if len(self.operation_metrics[document_id]) > 100:
			self.operation_metrics[document_id] = self.operation_metrics[document_id][-100:]
		
		# Update document average
		doc = self.documents[document_id]
		doc.average_sync_time_ms = sum(self.operation_metrics[document_id]) / len(self.operation_metrics[document_id])
	...
	async def _websocket_handler(self, event_type: str, event_data: Dict[str, Any]) -> None:
	...
	async def get_integration_statistics(self) -> Dict[str, Any]:
		...
		return {
			...
			"total_snapshots": sum(len(snaps) for snaps in self.document_snapshots.values()),
