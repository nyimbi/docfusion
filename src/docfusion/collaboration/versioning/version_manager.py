...
class VersionManager:
	...
	def __init__(self, document_id: str, storage_path: Optional[str] = None):
		...
		self.storage_path = Path(storage_path) if storage_path else Path.cwd() / "versions"
		...
		async with self._lock:
			...
			if not resolved_conflicts or all(c.resolved for c in resolved_conflicts):
				...
				merge_commit = DocumentCommit(
					...
					metadata={"merge_source": source_branch, "conflicts_resolved": len(resolved_conflicts)}
				)
				
				self.history.commits[merge_commit.commit_id] = merge_commit
				self.history.branches[target_branch].head_commit_id = merge_commit.commit_id
	...
	async def get_branch_history(
		...
		while current_commit_id and current_commit_id in self.history.commits:
			...
			current_commit_id = commit.parent_commits[0] if commit.parent_commits else None
	...
	async def _generate_changes(self, old_content: str, new_content: str) -> List[DocumentChange]:
		...
		for tag, i1, i2, j1, j2 in matcher.get_opcodes():
			if tag == 'replace':
				...
				conflict = MergeConflict(
					...
					context_after='\n'.join(target_lines[i2:min(len(target_lines), i2+3)]),
	...
	async def _reverse_changes(self, changes: List[DocumentChange]) -> List[DocumentChange]:
		...
		for change in reversed(changes):  # Apply in reverse order
			if change.change_type == ChangeType.INSERT:
				...
				reversed_changes.append(DocumentChange(
					change_type=ChangeType.DELETE,
	...
	async def _apply_changes(self, content: str, changes: List[DocumentChange]) -> str:
		...
		for change in sorted_changes:
			if change.change_type == ChangeType.INSERT:
				result = result[:change.position] + change.new_content + result[change.position:]
				...
				end_pos = change.position + change.length
				result = result[:change.position] + change.new_content + result[end_pos:]
