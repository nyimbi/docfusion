...
class DeliveryMetrics:
	...
	channel_performance: Dict[ChannelType, Dict[str, float]] = field(default_factory=dict)
...
class NotificationMessage(BaseModel):
	...
	retry_count: conint(ge=0, le=10) = Field(default=3, description="Maximum retry attempts")
	retry_delay: conint(ge=1, le=3600) = Field(default=60, description="Retry delay in seconds")
	...
	channel_data: Dict[str, Any] = Field(default_factory=dict, description="Channel-specific parameters")
	...
	tags: List[str] = Field(default_factory=list, description="Message classification tags")
...
class DeliveryResult(BaseModel):
	...
	# Tracking data
	tracking_id: Optional[str] = Field(None, description="External provider tracking ID")
...
class NotificationDelivery:
	...
	def __init__(
		...
		self.logger.info("NotificationDelivery initialized with %d channels", len(self.channels))
	...
	async def start(self) -> None:
		...
		self._running = True
		self.logger.info("Starting notification delivery service with %d workers", self.worker_count)
	...
	async def send_notification(self, message: NotificationMessage) -> str:
		...
		if message.scheduled_at and message.scheduled_at > datetime.now():
			...
			self.logger.debug("Scheduled message %s for delivery at %s", message.id, message.scheduled_at)
			return message.id
	...
	async def _validate_message(self, message: NotificationMessage) -> None:
		...
		if not await channel.validate_recipient(message.recipient_id):
			raise ValueError(f"Invalid recipient {message.recipient_id} for channel {message.channel.value}")
	...
	async def _delivery_worker(self, worker_id: str) -> None:
		...
		while self._running:
			try:
				...
				try:
					message = await asyncio.wait_for(self._delivery_queue.get(), timeout=1.0)
	...
	async def _process_message(self, message: NotificationMessage, worker_id: str) -> None:
		"""Process individual message delivery."""
		self.logger.debug("Worker %s processing message %s", worker_id, message.id)
		
		# Check expiry
		if message.expires_at and message.expires_at <= datetime.now():
			await self._record_delivery_result(message, DeliveryResult(
	...
	async def _record_delivery_result(self, message: NotificationMessage, result: DeliveryResult) -> None:
	...
	def _should_retry(self, message: NotificationMessage, result: DeliveryResult) -> bool:
		"""Determine if message should be retried."""
		if result.success:
			return False
		
		# Check retry count
		current_attempts = len([r for r in self._delivery_results.get(message.id, []) if not r.success])
	...
	async def _schedule_retry(self, message: NotificationMessage, last_result: DeliveryResult) -> None:
		"""Schedule message for retry delivery."""
		attempt_count = len([r for r in self._delivery_results.get(message.id, []) if not r.success])
	...
	async def _scheduler_worker(self) -> None:
		...
		while self._running:
			try:
				...
				if ready_messages:
					self.logger.debug("Scheduled %d messages for immediate delivery", len(ready_messages))
	...
	async def _apply_delivery_preferences(self, message: NotificationMessage) -> None:
		...
		if quiet_hours and message.priority != Priority.CRITICAL:
			...
			if start_hour <= current_hour or current_hour <= end_hour:
				...
				next_delivery = datetime.now().replace(hour=end_hour, minute=0, second=0, microsecond=0)
	...
	async def _update_metrics(self, result: DeliveryResult) -> None:
		...
		if self._metrics.total_sent > 0:
			self._metrics.success_rate = self._metrics.total_delivered / self._metrics.total_sent
		...
		if result.delivery_time_ms and result.success:
			...
			if total_delivered == 1:
				...
				self._metrics.average_delivery_time = (
					(current_avg * (total_delivered - 1) + result.delivery_time_ms) / total_delivered
				)
		
		# Update channel performance
		channel_perf = self._metrics.channel_performance.setdefault(result.channel, {
			'sent': 0,
			'delivered': 0,
			'failed': 0,
			'success_rate': 0.0,
			...
			if result.delivery_time_ms:
				...
				if delivered == 1:
					...
					channel_perf['avg_delivery_time'] = (
						(current_avg * (delivered - 1) + result.delivery_time_ms) / delivered
					)
		else:
		...
		if channel_perf['sent'] > 0:
			channel_perf['success_rate'] = channel_perf['delivered'] / channel_perf['sent']
	...
	async def _initialize_channel_health(self) -> None:
		...
		self.logger.debug("Initialized health monitoring for %d channels", len(self._channel_health))
