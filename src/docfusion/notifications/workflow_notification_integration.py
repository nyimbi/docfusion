...
class NotificationTemplate(BaseModel):
    ...
    default_priority: Priority = Field(
        Priority.MEDIUM, description="Default notification priority"
    )
    default_channels: List[ChannelType] = Field(
        default_factory=list, description="Default delivery channels"
    )

    # Conditions
    conditions: Dict[str, Any] = Field(
        default_factory=dict, description="Template activation conditions"
    )
...
class WorkflowNotificationIntegration:
    ...
    def __init__(
        ...
        self._user_channel_preferences: Dict[
            str, Dict[WorkflowEventType, List[ChannelType]]
        ] = {}
    ...
    async def handle_workflow_event(
        ...
        try:
            ...
            if not affected_users:
                affected_users = await self._determine_affected_users(
            ...
            if not templates:
                self.logger.warning(
                    "No templates found for event type: %s", event_type.value
                )
                return []
            ...
            for user_id in affected_users:
                ...
                for template in templates:
                    ...
                    if not await self._evaluate_template_conditions(
                        template, event_data, user_id
                    ):
                        continue
            ...
            self.logger.error(
                "Error handling workflow event %s: %s", event_type.value, str(e)
            )
            return []
    ...
    async def add_notification_template(self, template: NotificationTemplate) -> None:
        ...
        self.logger.info(
            "Added notification template: %s for event: %s",
            template.template_id,
            template.event_type.value,
        )

    async def subscribe_user(
        self, user_id: str, event_types: List[WorkflowEventType]
    ) -> None:
        ...
        self.logger.debug(
            "Subscribed user %s to %d event types", user_id, len(event_types)
        )
    ...
    async def unsubscribe_user(
        ...
        self.logger.debug(
            "Unsubscribed user %s from %d event types", user_id, len(event_types)
        )

    async def set_user_channel_preferences(
        self, user_id: str, preferences: Dict[WorkflowEventType, List[ChannelType]]
    ) -> None:
    ...
    async def set_user_quiet_hours(
        self, user_id: str, start_hour: int, end_hour: int, timezone: str = "UTC"
    ) -> None:
        ...
        self.logger.debug(
            "Set quiet hours for user %s: %d:00 - %d:00", user_id, start_hour, end_hour
        )

    def add_event_filter(
        self, filter_func: Callable[[WorkflowEventType, Dict[str, Any]], bool]
    ) -> None:
    ...
    async def _load_default_templates(self) -> None:
        ...
        self.logger.info(
            "Loaded %d default notification templates", len(default_templates)
        )

    async def _should_process_event(
        self, event_type: WorkflowEventType, event_data: Dict[str, Any]
    ) -> bool:
    ...
    async def _determine_affected_users(
        self, event_type: WorkflowEventType, event_data: Dict[str, Any]
    ) -> List[str]:
        ...
        if event_type in [
            ...
            WorkflowEventType.DOCUMENT_REVIEWED,
        ]:
    ...
    async def _is_user_subscribed(
        self, user_id: str, event_type: WorkflowEventType
    ) -> bool:
    ...
    async def _evaluate_template_conditions(
        self, template: NotificationTemplate, event_data: Dict[str, Any], user_id: str
    ) -> bool:
    ...
    async def _create_notification_from_template(
        ...
        try:
            ...
            preferred_channels = user_preferences.get(
                event_type, template.default_channels
            )
            ...
            if priority_override:
                ...
                importance_contexts = await self._determine_importance_contexts(
                    event_type, event_data
                )
            ...
            for channel in preferred_channels:
                ...
                if self.batch_processing:
                    ...
                    if self.notification_delivery:
                        notification_id = (
                            await self.notification_delivery.send_notification(message)
                        )
                        notification_ids.append(notification_id)

                    # Track analytics
                    await self._track_notification_sent(
                        notification_id, user_id, channel, event_type
                    )
            ...
            self.logger.error(
                "Error creating notification from template %s: %s",
                template.template_id,
                str(e),
            )
            return None
    ...
    async def _determine_importance_contexts(
        ...
        context_mapping = {
            ...
            WorkflowEventType.COLLABORATION_REQUEST: [
                ImportanceContext.COLLABORATION_URGENT
            ],
    ...
    async def _execute_event_handlers(
        ...
        for handler in handlers:
            try:
                ...
                self.logger.warning(
                    "Event handler error for %s: %s", event_type.value, str(e)
                )
    ...
    async def _process_batch(self) -> None:
        ...
        for channel, items in channel_groups.items():
            ...
            try:
                if self.notification_delivery:
                    notification_ids = await self.notification_delivery.send_batch(
                        messages
                    )
                else:
                    notification_ids = []
                ...
                self.logger.error(
                    "Batch processing error for channel %s: %s", channel.value, str(e)
                )
...
async def create_workflow_notification_integration(
    ...
    integration = WorkflowNotificationIntegration(
        notification_delivery=delivery, **kwargs
    )
...
async def example_workflow_integration():
    ...
    integration = await create_workflow_notification_integration(
        notification_channels=channels, batch_processing=True
    )

    # Subscribe users to events
    await integration.subscribe_user(
        "user123",
        [
            WorkflowEventType.TASK_ASSIGNED,
            WorkflowEventType.DEADLINE_APPROACHING,
    ...
    await integration.set_user_channel_preferences(
        "user123",
        {
