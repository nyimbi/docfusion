...
class UserPreferenceProfile:
	...
	preferred_channels_by_priority: Dict[PriorityLevel, List[str]] = field(default_factory=dict)
	...
	context_importance_weights: Dict[ImportanceContext, float] = field(default_factory=dict)
...
class NotificationMetadata(BaseModel):
	...
	# Content analysis
	title_keywords: List[str] = Field(default_factory=list, description="Extracted title keywords")
	content_keywords: List[str] = Field(default_factory=list, description="Extracted content keywords")
	sentiment_score: confloat(ge=-1.0, le=1.0) = Field(0.0, description="Content sentiment (-1 to 1)")
	urgency_keywords: List[str] = Field(default_factory=list, description="Urgency indicator words")
	...
	stakeholder_priority: Optional[str] = Field(None, description="Stakeholder priority level")
	
	# Business impact
	revenue_impact_estimate: Optional[float] = Field(None, description="Estimated revenue impact")
	compliance_risk_level: confloat(ge=0.0, le=1.0) = Field(0.0, description="Compliance risk (0-1)")
	blocking_dependencies: List[str] = Field(default_factory=list, description="IDs of blocked items")
	
	# Temporal context
	time_sensitivity_hours: Optional[int] = Field(None, description="Time sensitivity window")
	escalation_threshold_minutes: Optional[int] = Field(None, description="Auto-escalation time")
	optimal_delivery_window: Optional[Tuple[int, int]] = Field(None, description="Best delivery hours")
	
	# Collaboration context
	requires_immediate_response: bool = Field(False, description="Needs immediate response")
	involves_external_stakeholders: bool = Field(False, description="External parties involved")
	...
	custom_priority_override: Optional[PriorityLevel] = Field(None, description="Manual priority override")
...
class PriorityScore(BaseModel):
	...
	calculated_score: confloat(ge=0.0, le=1.0) = Field(..., description="ML-calculated priority score")
	final_priority: PriorityLevel = Field(..., description="Final priority after adjustments")
	
	# Score components
	content_score: confloat(ge=0.0, le=1.0) = Field(0.0, description="Content-based importance")
	context_score: confloat(ge=0.0, le=1.0) = Field(0.0, description="Context-based importance")
	temporal_score: confloat(ge=0.0, le=1.0) = Field(0.0, description="Time-sensitive importance")
	user_preference_score: confloat(ge=0.0, le=1.0) = Field(0.0, description="User preference alignment")
	business_impact_score: confloat(ge=0.0, le=1.0) = Field(0.0, description="Business impact weight")
	
	# Adjustments
	time_decay_factor: confloat(ge=0.0, le=1.0) = Field(1.0, description="Time-based priority decay")
	user_activity_multiplier: confloat(ge=0.1, le=3.0) = Field(1.0, description="User activity adjustment")
	load_balancing_adjustment: confloat(ge=0.1, le=2.0) = Field(1.0, description="System load adjustment")
	
	# Metadata
	calculation_timestamp: datetime = Field(default_factory=datetime.now)
	confidence_level: confloat(ge=0.0, le=1.0) = Field(0.5, description="Calculation confidence")
	reasoning: List[str] = Field(default_factory=list, description="Priority reasoning factors")
...
class PriorityManager:
	...
	def __init__(
		...
		self._urgency_keywords = {
			'urgent': 0.9, 'critical': 0.95, 'immediate': 0.85, 'asap': 0.8,
			'emergency': 0.95, 'failure': 0.8, 'error': 0.7, 'deadline': 0.75,
			'escalated': 0.7, 'blocked': 0.8, 'stuck': 0.6, 'help': 0.5,
			'important': 0.6, 'priority': 0.65, 'fast': 0.55, 'quick': 0.5
		}
		...
		self.logger.info("PriorityManager initialized with learning=%s", learning_enabled)
	...
	async def start(self) -> None:
		...
		self.logger.info("Priority management service started with %d background tasks", 
	...
	async def calculate_priority(
		...
		user_preference_score = await self._calculate_user_preference_score(metadata, user_profile)
		...
		activity_multiplier = await self._calculate_user_activity_multiplier(metadata.user_id)
		...
		final_priority = metadata.custom_priority_override or self._score_to_priority(final_score)
		
		# Build reasoning
		reasoning = self._build_priority_reasoning(
			metadata, content_score, context_score, temporal_score,
			user_preference_score, business_impact_score, final_score
		)
	...
	async def batch_calculate_priorities(
		self, 
		metadatas: List[NotificationMetadata]
	) -> List[PriorityScore]:
	...
	async def get_user_priority_preferences(self, user_id: str) -> UserPreferenceProfile:
	...
	def get_priority_statistics(self) -> Dict[str, Any]:
		...
		return {
			...
			'cache_hit_rate': (
				...
				max(1, self._priority_stats['cache_hits'] + self._priority_stats['cache_misses'])
			),
	...
	async def _calculate_temporal_score(self, metadata: NotificationMetadata) -> float:
		...
		if metadata.escalation_threshold_minutes:
			escalation_score = max(0.1, 1.0 - (metadata.escalation_threshold_minutes / 240))
	...
	async def _calculate_business_impact_score(self, metadata: NotificationMetadata) -> float:
		...
		if metadata.revenue_impact_estimate:
			...
			revenue_score = min(math.log10(max(1, abs(metadata.revenue_impact_estimate))) / 6, 1.0)
	...
	def _score_to_priority(self, score: float) -> PriorityLevel:
		...
		for priority, threshold in sorted(self._priority_thresholds.items(), 
	...
	def _build_priority_reasoning(
		...
		if metadata.urgency_keywords:
			reasoning.append(f"Explicit urgency indicators: {', '.join(metadata.urgency_keywords)}")
	...
	def _cache_priority(self, priority_score: PriorityScore) -> None:
		...
		self._cache_expiry[priority_score.notification_id] = (
			datetime.now() + timedelta(seconds=self.priority_refresh_interval)
		)
			...
			try:
				...
				try:
					update_data = await asyncio.wait_for(
						self._profile_update_queue.get(), 
						timeout=1.0
					)
	...
	async def _process_profile_update(self, update_data: Dict[str, Any]) -> None:
		...
		min_interactions = 10
		confidence = min(profile.total_notifications_received / (min_interactions * 3), 1.0)
...
def extract_urgency_keywords(text: str) -> List[str]:
	...
	urgency_terms = {
		...
		'failure', 'error', 'deadline', 'escalated', 'blocked',
		'stuck', 'help', 'important', 'priority', 'fast', 'quick'
	}
