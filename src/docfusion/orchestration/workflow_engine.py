...
class WorkflowEngine:
    ...
    def __init__(self):
        ...
        self.condition_evaluators = {
            ...
            "regex": lambda x, pattern: __import__("re").match(pattern, str(x))
            is not None,
            "success": lambda result: result.get("success", False)
            if isinstance(result, dict)
            else bool(result),
            "error": lambda result: result.get("error") is not None
            if isinstance(result, dict)
            else not bool(result),
        }

    async def execute_workflow(
        self, workflow: Workflow, initial_context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Workflow executed: {workflow.name} ({final_status.value})"
            )
        ...
        for level, nodes in hierarchy.items():
            ...
            if level_tasks:
                level_results = await asyncio.gather(
                    *level_tasks, return_exceptions=True
                )
    ...
    async def _execute_conditional(
        ...
        current_nodes = (
            workflow.entry_nodes.copy()
            if workflow.entry_nodes
            else list(workflow.nodes.keys())[:1]
        )
        visited = set()
    ...
    async def _execute_fork_node(
        ...
        return {
            ...
            "branches_created": len(
                [e for e in context.get("edges", []) if e.source_node == node.node_id]
            ),
        }
    ...
    async def _execute_join_node(
        ...
        return {
            ...
            "branches_joined": len(
                [e for e in context.get("edges", []) if e.target_node == node.node_id]
            ),
        }
    ...
    async def _execute_loop_node(
        ...
        return {"loop_id": node.node_id, "iterations": loop_count, "results": results}
    ...
    async def _get_execution_order(self, workflow: Workflow) -> List[str]:
        ...
        if workflow.entry_nodes:
            return workflow.entry_nodes + [
                n for n in workflow.nodes.keys() if n not in workflow.entry_nodes
            ]
        else:
        ...
        while levels.get(current_level):
            ...
            for node_id in levels[current_level]:
                ...
                children = [
                    ...
                    and edge.target_node not in node_levels
                ]
    ...
    async def _evaluate_edge_condition(
        self, edge: WorkflowEdge, context: Dict[str, Any], results: Dict[str, Any]
    ) -> bool:
    ...
    async def _check_dependencies(
        ...
        # Find incoming edges
        dependencies = [
            edge.source_node for edge in workflow.edges if edge.target_node == node_id
        ]
    ...
    def _determine_final_status(
        ...
        statuses = [
            result.get("status", ExecutionStatus.FAILED) for result in results.values()
        ]
