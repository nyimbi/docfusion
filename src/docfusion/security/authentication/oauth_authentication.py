...
class OAuthAuthentication:
	...
	async def revoke_token(self, token_id: str) -> bool:
		...
		# Provider-side revocation is not implemented; local revocation below
		# is the source of truth. Provider tokens remain valid until natural expiry.
		self.logger.debug(
			f"Provider-side revocation skipped for {token.provider.value}; "
			f"local token deleted"
		)
