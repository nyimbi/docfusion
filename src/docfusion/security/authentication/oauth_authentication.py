...
class OAuthAuthentication:
	...
	def __init__(self, config: Optional[OAuthConfiguration] = None):
		...
		self.logger.info(f"OAuth authentication initialized with {len(self.config.providers)} providers")
	...
	async def handle_authorization_callback(
		...
		try:
			...
			user_info = await self._get_user_info(provider, token_result.token.access_token)
			if not user_info:
				return OAuthResult(
					success=False,
				...
				async with session.post(
					...
					if response.status == 200 and 'access_token' in result:
						...
						token = OAuthToken(
							...
							token_type=OAuthTokenType(result.get('token_type', 'Bearer')),
						...
						return OAuthResult(
							...
							error_description=result.get('error_description', 'Failed to exchange code for token')
						)
	...
	async def _get_user_info(self, provider: OAuthProvider, access_token: str) -> Optional[OAuthUserInfo]:
	...
	def _parse_user_info(self, provider: OAuthProvider, user_data: Dict[str, Any]) -> OAuthUserInfo:
	...
	def _check_auth_rate_limit(self, identifier: str) -> bool:
		...
		if identifier in self.auth_attempts:
			self.auth_attempts[identifier] = [
				attempt for attempt in self.auth_attempts[identifier]
				if attempt > hour_ago
			]
		else:
			self.auth_attempts[identifier] = []
	...
	def _check_token_rate_limit(self, identifier: str) -> bool:
		...
		if identifier in self.token_requests:
			self.token_requests[identifier] = [
				request for request in self.token_requests[identifier]
				if request > hour_ago
			]
		else:
			self.token_requests[identifier] = []
		...
		# Check limit
		if len(self.token_requests[identifier]) >= self.config.token_rate_limit_per_hour:
			return False
...
# Factory functions
def create_oauth_authentication(config: Optional[OAuthConfiguration] = None) -> OAuthAuthentication:
...
def create_oauth_config_from_env() -> OAuthConfiguration:
	...
	if microsoft_client_id and microsoft_client_secret:
		...
		config.providers[OAuthProvider.MICROSOFT].client_secret = microsoft_client_secret
