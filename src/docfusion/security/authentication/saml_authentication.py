...
class SAMLIdentityProviderConfig:
	...
	supported_bindings: List[SAMLBinding] = field(default_factory=lambda: [SAMLBinding.HTTP_REDIRECT, SAMLBinding.HTTP_POST])
...
class SAMLConfiguration:
	...
	identity_providers: Dict[str, SAMLIdentityProviderConfig] = field(default_factory=dict)
...
class SAMLResult(BaseModel):
	...
	authentication_time: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
...
class SAMLAuthentication:
	...
	def __init__(self, config: SAMLConfiguration):
		...
		if not HAS_XMLSEC:
			self.logger.warning("xmlsec library not available - signature validation disabled")
		...
		self.logger.info(f"SAML authentication initialized with {len(self.config.identity_providers)} IdPs")
...
<md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata"
	...
	def create_authn_request(
		...
		request = SAMLRequest(
			...
			expires_at=datetime.now(timezone.utc) + timedelta(minutes=self.config.request_id_expiry_minutes),
	...
	async def handle_sso_response(
		...
		try:
			...
			return SAMLResult(
				...
				logout_url=self._get_logout_url(response.issuer, response.assertion.session_index)
			)
...
<samlp:LogoutRequest xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol"
	...
	def _build_authn_request_xml(self, request: SAMLRequest, idp_config: SAMLIdentityProviderConfig) -> str:
...
<samlp:AuthnRequest xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol"
	...
	def _parse_saml_response(self, response_xml: str) -> Optional[SAMLResponse]:
		...
		try:
			...
			status_message_elem = root.find('.//{urn:oasis:names:tc:SAML:2.0:protocol}StatusMessage')
			...
			return SAMLResponse(
				...
				assertion=assertion,
				issued_at=datetime.fromisoformat(issue_instant.replace('Z', '+00:00')) if issue_instant else datetime.now(timezone.utc)
			)
	...
	def _parse_assertion(self, assertion_elem: ET.Element) -> SAMLAssertion:
		...
		name_id_elem = subject_elem.find('.//{urn:oasis:names:tc:SAML:2.0:assertion}NameID') if subject_elem is not None else None
		subject = name_id_elem.text if name_id_elem is not None else ''
		subject_format = SAMLNameIDFormat(name_id_elem.get('Format', SAMLNameIDFormat.UNSPECIFIED)) if name_id_elem is not None else SAMLNameIDFormat.UNSPECIFIED
		...
		if conditions_elem is not None:
			...
			# Extract audience
			audience_elem = conditions_elem.find('.//{urn:oasis:names:tc:SAML:2.0:assertion}Audience')
			audience = audience_elem.text if audience_elem is not None else ''
		
		# Extract authentication statement
		authn_stmt_elem = assertion_elem.find('.//{urn:oasis:names:tc:SAML:2.0:assertion}AuthnStatement')
		...
		if authn_stmt_elem is not None:
			...
			authn_context_elem = authn_stmt_elem.find('.//{urn:oasis:names:tc:SAML:2.0:assertion}AuthnContextClassRef')
		...
		# Extract attributes
		attributes = {}
		attr_stmt_elem = assertion_elem.find('.//{urn:oasis:names:tc:SAML:2.0:assertion}AttributeStatement')
		if attr_stmt_elem is not None:
			for attr_elem in attr_stmt_elem.findall('.//{urn:oasis:names:tc:SAML:2.0:assertion}Attribute'):
	...
	async def _validate_response(self, response: SAMLResponse, response_xml: str) -> Dict[str, Any]:
		...
		if now < response.assertion.not_before - timedelta(seconds=self.config.clock_skew_seconds):
			return {
				'valid': False,
				'error': "Assertion not yet valid"
			}
		
		if now > response.assertion.not_on_or_after + timedelta(seconds=self.config.clock_skew_seconds):
			return {
				'valid': False,
	...
	def _extract_user_info(self, assertion: SAMLAssertion, idp_entity_id: str) -> SAMLUserInfo:
		...
		if not user_info.email and assertion.subject_name_id_format == SAMLNameIDFormat.EMAIL:
			user_info.email = assertion.subject
		
		return user_info
	
	def _get_logout_url(self, idp_entity_id: str, session_index: Optional[str]) -> Optional[str]:
...
def create_test_saml_config() -> SAMLConfiguration:
	...
	config = SAMLConfiguration(
		service_provider=sp_config,
		identity_providers={"test-idp": test_idp}
	)
