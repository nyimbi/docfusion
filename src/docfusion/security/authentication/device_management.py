...
class DeviceManager:
    ...
    async def register_device(
        ...
        try:
            ...
            if user_device_count >= policy.max_devices_per_user:
                raise ValueError(
                    f"User has reached maximum device limit ({policy.max_devices_per_user})"
                )
            ...
            if existing_device_id and existing_device_id in self.trusted_devices:
                ...
                if existing_device.user_id != user_id:
                    ...
                    raise ValueError(
                        "Device appears to be already registered to different user"
                    )
                ...
                if (
                    ...
                    and webauthn_credential_id
                    not in existing_device.webauthn_credential_ids
                ):
                    existing_device.webauthn_credential_ids.append(
                        webauthn_credential_id
                    )

                self.logger.info(
                    f"Updated existing device {existing_device_id} for user {user_id}"
                )
            ...
            device = TrustedDevice(
                ...
                webauthn_credential_ids=[webauthn_credential_id]
            ...
            self.logger.info(
                f"Registered new device {device.device_id} for user {user_id}"
            )
    ...
    async def trust_device(
        ...
        try:
            ...
            if trust_duration_days:
                device.trust_expires_at = datetime.now(timezone.utc) + timedelta(
                    days=trust_duration_days
                )
            else:
                policy = self.device_policies.get("default")
                device.trust_expires_at = datetime.now(timezone.utc) + timedelta(
                    days=policy.trust_duration_days
                )
            ...
            self.logger.info(
                f"Established trust for device {device_id} at {trust_level} level"
            )
            return True
    ...
    async def verify_device(
        ...
        try:
            ...
            if (
                ...
                and datetime.now(timezone.utc) > device.trust_expires_at
            ):
            ...
            if webauthn_credential_id:
                webauthn_verified = (
                    webauthn_credential_id in device.webauthn_credential_ids
                )
            ...
            verification_result = {
                ...
                "requires_additional_auth": not (
                    device.status == DeviceStatus.TRUSTED
                    and device.trust_level in [TrustLevel.HIGH, TrustLevel.MEDIUM]
                ),
    ...
    async def _perform_security_checks(
        self, device: TrustedDevice, current_fingerprint: DeviceFingerprint
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if device.last_seen_ip != current_fingerprint.ip_address:
                ...
                if self.config.enable_geolocation_tracking:
                    distance = await self._calculate_location_distance(
                        device.last_seen_location, current_fingerprint.ip_geolocation
                    )

                    if (
                        distance
                        and distance > self.config.suspicious_location_threshold_km
                    ):
                        warnings.append(
                            f"Suspicious location change: {distance:.1f} km"
                        )
                        # Don't fail verification, but flag for attention

            # Check fingerprint consistency
            fingerprint_similarity = self._calculate_fingerprint_similarity(
                device.device_fingerprint, current_fingerprint
            )

            if fingerprint_similarity < self.config.fingerprint_match_threshold:
                warnings.append(
                    f"Device fingerprint mismatch (similarity: {fingerprint_similarity:.2f})"
                )
                passed = False
            ...
            return {"passed": False, "warnings": [f"Security check error: {str(e)}"]}
    ...
    async def _assess_device_risk(self, device: TrustedDevice):
        ...
        try:
            ...
            if (
                device.risk_level == DeviceRiskLevel.CRITICAL
                and self.config.auto_block_critical_risk
            ):
            ...
            self.logger.error(
                f"Risk assessment failed for device {device.device_id}: {e}"
            )
    ...
    def _calculate_fingerprint_similarity(
        self, fingerprint1: DeviceFingerprint, fingerprint2: DeviceFingerprint
    ) -> float:
    ...
    async def _check_policy_compliance(self, device: TrustedDevice) -> bool:
        ...
        try:
            ...
            if policy.require_screen_lock and not device.security_features.get(
                "screen_lock", False
            ):
                violations.append("screen_lock_required")

            # Check encryption requirement
            if policy.require_encryption and not device.security_features.get(
                "encryption", False
            ):
                violations.append("encryption_required")

            # Check biometrics requirement
            if policy.require_biometrics and not device.security_features.get(
                "biometrics", False
            ):
            ...
            if (
                policy.blocked_device_types
                and device.device_type in policy.blocked_device_types
            ):
                violations.append("device_type_blocked")

            if (
                policy.allowed_device_types
                and device.device_type not in policy.allowed_device_types
            ):
    ...
    async def _determine_initial_trust(
        self, device: TrustedDevice, policy: DevicePolicy
    ):
        ...
        try:
            ...
            if (
                self.config.enable_auto_trust
                and fingerprint_confidence
                >= self.config.auto_trust_confidence_threshold
                and device.complies_with_policy
                and device.risk_level in [DeviceRiskLevel.VERY_LOW, DeviceRiskLevel.LOW]
            ):
                device.status = DeviceStatus.TRUSTED
                ...
                self.config.require_admin_approval_for_new_devices
                or policy.require_admin_approval
            ):
    ...
    async def _calculate_location_distance(
        self, location1: Optional[Dict[str, Any]], location2: Optional[Dict[str, Any]]
    ) -> Optional[float]:
    ...
    async def _log_security_event(
        ...
        try:
            ...
            if severity in ["high", "critical"] or requires_attention:
                self.logger.warning(
                    f"Device security event: {description} (Device: {device_id})"
                )
            else:
                self.logger.info(
                    f"Device security event: {description} (Device: {device_id})"
    ...
    async def cleanup_expired_devices(self) -> Dict[str, int]:
        ...
        cleanup_stats = {"expired_trust": 0, "inactive_devices": 0, "old_events": 0}

        # Update expired trust status
        for device in self.trusted_devices.values():
            if (
                device.trust_expires_at
                and now > device.trust_expires_at
                and device.status == DeviceStatus.TRUSTED
            ):
                device.status = DeviceStatus.EXPIRED
                device.trust_level = TrustLevel.UNTRUSTED
        ...
        cleanup_threshold = now - timedelta(
            days=self.config.cleanup_expired_devices_days
        )
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        for device in self.trusted_devices.values():
            ...
            trust_level_counts[device.trust_level] = (
                trust_level_counts.get(device.trust_level, 0) + 1
            )
            device_type_counts[device.device_type] = (
                device_type_counts.get(device.device_type, 0) + 1
            )
            risk_level_counts[device.risk_level] = (
                risk_level_counts.get(device.risk_level, 0) + 1
            )

        # Security events by severity
        event_severity_counts = {}
        for event in self.security_events.values():
            event_severity_counts[event.severity] = (
                event_severity_counts.get(event.severity, 0) + 1
            )
