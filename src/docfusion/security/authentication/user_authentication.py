...
class UserAuthentication:
    ...
    async def authenticate(self, credentials: UserCredentials) -> AuthenticationResult:
        ...
        try:
            ...
            device_trusted = self._is_device_trusted(
                user_profile, credentials.device_id
            )
            ...
            await self._create_session(
                session_id, user_profile.user_id, credentials, expires_at
            )
            ...
            self.logger.info(
                f"Successful authentication for user: {credentials.username}"
            )
    ...
    async def change_password(
        ...
        try:
            ...
            if not self._verify_password(
                current_password, user_profile.password_hash, user_profile.password_salt
            ):
            ...
            user_profile.password_expires_at = datetime.now(timezone.utc) + timedelta(
    ...
    async def enable_mfa(self, user_id: str) -> Dict[str, Any]:
        ...
        totp_uri = pyotp.TOTP(mfa_secret).provisioning_uri(
            name=user_id, issuer_name=self.config.mfa_issuer_name
        )
    ...
    def _validate_password_strength(self, password: str) -> Dict[str, Any]:
        ...
        if self.config.require_uppercase and not self.patterns["uppercase"].search(
            password
        ):
            errors.append("Password must contain at least one uppercase letter")

        if self.config.require_lowercase and not self.patterns["lowercase"].search(
            password
        ):
            errors.append("Password must contain at least one lowercase letter")

        if self.config.require_numbers and not self.patterns["numbers"].search(
            password
        ):
            errors.append("Password must contain at least one number")

        if self.config.require_special_chars and not self.patterns["special"].search(
            password
        ):
    ...
    def _check_rate_limit(self, ip_address: Optional[str]) -> bool:
        ...
        if ip_address in self.rate_limit_tracker:
            self.rate_limit_tracker[ip_address] = [
                timestamp
                for timestamp in self.rate_limit_tracker[ip_address]
    ...
    async def _handle_failed_login(self, user_profile: UserSecurityProfile):
        ...
        if user_profile.failed_login_attempts >= self.config.max_failed_attempts:
            ...
            user_profile.account_locked_until = (
                datetime.now(timezone.utc) + lockout_duration
            )
            self.logger.warning(
                f"Account locked for user: {user_profile.user_id} until {user_profile.account_locked_until}"
            )

    async def _handle_mfa(
        self, user_profile: UserSecurityProfile, mfa_code: Optional[str]
    ...
    def _is_device_trusted(
        self, user_profile: UserSecurityProfile, device_id: Optional[str]
    ) -> bool:
    ...
    def _generate_security_warnings(
        self, user_profile: UserSecurityProfile, credentials: UserCredentials
    ) -> List[str]:
        ...
        if user_profile.password_expires_at:
            days_until_expiry = (
                user_profile.password_expires_at - datetime.now(timezone.utc)
            ).days
            if days_until_expiry <= 7:
                warnings.append(f"Password expires in {days_until_expiry} days")

        # Check for new device
        if credentials.device_id and not self._is_device_trusted(
            user_profile, credentials.device_id
        ):
