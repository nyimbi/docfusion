...
class APIAuthentication:
	...
	async def create_api_key(
		...
		try:
			...
			return APIKeyResult(
				success=False,
				error_message=f"Failed to create API key: {e}"
			)
	...
	async def authenticate_api_key(
		...
		try:
			...
			if not key_id:
				await self._log_request(None, ip_address, user_agent, endpoint, method, False, "Invalid API key")
				return APIKeyResult(
					success=False,
			...
			if api_key_record.status != APIKeyStatus.ACTIVE:
				await self._log_request(key_id, ip_address, user_agent, endpoint, method, False, f"Key status: {api_key_record.status}")
				return APIKeyResult(
					success=False,
			...
			if api_key_record.expires_at and datetime.now(timezone.utc) > api_key_record.expires_at:
				...
				await self._log_request(key_id, ip_address, user_agent, endpoint, method, False, "Key expired")
				return APIKeyResult(
					success=False,
			...
			if api_key_record.allowed_ips and ip_address not in api_key_record.allowed_ips:
				await self._log_request(key_id, ip_address, user_agent, endpoint, method, False, "IP not allowed")
				return APIKeyResult(
					success=False,
					error_message="Request from unauthorized IP address"
				)
			
			# Check suspicious IP
			if self.config.block_suspicious_ips and ip_address in self.suspicious_ips:
				await self._log_request(key_id, ip_address, user_agent, endpoint, method, False, "Suspicious IP")
				return APIKeyResult(
					success=False,
			...
			if not rate_limit_result['allowed']:
				await self._log_request(key_id, ip_address, user_agent, endpoint, method, False, "Rate limited")
				return APIKeyResult(
					success=False,
			...
			await self._log_request(key_id, ip_address, user_agent, endpoint, method, True, "Success")
	...
	async def get_api_key_stats(self, key_id: str, user_id: str) -> Optional[Dict[str, Any]]:
		...
		if api_key_record.total_requests > 0:
			success_rate = api_key_record.successful_requests / api_key_record.total_requests
		
		# Get recent requests (last 24 hours)
		twenty_four_hours_ago = datetime.now(timezone.utc) - timedelta(hours=24)
		recent_requests = [
			log for log in self.request_logs
			if log.get('key_id') == key_id and log.get('timestamp', datetime.min.replace(tzinfo=timezone.utc)) > twenty_four_hours_ago
		]
	...
	async def _check_rate_limit(self, api_key_record: APIKey, ip_address: Optional[str]) -> Dict[str, Any]:
		...
		self.rate_limit_tracker[key_id] = [
			timestamp for timestamp in self.rate_limit_tracker[key_id]
		...
		if current_count >= max_requests:
			...
			reset_time = datetime.fromtimestamp(oldest_request + window_seconds, tz=timezone.utc)
	...
	async def _track_suspicious_activity(self, ip_address: str):
		...
		failed_attempts = [
			log for log in self.request_logs
			if (log.get('ip_address') == ip_address and 
				not log.get('success') and 
				log.get('timestamp', datetime.min.replace(tzinfo=timezone.utc)) > one_hour_ago)
		]
		
		# Flag as suspicious if too many failures
		if len(failed_attempts) > 50:  # Configurable threshold
			self.suspicious_ips.add(ip_address)
			self.logger.warning(f"Marked IP as suspicious: {ip_address} ({len(failed_attempts)} failures)")
	...
	async def cleanup_expired_keys(self) -> int:
		...
		for key_id, api_key_record in list(self.api_keys.items()):
			if (api_key_record.expires_at and now > api_key_record.expires_at and 
				api_key_record.status == APIKeyStatus.ACTIVE):
	...
	async def get_usage_analytics(
		...
		for log in relevant_logs:
			date_key = log.get('timestamp', datetime.min.replace(tzinfo=timezone.utc)).date().isoformat()
...
# Factory function
def create_api_authentication(config: Optional[APIAuthenticationConfig] = None) -> APIAuthentication:
