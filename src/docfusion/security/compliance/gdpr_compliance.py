...
class ConsentRecord(BaseModel):
    ...
    granular_consent: Dict[str, bool] = Field(
...
class GDPRCompliance:
    ...
    async def record_consent(
        ...
        if expires_in_days:
            ...
            expires_at = datetime.now(timezone.utc) + timedelta(
                days=self.config.consent_renewal_days
            )
    ...
    async def withdraw_consent(
        ...
        for consent in reversed(user_consents):
            if consent.purpose == purpose and consent.status == ConsentStatus.GIVEN:
                ...
                self.logger.info(
                    f"Withdrew consent for user {user_id}, purpose: {purpose}"
                )
                return True
    ...
    def check_consent(
        self, user_id: str, purpose: DataProcessingPurpose
    ) -> ConsentStatus:
        ...
        relevant_consents = [c for c in user_consents if c.purpose == purpose]
        ...
        # Check if expired
        if (
            latest_consent.expires_at
            and datetime.now(timezone.utc) > latest_consent.expires_at
        ):
    ...
    async def _handle_restrict_processing_request(
    ...
    async def _create_export_file(
        self, export_result: DataExportResult, format: str
    ) -> Optional[str]:
        """Create export file"""
        try:
        ...
        for source_name, source_config in self.data_sources.items():
            try:
                ...
                if deletion_result.get("retained"):
                    ...
                    retention_reasons.extend(
                        deletion_result.get("retention_reasons", [])
                    )
            ...
            if user_id in self.consent_records:
                ...
                if withdrawn_consents:
                    ...
                    retention_reasons.append(
                        "Legal obligation to retain consent withdrawal records"
                    )
    ...
    async def cleanup_expired_data(self) -> Dict[str, int]:
        ...
        expired_exports = []
        for export_id, export in self.data_exports.items():
            if export.access_link_expires_at and now > export.access_link_expires_at:
