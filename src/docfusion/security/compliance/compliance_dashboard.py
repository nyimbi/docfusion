...
class ComplianceDashboard:
    ...
    def __init__(self, config: Optional[DashboardConfiguration] = None):
        ...
        self.framework_assessments: Dict[
            ComplianceFramework, ComplianceFrameworkAssessment
        ] = {}
    ...
    async def add_compliance_metric(self, metric: ComplianceMetric) -> str:
        ...
        if len(self.metric_history[metric.metric_id]) > 100:
            self.metric_history[metric.metric_id] = self.metric_history[
                metric.metric_id
            ][-100:]
        ...
        self.logger.info(
            f"Added compliance metric: {metric.control_name} ({metric.framework})"
        )
        return metric.metric_id

    async def update_framework_assessment(
        self, framework: ComplianceFramework, force_recalculate: bool = False
    ) -> ComplianceFrameworkAssessment:
        """Update overall framework assessment"""
        # Get all metrics for this framework
        framework_metrics = [
            m for m in self.compliance_metrics.values() if m.framework == framework
        ]
        ...
        if not framework_metrics:
            ...
            assessment = ComplianceFrameworkAssessment(
                ...
                + timedelta(days=self.config.default_assessment_interval_days),
            )
        else:
            ...
            # Count by status
            compliant = len(
                [m for m in framework_metrics if m.status == ComplianceStatus.COMPLIANT]
            )
            non_compliant = len(
                [
                    m
            ...
            under_review = len(
                [
                    m
                    for m in framework_metrics
                    if m.status == ComplianceStatus.UNDER_REVIEW
                ]
            )
            partially_compliant = len(
                [
                    m
                    for m in framework_metrics
                    if m.status == ComplianceStatus.PARTIALLY_COMPLIANT
                ]
            )
            ...
            # Count risk levels
            critical_issues = len(
                [m for m in framework_metrics if m.risk_level == RiskLevel.CRITICAL]
            )
            high_risk_issues = len(
                [m for m in framework_metrics if m.risk_level == RiskLevel.HIGH]
            )
            medium_risk_issues = len(
                [m for m in framework_metrics if m.risk_level == RiskLevel.MEDIUM]
            )
            low_risk_issues = len(
                [m for m in framework_metrics if m.risk_level == RiskLevel.LOW]
            )
        ...
        self.logger.info(
            f"Updated {framework} assessment: {overall_score:.1f}% ({overall_status})"
        )
        return assessment
    ...
    async def _check_metric_alerts(self, metric: ComplianceMetric):
        ...
        if metric.status == ComplianceStatus.NON_COMPLIANT and metric.risk_level in [
            RiskLevel.CRITICAL,
            RiskLevel.HIGH,
        ]:
        ...
        # Score degradation
        if (
            metric.metric_id in self.metric_history
            and len(self.metric_history[metric.metric_id]) > 1
        ):
        ...
        for alert_data in alerts_to_create:
            alert = ComplianceAlert(
                ...
                due_date=metric.next_assessment_due
                if alert_data["type"] == "assessment_overdue"
                else None,
            )
    ...
    async def _check_framework_alerts(self, assessment: ComplianceFrameworkAssessment):
        ...
        if (
            assessment.trend_direction == "declining"
            and assessment.score_change
            and assessment.score_change < -10
        ):
            alerts_to_create.append(
                {
        ...
        if assessment.certification_expiry:
            days_to_expiry = (
                assessment.certification_expiry - datetime.now(timezone.utc)
            ).days
    ...
    async def record_security_incident(self, incident: SecurityIncident) -> str:
        ...
        if (
            ...
            and not incident.regulatory_notification_sent
        ):
            alert = ComplianceAlert(
    ...
    async def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        compliant_frameworks = len(
            [
                a
                for a in self.framework_assessments.values()
        ...
        critical_alerts = len(
            [
                a
                for a in self.compliance_alerts.values()
        ...
        total_open_alerts = len(
            [a for a in self.compliance_alerts.values() if a.status == "open"]
        )
        ...
        for assessment in sorted(
            self.framework_assessments.values(),
            ...
            "incidents": {
                ...
                "notification_required": len(
                    [
                        ...
                        if i.regulatory_notification_required
                        and not i.regulatory_notification_sent
                    ]
                ),
            },
    ...
    async def get_alert_dashboard(self) -> Dict[str, Any]:
        ...
        sorted_alerts = sorted(
            self.compliance_alerts.values(), key=lambda a: a.created_at, reverse=True
        )
    ...
    async def generate_compliance_report(
        ...
        for fw in frameworks_to_include:
            ...
            # Get metrics for this framework
            framework_metrics = [
                m for m in self.compliance_metrics.values() if m.framework == fw
            ]
            ...
            period_alerts = [
                a
                for a in self.compliance_alerts.values()
                if (a.framework == fw and start_date <= a.created_at <= end_date)
            ]
            ...
            report["frameworks"][fw.value] = {
                ...
                "risks": {
                    ...
                    "medium": len(
                        [
                            m
                            for m in framework_metrics
                            if m.risk_level == RiskLevel.MEDIUM
                        ]
                    ),
                    "low": len(
                        [m for m in framework_metrics if m.risk_level == RiskLevel.LOW]
                ...
                "activity_summary": {
                    ...
                    "critical_alerts": len(
                        [a for a in period_alerts if a.severity == RiskLevel.CRITICAL]
                    ),
