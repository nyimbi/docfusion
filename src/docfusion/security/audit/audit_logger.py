...
class AuditLogger:
	...
	async def log_event(
		...
		self.events_by_severity[severity.value] = self.events_by_severity.get(severity.value, 0) + 1
	...
	async def query_events(
		...
		for event in self.event_buffer:
			if self._event_matches_filters(
				event, start_time, end_time, event_types, user_id,
				resource_type, resource_id, severity, tags
			):
	...
	async def export_audit_log(
		...
		if include_integrity and self.config.enable_integrity_protection:
			export_data['integrity_chain'] = [
				record.model_dump() for record in self.integrity_chain
	...
	async def verify_log_integrity(self, event_id: Optional[str] = None) -> Dict[str, Any]:
	...
	async def get_audit_statistics(self) -> Dict[str, Any]:
		...
		return {
			...
			'events_by_type': dict(sorted(self.events_by_type.items(), key=lambda x: x[1], reverse=True)),
	...
	async def cleanup_old_logs(self, older_than_days: int = None) -> int:
		...
		self.integrity_chain = [
			record for record in self.integrity_chain
	...
	def _should_log_event(self, event_type: AuditEventType, severity: AuditSeverity) -> bool:
		...
		return severity_levels[severity] >= severity_levels[self.config.minimum_severity]
	...
	def _calculate_event_checksum(self, event: AuditEvent) -> str:
		...
		event_data = {
			...
			'details': json.dumps(event.details, sort_keys=True) if event.details else None
		}
		...
		return hmac.new(key_bytes, event_string.encode('utf-8'), hashlib.sha256).hexdigest()
	...
	def _create_integrity_record(self, event: AuditEvent) -> LogIntegrityRecord:
		...
		signature = hmac.new(key_bytes, signature_data.encode('utf-8'), hashlib.sha256).hexdigest()
	...
	def _verify_integrity_chain(self) -> bool:
		...
		for record in self.integrity_chain:
			# Verify signature
			signature_data = f"{record.event_id}:{record.current_hash}:{record.previous_hash or ''}"
			...
			expected_signature = hmac.new(key_bytes, signature_data.encode('utf-8'), hashlib.sha256).hexdigest()
	...
	async def _flush_events(self):
		...
		try:
			...
			self.logger.debug(f"Flushed {min(self.config.batch_size, len(self.event_buffer))} audit events")
	...
	async def _handle_real_time_alert(self, event: AuditEvent):
		...
		if not self.config.alert_on_critical_events or event.severity != AuditSeverity.CRITICAL:
			return
		
		try:
			# In production, this would send alerts via webhook, email, etc.
			alert_message = f"CRITICAL AUDIT EVENT: {event.event_type.value} - {event.description}"
...
async def log_authentication_event(
	...
	description = f"Authentication {'succeeded' if success else 'failed'} for user {user_id}"
