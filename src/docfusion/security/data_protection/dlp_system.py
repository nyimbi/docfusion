...
class DLPSystem:
	...
	def __init__(self, config: Optional[DLPConfiguration] = None):
		...
		# Scanning queue
		self.scan_queue: asyncio.Queue = asyncio.Queue(maxsize=self.config.scan_queue_size)
		...
		try:
			...
			for pattern in patterns:
				...
				if matches:
					...
					confidence_scores.extend([m.get('confidence', pattern.base_confidence) for m in matches])
					data_types.update(pattern.data_types)
			
			# Calculate overall results
			scan_result.patterns_found = patterns_found
			scan_result.matches = all_matches
			scan_result.overall_confidence = max(confidence_scores) if confidence_scores else 0.0
			scan_result.data_types_detected = list(data_types)
			
			# Determine sensitivity level
			if patterns_found:
				sensitivity_levels = [
					self.data_patterns[pid].sensitivity_level
					for pid in patterns_found
				]
				scan_result.max_sensitivity = max(sensitivity_levels, key=lambda x: self._sensitivity_to_numeric(x))
			...
			scan_result.scanning_engines_used = [ScanningEngine.REGEX, ScanningEngine.KEYWORD]
			...
			self.logger.info(f"Content scan completed: {len(patterns_found)} patterns found, confidence: {scan_result.overall_confidence:.2f}")
			return scan_result
	...
	async def _scan_with_pattern(self, content: str, pattern: DataPattern) -> List[Dict[str, Any]]:
		...
		try:
			...
			if pattern.regex_patterns:
				for regex_pattern in pattern.regex_patterns:
					...
					for match in regex_matches:
						...
						match_info['confidence'] = self._calculate_match_confidence(match_info, pattern)
				...
				for keyword in pattern.keywords:
					...
					for match in keyword_matches:
						match_info = {
							...
							'start_pos': match.start(),
							'end_pos': match.end(),
							'confidence': pattern.base_confidence * 0.8,  # Lower confidence for keywords
							'context': self._extract_context(content, match.start(), match.end())
						}
			...
			if pattern.require_context and pattern.context_patterns:
				matches = [m for m in matches if self._validate_context(m['context'], pattern.context_patterns)]
			
			return matches
		
		except Exception as e:
			self.logger.error(f"Pattern scanning failed for pattern {pattern.name}: {e}")
			return []
	...
	def _extract_context(self, content: str, start_pos: int, end_pos: int, context_chars: int = 100) -> str:
		"""Extract context around a match"""
		context_start = max(0, start_pos - context_chars)
		context_end = min(len(content), end_pos + context_chars)
		return content[context_start:context_end]
	
	def _calculate_match_confidence(self, match_info: Dict[str, Any], pattern: DataPattern) -> float:
	...
	async def check_dlp_policies(
		...
		try:
			...
			applicable_rules = self._get_applicable_rules(user_id, resource_id, scan_result.content_type)
			
			for rule in applicable_rules:
				violation = await self._evaluate_rule(rule, scan_result, user_id, resource_id, action)
				if violation:
			...
			for violation in violations:
				...
				if self.config.auto_create_incidents and violation.severity in ["high", "critical"]:
		...
		for rule in self.dlp_rules.values():
			...
			if resource_id:
				if rule.excluded_resources and resource_id in rule.excluded_resources:
					continue
				if rule.applies_to_resources and resource_id not in rule.applies_to_resources:
					continue
		...
		try:
			...
			violation = DLPViolation(
				...
				evidence={
					...
					'sensitivity_level': scan_result.max_sensitivity.value if scan_result.max_sensitivity else None,
	...
	def _calculate_violation_severity(self, rule: DLPRule, scan_result: ScanResult) -> str:
	...
	async def _create_incident_for_violation(self, violation: DLPViolation):
		...
		try:
			incident = DLPIncident(
				...
				affected_resources=[violation.resource_id] if violation.resource_id else [],
			...
			self.logger.warning(f"Created DLP incident {incident.incident_id} for violation {violation.violation_id}")
		
		except Exception as e:
			self.logger.error(f"Failed to create incident for violation {violation.violation_id}: {e}")
	...
	async def _process_background_scan(self, scan_task: Dict[str, Any]):
		...
		try:
			...
			if scan_result.patterns_found:
				...
				if violations:
					self.logger.info(f"Background scan found {len(violations)} violations")
		...
		try:
			...
			return {
				...
				'scan_result': {
					...
					'sensitivity': scan_result.max_sensitivity.value if scan_result.max_sensitivity else None,
					'data_types': scan_result.data_types_detected
				},
				'violations': [
					{
	...
	async def cleanup_old_data(self) -> Dict[str, int]:
		...
		old_scans = [
			scan_id for scan_id, scan in self.scan_results.items()
		...
		old_cache_entries = [
			cache_key for cache_key, result in self.scan_cache.items()
	...
	def get_statistics(self) -> Dict[str, Any]:
		...
		return {
			...
			'active_patterns': len([p for p in self.data_patterns.values() if p.active]),
