...
class DataEncryption:
	...
	async def create_encryption_key(
		...
		if self.config.auto_rotate_keys and expires_at:
			encryption_key.rotation_scheduled_at = expires_at - timedelta(days=7)  # Rotate 1 week before expiry
		
		self.encryption_keys[encryption_key.key_id] = encryption_key
		
		self.logger.info(f"Created encryption key: {encryption_key.key_id} for {purpose}")
		...
		async with self.semaphore:
			try:
				...
				if not encryption_key.is_active:
					return EncryptionResult(
						success=False,
						error_message="Encryption key is not active"
					)
				...
				if encryption_key.expires_at and datetime.now(timezone.utc) > encryption_key.expires_at:
					return EncryptionResult(
						success=False,
						error_message="Encryption key has expired"
					)
				...
				if not key_material:
					return EncryptionResult(
						success=False,
						error_message="Failed to retrieve key material"
					)
				...
				if encryption_key.algorithm == EncryptionAlgorithm.AES_256_GCM:
					...
					encrypted_data, nonce = await self._encrypt_aes_cbc(data_bytes, key_material)
					tag = None
				else:
					return EncryptionResult(
						success=False,
			...
			try:
				...
				if encryption_key.algorithm == EncryptionAlgorithm.AES_256_GCM:
					decrypted_data = await self._decrypt_aes_gcm(
						data_bytes, key_material, nonce_bytes, tag_bytes, additional_data
					)
				elif encryption_key.algorithm == EncryptionAlgorithm.AES_256_CBC:
					decrypted_data = await self._decrypt_aes_cbc(data_bytes, key_material, nonce_bytes)
				else:
					return DecryptionResult(
						success=False,
	...
	async def decrypt_field(
		...
		return await self.decrypt_data(encrypted_data, key_id, nonce, tag, additional_data)
	...
	async def rotate_key(self, key_id: str, rotated_by: str) -> str:
		...
		new_key_id = await self.create_encryption_key(
			...
			expires_in_days=self.config.key_rotation_days if self.config.auto_rotate_keys else None
		)
	...
	async def _get_key_material(self, key_id: str) -> Optional[bytes]:
		...
		try:
			key_material = await self._decrypt_key_material(encryption_key.encrypted_key)
...
# Factory function
def create_data_encryption(config: Optional[EncryptionConfiguration] = None) -> DataEncryption:
