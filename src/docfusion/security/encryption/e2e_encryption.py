...
class EncryptedDocument(BaseModel):
	...
	sharing_permissions: Dict[str, List[str]] = Field(default_factory=dict)  # user_id -> permissions
...
class E2EEncryption:
	...
	async def generate_user_keypair(self, user_id: str) -> str:
		...
		try:
			if self.config.default_key_exchange == KeyExchangeProtocol.RSA_OAEP:
				...
				raise ValueError(f"Unsupported key exchange protocol: {self.config.default_key_exchange}")
	...
	async def share_document(
		...
		try:
			...
			self.logger.info(f"Document {document_id} shared by {shared_by} with {shared_with}")
	...
	async def revoke_document_access(
		...
		try:
			...
			if revoked_by != encrypted_doc.encrypted_by and revoked_by not in encrypted_doc.authorized_users:
				return E2EEncryptionResult(
					success=False,
					operation="revoke",
			...
			for share_id, share in self.document_shares.items():
				if share.document_id == document_id and share.shared_with == revoked_user:
					shares_to_remove.append(share_id)
			
			for share_id in shares_to_remove:
				del self.document_shares[share_id]
			
			self.logger.info(f"Revoked access to document {document_id} for user {revoked_user}")
	...
	def _generate_document_key(self, scheme: EncryptionScheme) -> bytes:
		...
		if scheme == EncryptionScheme.AES_256_GCM or scheme == EncryptionScheme.AES_256_CBC:
	...
	async def _encrypt_key_for_user(
		self,
		key: bytes,
		...
		if user_keypair.key_exchange_protocol == KeyExchangeProtocol.RSA_OAEP:
			...
			raise ValueError(f"Unsupported key exchange protocol: {user_keypair.key_exchange_protocol}")
	...
	async def get_statistics(self) -> Dict[str, Any]:
		...
		return {
			...
			'encryption_schemes_used': list(set(
...
# Factory functions
def create_e2e_encryption(config: Optional[E2EEncryptionConfig] = None) -> E2EEncryption:
