...
class DocumentPermissions:
	...
	def __init__(self, config: Optional[DocumentPermissionsConfig] = None):
		...
		self.document_access: Dict[str, List[DocumentAccess]] = {}  # document_id -> access list
		self.sharing_settings: Dict[str, DocumentSharingSettings] = {}  # document_id -> settings
		self.inheritance_settings: Dict[str, DocumentPermissionInheritance] = {}  # document_id -> inheritance
	...
	async def grant_document_access(
		...
		if sum(1 for s in subjects if s is not None) != 1:
			raise ValueError("Exactly one of user_id, role_id, team_id, or email must be specified")
		...
		if email and self.config.require_approval_for_external_shares:
			...
			self.logger.info(f"External share requires approval: {email} for document {document_id}")
		...
		if email:
			...
			if external_count >= self.config.max_external_shares_per_document:
				raise ValueError(f"Maximum external shares limit reached: {self.config.max_external_shares_per_document}")
		...
		existing_access = await self._find_existing_access(document_id, user_id, role_id, team_id, email)
		if existing_access:
		...
		# Log access grant
		await self._log_access_event("access_granted", document_id, granted_by, {
			'access_id': access_id,
		...
		self.logger.info(f"Granted {permission_level.value} access to document {document_id}")
		return access_id
	...
	async def check_document_permission(
		...
		for access in effective_permissions:
			if self._permission_level_value(access.permission_level) > self._permission_level_value(highest_permission):
		...
		has_permission = self._permission_level_value(highest_permission) >= self._permission_level_value(requested_permission)
		
		# Log access check
		if self.config.log_all_access:
			await self._log_access_event("permission_check", document_id, user_id, {
				'requested_permission': requested_permission.value,
				'granted_permission': highest_permission.value,
				'has_permission': has_permission,
				'access_type': granted_by_access.access_type.value if granted_by_access else None
			})
	...
	async def revoke_document_access(
		...
		for access in document_accesses:
			if access.access_id == access_id and access.is_active:
				...
				# Log revocation
				await self._log_access_event("access_revoked", document_id, revoked_by, {
					'access_id': access_id,
					'permission_level': access.permission_level.value
				})
				
				self.logger.info(f"Revoked access {access_id} to document {document_id}")
				return True
	...
	async def create_share_link(
		...
		if expires_in_days:
			...
			expires_at = datetime.now(timezone.utc) + timedelta(days=self.config.default_link_expiry_days)
		...
		await self._log_access_event("share_link_created", document_id, created_by, {
			'share_link': share_link,
			'permission_level': permission_level.value,
	...
	async def access_via_share_link(
		...
		if sharing_settings.link_expires_at and datetime.now(timezone.utc) > sharing_settings.link_expires_at:
			return {'success': False, 'error': 'Share link has expired'}
		
		# Check password if required
		if sharing_settings.link_password and password != sharing_settings.link_password:
		...
		# Log link access
		await self._log_access_event("share_link_accessed", document_id, user_id or "anonymous", {
			'share_link': share_link,
	...
	async def set_document_inheritance(
		...
		if parent_document_id:
			...
			if depth >= self.config.max_inheritance_depth:
				raise ValueError(f"Maximum inheritance depth exceeded: {self.config.max_inheritance_depth}")
		...
		if inherit_permissions and parent_document_id:
			await self._apply_inherited_permissions(document_id, parent_document_id, updated_by)
		
		# Log inheritance change
		await self._log_access_event("inheritance_updated", document_id, updated_by, {
			'parent_document_id': parent_document_id,
			'inherit_permissions': inherit_permissions,
			'inherit_sharing': inherit_sharing
		})
		
		self.logger.info(f"Set inheritance for document {document_id} -> parent {parent_document_id}")
		return True
	...
	async def get_user_accessible_documents(
		self,
		user_id: str,
		minimum_permission: PermissionLevel = PermissionLevel.VIEW
	) -> List[Dict[str, Any]]:
	...
	async def _get_user_effective_permissions(
		...
		if inheritance and inheritance.inherit_permissions and inheritance.parent_document_id:
	...
	def _check_access_restrictions(self, access: DocumentAccess, context: Dict[str, Any]) -> bool:
	...
	async def _find_existing_access(
		...
		for access in document_accesses:
			if (access.user_id == user_id and
				access.role_id == role_id and
				access.team_id == team_id and
				access.email == email):
				return access
	...
	async def _calculate_inheritance_depth(self, document_id: str) -> int:
		...
		return 1 + await self._calculate_inheritance_depth(inheritance.parent_document_id)
...
# Factory function
def create_document_permissions(config: Optional[DocumentPermissionsConfig] = None) -> DocumentPermissions:
