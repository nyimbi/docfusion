...
class RoleBasedAccess:
    ...
    def __init__(self, config: Optional[RBACConfiguration] = None):
        ...
        self.user_role_assignments: Dict[
            str, List[UserRoleAssignment]
        ] = {}  # user_id -> assignments
    ...
    async def create_role(
        ...
        if parent_role_id:
            role.inherited_permissions = await self._calculate_inherited_permissions(
                parent_role_id
            )
    ...
    async def assign_role_to_user(
        ...
        if role.requires_approval:
            ...
            self.logger.info(
                f"Role assignment requires approval: {role.name} for user {user_id}"
            )
        ...
        active_assignments = [
            ...
            if a.is_active and not self._is_assignment_expired(a)
        ]
        if len(active_assignments) >= self.config.max_roles_per_user:
            raise ValueError(
                f"User has reached maximum role limit: {self.config.max_roles_per_user}"
            )
        ...
        if expires_in_days:
            ...
            expires_at = datetime.now(timezone.utc) + timedelta(
                days=role.auto_expire_days
            )
        elif self.config.default_assignment_duration_days:
            expires_at = datetime.now(timezone.utc) + timedelta(
                days=self.config.default_assignment_duration_days
            )
    ...
    async def check_permission(
            ...
            for perm_id in role_permissions:
                ...
                if (
                    ...
                    if not self._check_ip_restrictions(
                        permission, context.get("ip_address")
                    ):
                        continue
        ...
        if self.config.log_all_access_checks:
            self.logger.info(
                f"Access check: {user_id} -> {resource_type.value}:{action.value} = {decision.value}"
            )
    ...
    async def revoke_user_role(
        ...
        for assignment in user_assignments:
            if assignment.role_id == role_id and assignment.is_active:
                ...
                self.logger.info(
                    f"Revoked role {role_id} from user {user_id} by {revoked_by}"
                )
                return True
    ...
    async def suspend_user_role(
        ...
        for assignment in user_assignments:
            if assignment.role_id == role_id and assignment.is_active:
                assignment.suspended_until = datetime.now(timezone.utc) + timedelta(
                    hours=suspension_duration_hours
                )
                assignment.suspension_reason = reason
                self._invalidate_cache()

                self.logger.info(
                    f"Suspended role {role_id} for user {user_id} for {suspension_duration_hours}h"
                )
                return True
    ...
    async def _get_user_active_roles(self, user_id: str) -> List[UserRoleAssignment]:
        ...
        for assignment in user_assignments:
            if (
                ...
                and not self._is_assignment_expired(assignment)
                and not self._is_assignment_suspended(assignment)
            ):
    ...
    def _check_scope_match(
        self, permission_scope: str, resource_id: Optional[str]
    ) -> bool:
        ...
        if ":" in permission_scope:
            return resource_id.startswith(
                permission_scope
            ) or permission_scope.startswith(resource_id)
    ...
    def _check_ip_restrictions(
        self, permission: Permission, ip_address: Optional[str]
    ) -> bool:
    ...
    async def _calculate_inherited_permissions(self, parent_role_id: str) -> List[str]:
        ...
        if parent_role:
            ...
            if parent_role.parent_role_id:
                grandparent_permissions = await self._calculate_inherited_permissions(
    ...
    async def _validate_role_hierarchy(
        self, parent_role_id: str, child_level: RoleLevel
    ) -> bool:
...
def create_role_based_access(
    config: Optional[RBACConfiguration] = None,
) -> RoleBasedAccess:
