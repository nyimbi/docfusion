...
from .interpreter import InterpretedComposition, ExecutionGraph, ExecutionNode, NodeType, ExecutionMode
...
class CompositionRunnerError(Exception):
	...
	def __init__(self, message: str, execution_id: Optional[str] = None, node_id: Optional[str] = None):
...
class CompositionRunner:
	...
	async def execute_composition(
		...
		self.logger.info(f"Starting execution {execution_id} for composition: {composition.name}")
		...
		try:
			...
			self.logger.info(f"Completed execution {execution_id} with status: {execution_report.status}")
	...
	async def _execute_workflow(self, composition: InterpretedComposition, context: ExecutionContext) -> ExecutionReport:
		...
		try:
			...
			for level_index, level_nodes in enumerate(graph.execution_order):
				self.logger.debug(f"Executing level {level_index} with {len(level_nodes)} nodes")
				...
				failed_nodes = [r for r in level_results.values() if r.status == ExecutionStatus.FAILED]
				if failed_nodes and composition.global_config.get('fail_fast', True):
					self.logger.warning(f"Stopping execution due to {len(failed_nodes)} failed nodes")
					break
			
			# Determine overall status
			end_time = time.time()
			successful_count = sum(1 for r in node_results.values() if r.status == ExecutionStatus.COMPLETED)
			failed_count = sum(1 for r in node_results.values() if r.status == ExecutionStatus.FAILED)
			
			overall_status = ExecutionStatus.COMPLETED if failed_count == 0 else ExecutionStatus.FAILED
			
			# Generate performance metrics
			performance_metrics = await self._calculate_performance_metrics(node_results, start_time, end_time)
	...
	async def _execute_node(self, node_id: str, node: ExecutionNode, context: ExecutionContext) -> NodeExecutionResult:
		...
		try:
			...
			if max_attempts > 1:
				...
				if should_retry:
					...
					self.logger.info(f"Error type {type(e).__name__} not configured for retry")
	...
	async def _execute_agent_node(self, node: ExecutionNode, context: ExecutionContext) -> Any:
		"""Execute agent node"""
	...
	async def _execute_function_node(self, node: ExecutionNode, context: ExecutionContext) -> Any:
	...
	async def _execute_condition_node(self, node: ExecutionNode, context: ExecutionContext) -> Any:
	...
	async def _execute_join_node(self, node: ExecutionNode, context: ExecutionContext) -> Any:
		"""Execute join node"""
	...
	async def _check_dependencies(self, node: ExecutionNode, context: ExecutionContext) -> bool:
		...
		for dep_id in node.dependencies:
			...
			if hasattr(dep_result, 'status') and dep_result.status != ExecutionStatus.COMPLETED:
				return False
		
		return True
	
	async def _update_context_with_results(self, context: ExecutionContext, results: Dict[str, NodeExecutionResult]):
	...
	async def _builtin_merge(self, node: ExecutionNode, context: ExecutionContext) -> Any:
	...
	async def _builtin_filter(self, node: ExecutionNode, context: ExecutionContext) -> Any:
	...
	async def _builtin_transform(self, node: ExecutionNode, context: ExecutionContext) -> Any:
		...
		for var_name, var_value in context.variables.items():
			transformed_output = transformed_output.replace(f'{{{var_name}}}', str(var_value))
	...
	async def _apply_execution_options(self, context: ExecutionContext, options: Dict[str, Any]):
	...
	async def _calculate_performance_metrics(
		...
		return {
			...
			'average_node_duration': sum(durations) / len(durations) if durations else 0,
			'max_node_duration': max(durations) if durations else 0,
			'min_node_duration': min(durations) if durations else 0,
			'throughput': len(node_results) / (end_time - start_time) if (end_time - start_time) > 0 else 0,
			'success_rate': sum(1 for r in node_results.values() if r.status == ExecutionStatus.COMPLETED) / len(node_results) if node_results else 0
		}
	
	async def _calculate_resource_usage(self, node_results: Dict[str, NodeExecutionResult]) -> Dict[str, Any]:
		...
		return {
			...
			'agent_invocations': sum(1 for r in node_results.values() if 'agent_id' in r.metadata)
		}
	...
	async def stream_execution(self, composition: InterpretedComposition, input_data: Dict[str, Any]) -> AsyncIterator[Dict[str, Any]]:
	...
	async def _execute_node_with_retry(
		...
		retry_on = retry_config.get('retry_on', ['Exception'])  # Exception types to retry on
		...
		self.logger.info(f"Starting retry for node {node_id} (max_attempts: {max_attempts})")
		
		for attempt in range(2, max_attempts + 1):  # Start from 2 since first attempt already failed
			# Check if this error type should trigger a retry
			should_retry = False
			if retry_on == ['Exception']:  # Default: retry on any exception
				should_retry = True
			...
			if not should_retry:
				self.logger.info(f"Error type {type(last_error).__name__} not configured for retry")
				break
			...
			self.logger.info(f"Retry attempt {attempt}/{max_attempts} for node {node_id} after {delay:.2f}s delay")
			...
			try:
				...
				self.logger.info(f"Node {node_id} succeeded on attempt {attempt}/{max_attempts}")
				...
				self.logger.warning(f"Retry attempt {attempt}/{max_attempts} failed for node {node_id}: {str(e)}")
		...
		self.logger.error(f"Node {node_id} failed after {max_attempts} attempts. Final error: {str(last_error)}")
