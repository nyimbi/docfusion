...
class CompositionParser:
	...
	def _compile_patterns(self):
		...
		self.operator_patterns['branch'] = re.compile(r'(\w+)\s*<\s*\(\s*(\w+)\s*,\s*(\w+)\s*,\s*(\w+)\s*\)')
		...
		try:
			...
			for flow_id, flow_definition in flow_data.items():
				...
				if 'sequence' in flow_definition:
					...
					parsed_flow = await self._parse_flow(flow_id, flow_definition, agents)
			...
			self.logger.info(f"Successfully parsed composition: {parsed_composition.name}")
	...
	async def _parse_flow(self, flow_id: str, flow_definition: Dict[str, Any], agents: Dict[str, Any]) -> ParsedFlow:
		...
		try:
			...
			if not sequence:
				raise CompositionParseError(f"Flow {flow_id} missing sequence definition")
			...
			left_operands = {expr.left_operand for expr in expressions if expr.left_operand}
	...
	async def _parse_sequence(self, sequence: str, agents: Dict[str, Any]) -> List[ParsedExpression]:
	...
	async def _parse_expression(self, expr_text: str, agents: Dict[str, Any]) -> Optional[ParsedExpression]:
		...
		for operator_name, pattern in self.operator_patterns.items():
			...
			if match:
				...
				operator_type = OperatorType(self.operators.get(self._get_operator_symbol(operator_name), operator_name))
				...
				if operator_type == OperatorType.BRANCH:
					...
					return ParsedExpression(
						operator=operator_type,
						left_operand=match.group(1),
						right_operand=match.group(2) if len(match.groups()) > 1 else None,
						raw_expression=expr_text
					)
	...
	async def validate_expression(self, expression: str, agents: List[str]) -> Dict[str, Any]:
		...
		try:
			...
			if parsed_expr:
				...
				return {
					"valid": False,
					"issues": ["Unable to parse expression"]
				}
