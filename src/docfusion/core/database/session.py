...
from typing import Optional, Callable, AsyncGenerator, Generator, Dict, Any, TypeVar
...
class DatabaseSession:
	...
	def sync_session(self, autocommit: bool = True) -> Generator[Session, None, None]:
		...
		"""
		if self.connection is None:
			# Use ThreadPoolExecutor to avoid asyncio.run() crashing in async contexts
			with ThreadPoolExecutor(max_workers=1) as executor:
				future = executor.submit(asyncio.run, self._get_connection())
	...
	async def health_check(self) -> Dict[str, Any]:
		...
		try:
			...
			with self.sync_session() as session:
				session.execute(text("SELECT current_database(), current_user, version()"))
		...
		try:
			...
			async with self.async_session() as session:
				await session.execute(text("SELECT current_database(), current_user, version()"))
