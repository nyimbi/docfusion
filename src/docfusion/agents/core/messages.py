...
class MessagePayload(BaseModel):
	...
	references: List[str] = Field(default_factory=list)  # References to other messages/resources
...
class AgentMessage(BaseModel):
	...
	def is_expired(self) -> bool:
		...
		if self.header.ttl_seconds:
			expiry_time = self.header.timestamp + timedelta(seconds=self.header.ttl_seconds)
...
class MessageBuilder:
	...
	def with_attachment(self, name: str, data: Any, content_type: str = "application/octet-stream") -> 'MessageBuilder':
	...
	def build(self) -> AgentMessage:
		...
		if header.ttl_seconds:
			metadata.expires_at = header.timestamp + timedelta(seconds=header.ttl_seconds)
...
class MessageTemplates:
	...
	@staticmethod
	def task_request(sender_id: str, recipient_id: str, task_data: Any, 
					priority: MessagePriority = MessagePriority.MEDIUM) -> AgentMessage:
		"""Create a task request message"""
		return (MessageBuilder(sender_id)
				.to(recipient_id)
	...
	@staticmethod
	def task_response(sender_id: str, recipient_id: str, result: Any, 
					 original_message_id: str) -> AgentMessage:
	...
	@staticmethod
	def collaboration_request(sender_id: str, recipients: List[str], 
							 collaboration_data: Any) -> List[AgentMessage]:
		"""Create collaboration request messages"""
		messages = []
		conversation_id = uuid7str()
		
		for recipient in recipients:
			message = (MessageBuilder(sender_id)
					  .to(recipient)
	...
	@staticmethod
	def broadcast_announcement(sender_id: str, announcement: str, 
							  priority: MessagePriority = MessagePriority.MEDIUM) -> AgentMessage:
	...
	@staticmethod
	def error_report(sender_id: str, recipient_id: str, error_info: Dict[str, Any]) -> AgentMessage:
	...
	@staticmethod
	def knowledge_share(sender_id: str, knowledge_data: Any, 
					   category: str = "general") -> AgentMessage:
	...
	@staticmethod
	def urgent_alert(sender_id: str, alert_message: str, 
					recipients: Optional[List[str]] = None) -> AgentMessage:
		"""Create an urgent alert message"""
		builder = (MessageBuilder(sender_id)
				  .with_type(MessageType.URGENT_ALERT)
				  .urgent()
				  .with_content({"alert": alert_message, "severity": "urgent"})
				  .with_ttl(300))  # 5 minutes TTL
...
class MessageValidator:
	...
	def validate_message(message: AgentMessage) -> List[str]:
		...
		if (message.header.delivery_mode == MessageDeliveryMode.DIRECT and 
			not message.header.recipient_id):
			errors.append("Recipient ID required for direct messages")
		
		# Check response requirements
		if (message.header.response_expected and 
			not message.header.response_timeout_seconds):
