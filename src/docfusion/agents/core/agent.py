...
class AgentConfig(BaseModel):
    ...
    # LLM settings
    llm_model: str = Field(
        default="qwen2.5:1.5b", description="Default LLM model to use"
    )
    llm_host: str = Field(
...
class Agent(BaseAgent, ABC, Generic[T]):
    ...
    async def assign_task(
        self, task: Any, priority: AgentPriority = AgentPriority.MEDIUM
    ) -> str:
        """Assign a new task to this agent"""
        if (
            len(self.context.current_tasks)
            >= self.config.capabilities.max_concurrent_tasks
        ):
    ...
    async def send_message(
        self, recipient: str, message: Any, message_type: str = "general"
    ) -> str:
    ...
    async def broadcast_message(
        self, message: Any, message_type: str = "broadcast"
    ) -> List[str]:
    ...
    async def request_collaboration(self, agents: List[str], task: Any) -> str:
        ...
        self.logger.info(
            f"Requested collaboration {collaboration_id} with agents: {agents}"
        )
    ...
    def set_goal(
        self, goal_id: str, goal_description: str, target_value: float = 1.0
    ) -> None:
    ...
    async def use_tool(self, tool_name: str, **kwargs) -> Any:
        ...
        if not self.tool_registry:
            self.logger.warning(
                f"Tool registry not available, cannot use tool: {tool_name}"
            )
            return None
    ...
    async def write_file(
        ...
        return await self.use_tool(
            "file_write", file_path=file_path, content=content, encoding=encoding
        )
    ...
    async def list_directory(
        ...
        return await self.use_tool(
            "directory_list", directory_path=directory_path, recursive=recursive
        )

    async def search_files(
        self, search_term: str, search_path: str = ".", search_type: str = "name"
    ) -> Optional[Any]:
    ...
    async def process_json(self, operation: str, data: Any, **kwargs) -> Optional[Any]:
        ...
        return await self.use_tool(
            "json_processor", operation=operation, data=data, **kwargs
        )

    async def process_csv(self, operation: str, data: Any, **kwargs) -> Optional[Any]:
        """Convenience method for CSV processing"""
        return await self.use_tool(
            "csv_processor", operation=operation, data=data, **kwargs
        )

    async def process_text(self, operation: str, text: str, **kwargs) -> Optional[Any]:
        """Convenience method for text processing"""
        return await self.use_tool(
            "text_processor", operation=operation, text=text, **kwargs
        )

    async def research_company(
        self, company_name: str, research_depth: str = "basic"
    ) -> Optional[Any]:
        """Convenience method for company research"""
        return await self.use_tool(
            "company_research", company_name=company_name, research_depth=research_depth
        )

    async def research_market(
        self, market_or_industry: str, geographic_scope: str = "global"
    ) -> Optional[Any]:
    ...
    async def _notify_tool_usage(
        self, tool_name: str, success: bool, metadata: Dict[str, Any]
    ) -> None:
        """Notify context manager about tool usage"""
        if self.context_manager:
            try:
                await self.context_manager.update_agent_status(
                    self.agent_id,
                    {
        ...
        try:
            ...
            if not system_prompt:
                if self.context_manager and task_info:
                    ...
                    system_prompt = await self.context_manager.build_context_prompt(
                        self.agent_id, task_info
                    )
                else:
                    # Fallback to basic agent context
                    system_prompt = f"You are {self.name}, {self.config.description}. "
                    system_prompt += (
                        f"Your primary role is {self.config.primary_role}. "
                    )
    ...
    async def analyze_voice_consistency(
        ...
        try:
            ...
            self.store_knowledge(
                f"voice_analysis_{organization}", result, "voice_consistency"
            )
    ...
    async def _execute_task(self, task_id: str, task: Any) -> Any:
        ...
        try:
            ...
            self.metrics.success_rate = self.metrics.tasks_completed / (
    ...
    async def _monitor_performance(self) -> None:
        ...
        while self._running:
            try:
                ...
                self.metrics.memory_usage = len(self._knowledge_base) + len(
                ...
                if (
                    ...
                    > self.config.capabilities.max_memory_items
                ):
                ...
                timeout_threshold = timedelta(
                    seconds=self.config.capabilities.task_timeout_seconds * 2
                )
    ...
    async def _cleanup_memory(self) -> None:
        ...
        for key in items_to_remove[
            : len(items_to_remove) // 2
        ]:  # Remove half of candidates
    ...
    def _setup_llm_client(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"LLM client initialized: {self.config.llm_model} @ {self.config.llm_host}"
            )
        except Exception as e:
    ...
    def _initialize_goals(self) -> None:
        ...
        self.set_goal(
            "operational_efficiency", "Maintain high task completion rate", 0.9
        )
    ...
    async def _register_with_context_manager(self) -> None:
        ...
        try:
            ...
            success = await self.context_manager.register_agent(
                self.agent_id, agent_info
            )
            if success:
                self.logger.info(f"Agent {self.name} registered with context manager")
            else:
                self.logger.warning(
                    f"Failed to register agent {self.name} with context manager"
                )
        except Exception as e:
    ...
    async def _notify_task_completion(
        ...
        if self.context_manager:
            try:
                await self.context_manager.register_task_completion(
                    self.agent_id, task_id, results
                )
            except Exception as e:
    ...
    def __repr__(self) -> str:
        return (
            f"Agent(name='{self.name}', id='{self.agent_id}', "
