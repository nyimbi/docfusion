	...
	def validate_parameters(self, parameters: Dict[str, Any]) -> bool:
		...
		for param in required_params:
			if param not in parameters:
				raise ToolError(f"Missing required parameter: {param}", self.name, "MISSING_PARAM")
	...
	async def _execute_with_retries(self, **kwargs) -> ToolResult:
		...
		for attempt in range(self.config.max_retries + 1):
			try:
				...
				last_error = ToolError(f"Tool execution timed out after {self.config.timeout_seconds}s", 
									 self.name, "TIMEOUT")
...
class ToolRegistry:
	...
	def get_registry_stats(self) -> Dict[str, Any]:
		...
		return {
			...
			"capabilities": {cap.value: len(tools) for cap, tools in self.tool_categories.items()},
			"tool_stats": {name: tool.get_usage_stats() for name, tool in self.tools.items()}
		}
