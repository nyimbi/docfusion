...
class TaskExecutor:
	...
	async def submit_task(self, task_def: TaskDefinition) -> str:
		...
		# Sort by priority
		self.pending_tasks.sort(key=lambda t: self._get_priority_value(t.priority), reverse=True)
	...
	async def wait_for_task(self, task_id: str, timeout_seconds: Optional[int] = None) -> TaskResult:
		"""
	...
	async def _execute_pending_tasks(self) -> None:
		...
		while (len(self.running_tasks) < self.max_concurrent_tasks and 
			   self.pending_tasks and self._running):
	...
	async def _perform_service_health_checks(self) -> None:
		...
		for service_name, service in self.service_registry.items():
			try:
				...
				self.logger.warning(f"Service health check failed for {service_name}: {e}")
