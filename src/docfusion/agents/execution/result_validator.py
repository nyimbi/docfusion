...
class ResultValidator:
	...
	async def validate_result(self, data: Any, rule_set: Optional[List[str]] = None,
		...
		try:
			...
			rules_to_apply = []
			if rule_set:
				rules_to_apply = [self.validation_rules[rid] for rid in rule_set 
								 if rid in self.validation_rules and self.validation_rules[rid].enabled]
			else:
			...
			self.logger.info(f"Validation completed: {validation_result.overall_score:.1f} score, "
	...
	async def validate_consistency(self, results: List[Any], 
								   consistency_rules: Optional[Dict[str, Any]] = None) -> ValidationResult:
		"""
		...
		for rule_name, rule_config in consistency_rules.items():
			issues = await self._apply_custom_consistency_rule(rule_name, rule_config, results)
			validation_result.issues.extend(issues)
		...
		self.logger.info(f"Consistency validation completed: {validation_result.overall_score:.1f} score")
		
		return validation_result

	async def validate_quality_score(self, result: Any, quality_criteria: Dict[str, Any]) -> float:
		"""
		...
		try:
			...
			for criterion, config in quality_criteria.items():
				...
				score = await self._evaluate_quality_criterion(result, criterion, config)
	...
	async def get_validation_report(self, validation_result: ValidationResult) -> str:
		...
		if critical_issues:
			report_lines.extend([
				"## 🔴 Critical Issues",
				*[f"- **{issue.rule_name or 'Unknown'}**: {issue.message}" for issue in critical_issues],
				""
			])
		...
		if error_issues:
			report_lines.extend([
				"## 🟠 Errors",
				*[f"- **{issue.rule_name or 'Unknown'}**: {issue.message}" for issue in error_issues],
				""
			])
		...
		if warning_issues:
			report_lines.extend([
				"## 🟡 Warnings",
				*[f"- **{issue.rule_name or 'Unknown'}**: {issue.message}" for issue in warning_issues[:10]],  # Limit to 10
				"" if len(warning_issues) <= 10 else f"... and {len(warning_issues) - 10} more warnings",
				""
			])
		...
		if info_issues:
			report_lines.extend([
				"## ℹ️ Information",
				*[f"- **{issue.rule_name or 'Unknown'}**: {issue.message}" for issue in info_issues[:5]],  # Limit to 5
				"" if len(info_issues) <= 5 else f"... and {len(info_issues) - 5} more info items",
				""
			])
		...
		if all_suggestions:
			unique_suggestions = list(set(all_suggestions))[:10]  # Top 10 unique suggestions
			report_lines.extend([
				"## Recommendations",
	...
	async def _apply_validation_rule(self, rule: ValidationRule, data: Any, 
		...
		try:
			...
			if rule.custom_validator and rule.custom_validator in self.custom_validators:
				...
				try:
					custom_issues = await self._call_custom_validator(validator_func, target_value, rule, context)
	...
	async def _call_custom_validator(self, validator_func: Callable, value: Any, 
									 rule: ValidationRule, context: Dict[str, Any]) -> List[ValidationIssue]:
	...
	async def _check_structural_consistency(self, results: List[Any], validation_result: ValidationResult):
	...
	async def _check_value_consistency(self, results: List[Any], validation_result: ValidationResult):
	...
	async def _check_type_consistency(self, results: List[Any], validation_result: ValidationResult):
	...
	async def _apply_custom_consistency_rule(self, rule_name: str, rule_config: Dict[str, Any], 
											 results: List[Any]) -> List[ValidationIssue]:
		"""Apply custom consistency rule"""
		# This would be implemented based on specific consistency requirements
		return []

	async def _evaluate_quality_criterion(self, result: Any, criterion: str, config: Dict[str, Any]) -> float:
	...
	async def _evaluate_completeness(self, result: Any, config: Dict[str, Any]) -> float:
	...
	def _determine_validity(self, validation_result: ValidationResult) -> bool:
		...
		if self.strict_mode:
			return not any(issue.level in [ValidationLevel.ERROR, ValidationLevel.CRITICAL] 
						  for issue in validation_result.issues)
		else:
	...
	def _update_validation_stats(self, validation_result: ValidationResult):
		...
		new_avg = ((current_avg * (total - 1)) + validation_result.overall_score) / total
	...
	def _initialize_default_rules(self):
		...
		self.register_validation_rule(ValidationRule(
