...
class ContextManager:
    ...
    def __init__(self):
        ...
        self.workflow_subscriptions: Dict[
            str, Set[str]
        ] = {}  # workflow_id -> agent_ids
    ...
    async def update_agent_status(
        self, agent_id: str, status_update: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            for entry_id, entry in self.context_store.items():
                ...
                if entry.scope == ContextScope.GLOBAL:
                    ...
                    entry.scope == ContextScope.PRIVATE
                    and entry.source_agent_id == agent_id
                ):
                    should_include = True
                    ...
                    if (
                        agent_id in self.agent_awareness
                        and entry.source_agent_id in self.agent_awareness
                    ):
                        agent_role = self.agent_awareness[agent_id].agent_role
                        source_role = self.agent_awareness[
            ...
            relevant_entries.sort(
                key=lambda x: (x.priority, x.created_at), reverse=True
            )
    ...
    async def get_agent_awareness(
        self, requesting_agent_id: str
    ) -> Dict[str, AgentAwarenessInfo]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get agent awareness for {requesting_agent_id}: {e}"
            )
            return {}
        ...
        try:
            await self.add_context(
                ContextEntry(
                    ...
                    scope=ContextScope.WORKFLOW,
                    source_agent_id=agent_id,
                    source_agent_name=self.agent_awareness[agent_id].agent_name
    ...
    async def register_task_completion(
        self, agent_id: str, task_id: str, results: Dict[str, Any]
    ) -> bool:
...
{other_agents_work}""")
    ...
    def _filter_agent_awareness(
        self, awareness: AgentAwarenessInfo, requesting_agent_id: str
    ) -> AgentAwarenessInfo:
...
Client: {workflow.client_context.get("organization", "Not specified")}""")
    ...
    async def _get_recent_agent_work(
        self, requesting_agent_id: str, hours: int = 24
    ) -> Optional[str]:
        ...
        for entry in self.context_store.values():
            if (
                entry.context_type in [ContextType.RESULTS, ContextType.WORK_PROGRESS]
                and entry.created_at > cutoff_time
                and entry.source_agent_id != requesting_agent_id
            ):
        ...
        for workflow_id in workflows:
            ...
            if workflow and workflow.quality_requirements:
                quality_parts.append(
                    f"Quality standards: {workflow.quality_requirements}"
                )
    ...
    async def _cleanup_expired_context(self):
        ...
        if expired_entries:
            self.logger.debug(
                f"Cleaned up {len(expired_entries)} expired context entries"
            )
