...
class MessageBus:
	...
	def __init__(self, config: Optional[MessageBusConfig] = None):
		...
		self.request_response_registry: Dict[str, str] = {}  # message_id -> waiting_agent_id
	...
	async def publish_to_topic(self, topic: str, message: AgentMessage) -> int:
		...
		try:
			...
			self.logger.debug(f"Published message to topic {topic} - {delivery_count} recipients")
		...
		for agent_id, channel in self.agent_channels.items():
			if agent_id != message.header.sender_id:  # Don't send to sender
				try:
					...
					self.logger.warning(f"Broadcast delivery failed for {agent_id}: {e}")
		...
		for member_id in group_members:
			if member_id in self.agent_channels:
				try:
					...
					self.logger.warning(f"Multicast delivery failed for {member_id}: {e}")
	...
	async def _retry_failed_messages(self) -> None:
		...
		for message_id, message in self.failed_messages.items():
			if (message.should_retry() and 
				len(message.metadata.delivery_attempts) < self.config.retry_attempts):
	...
	async def broadcast_system_message(self, message_content: Any, 
									  message_type: MessageType = MessageType.SYSTEM_NOTIFICATION) -> bool:
		"""Broadcast a system message to all agents"""
		from ..core.messages import MessageBuilder  # Import here to avoid circular imports
		
		message = (MessageBuilder("system")
				  .broadcast()
