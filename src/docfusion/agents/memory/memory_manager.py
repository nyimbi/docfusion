...
class MemoryIndex:
	...
	def remove_entry(self, entry: MemoryEntry) -> None:
		...
		for index in [self.content_index, self.tag_index, self.type_index, 
...
class MemoryManager:
	...
	async def store_memory(self, content: Any, memory_type: MemoryType = MemoryType.WORKING,
		...
		try:
			...
			if ttl_seconds:
				...
				entry.expires_at = datetime.now() + timedelta(hours=self.config.short_term_ttl_hours)
			elif memory_type == MemoryType.WORKING:
				entry.expires_at = datetime.now() + timedelta(minutes=self.config.working_memory_ttl_minutes)
	...
	async def create_memory_association(self, entry_id1: str, entry_id2: str, 
	...
	async def get_related_memories(self, entry_id: str, depth: int = 1) -> List[MemoryEntry]:
	...
	async def _perform_cleanup(self) -> None:
		...
		if len(self.memories) > self.config.max_long_term_entries:
			...
			entries_by_access = sorted(self.memories.values(), 
	...
	async def clear_memories(self, memory_type: Optional[MemoryType] = None,
							 scope: Optional[MemoryScope] = None,
