...
class SharedContext:
	...
	async def set(self, key: str, value: Any, context_type: ContextType = ContextType.STATE,
		...
		async with self._lock:
			try:
				...
				if not self._can_write(agent_id):
					self.logger.warning(f"Agent {agent_id} lacks write permission for {key}")
					return False
	...
	async def get(self, key: str, agent_id: Optional[str] = None, 
		...
		async with self._lock:
			try:
				...
				if not self._can_read(agent_id, entry):
					self.logger.warning(f"Agent {agent_id} lacks read permission for {key}")
					return default
		...
		async with self._lock:
			try:
				...
				if not self._can_delete(agent_id, entry):
					self.logger.warning(f"Agent {agent_id} lacks delete permission for {key}")
					return False
	...
	async def get_all(self, agent_id: Optional[str] = None, 
					  context_type: Optional[ContextType] = None) -> Dict[str, Any]:
	...
	def _can_write(self, agent_id: Optional[str]) -> bool:
		...
		return permission in [ContextAccess.READ_WRITE, ContextAccess.ADMIN, ContextAccess.OWNER]
	...
	async def _notify_observers(self, action: str, key: str, value: Any, agent_id: Optional[str]) -> None:
...
class ContextManager:
	...
	async def create_context(self, scope: ContextScope, name: str = "", 
		...
		try:
			...
			self.logger.info(f"Created context: {name} ({context_id}) with scope {scope}")
			return context_id
	...
	async def delete_context(self, context_id: str, agent_id: Optional[str] = None) -> bool:
		"""Delete context"""
		try:
	...
	async def add_participant(self, context_id: str, agent_id: str, 
							  access: ContextAccess = ContextAccess.READ_ONLY) -> bool:
	...
	async def get_inherited_value(self, context_id: str, key: str, 
	...
	async def propagate_to_children(self, context_id: str, key: str, value: Any,
	...
	async def get_agent_contexts(self, agent_id: str, 
								 scope: Optional[ContextScope] = None) -> List[SharedContext]:
	...
	async def create_crew_context(self, crew_id: str, participants: List[str]) -> str:
		...
		for participant_id in participants:
			await self.add_participant(context_id, participant_id, ContextAccess.READ_WRITE)
		
		return context_id
	
	async def create_workflow_context(self, workflow_id: str, execution_context: Dict[str, Any]) -> str:
	...
	async def query_contexts(self, 
							 ...
							 contains_key: Optional[str] = None) -> List[SharedContext]:
	...
	def get_context_statistics(self) -> Dict[str, Any]:
		...
		for context in self.contexts.values():
			...
			stats["contexts_by_scope"][scope_key] = stats["contexts_by_scope"].get(scope_key, 0) + 1
