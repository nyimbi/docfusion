...
from ..orchestration.task_orchestrator import TaskOrchestrator, WorkflowEngine, WorkflowDefinition
from ..specialists import ResearchAgent, WriterAgent, ReviewerAgent, QualityAgent
...
class ProposalAgentBridge:
	...
	async def generate_proposal_with_crew(self, context: ProposalContext) -> Dict[str, Any]:
	...
	async def generate_proposal_with_swarm(self, context: ProposalContext) -> Dict[str, Any]:
	...
	async def _create_proposal_agent_configs(self, context: ProposalContext) -> List[Dict[str, Any]]:
		...
		configs.append({
			...
			"role": "researcher", 
			"config": {
				"research_domains": [context.requirements.industry] if context.requirements.industry else ["business"],
		...
		configs.append({
			...
			"role": "writer",
			"config": {
				"writing_style": context.requirements.style_requirements.get("style", "professional"),
				...
				"organization_profile": context.organizational_context.get("organization_name", "default")
			}
		})
		...
		configs.append({
			...
			"config": {
				...
				"quality_standards": {"minimum_score": 0.85, "voice_consistency": 0.9},
	...
	async def _create_proposal_workflow_tasks(self, context: ProposalContext) -> List[Any]:
		...
		# Content Generation
		for i, section in enumerate(context.requirements.sections_required or ["executive_summary", "approach", "timeline", "budget"]):
			writing_task = CrewTask(
				...
				context={
					...
					"voice_profile": context.organizational_context.get("organization_name")
				}
			)
		...
		review_task = CrewTask(
			...
			task_type="review",
			priority=TaskPriority.HIGH,
			dependencies=[f"write_{s}" for s in (context.requirements.sections_required or ["executive_summary", "approach", "timeline", "budget"])],
			context={
		...
		assembly_task = CrewTask(
			...
			context={
				...
				"client_branding": context.client_profile.get("branding_requirements", {}),
	...
	async def _create_proposal_swarm_tasks(self, context: ProposalContext) -> List[Any]:
		...
		sections = context.requirements.sections_required or [
			...
			"methodology", "timeline", "budget", "team", "conclusion"
		]
		...
		for section in sections:
			task = SwarmTask(
				...
				complexity=0.7,
				required_agents=1,
				dependencies=[f"research_{t}" for t in research_tasks[:3]],  # Depend on key research
				context={
					"section": section,
	...
	async def _create_proposal_workflow_definition(self, context: ProposalContext) -> WorkflowDefinition:
	...
	async def _process_crew_results(self, crew_result: Dict[str, Any], context: ProposalContext) -> Dict[str, Any]:
	...
	async def _process_swarm_results(self, swarm_result: Dict[str, Any], context: ProposalContext) -> Dict[str, Any]:
		...
		research_results = {k: v for k, v in results.items() if k.startswith("research_")}
		...
		return {
			...
			"metadata": {
				...
				"collective_intelligence": swarm_result.get("collective_intelligence", 0.0),
				"generation_timestamp": datetime.now().isoformat()
			}
		}
	...
	def get_integration_status(self) -> Dict[str, Any]:
		...
		return {
			"orchestrator_running": self.task_orchestrator and self.task_orchestrator._running,
...
class ProposalIntegration:
	...
	async def initialize(self, services: Dict[str, Any] = field(default_factory=dict)) -> None:
	...
	async def generate_proposal(self, requirements: ProposalRequirements,
								context: Optional[ProposalContext] = None,
								method: str = "crew") -> Dict[str, Any]:
	...
	def get_status(self) -> Dict[str, Any]:
		...
		return {
			...
			"bridge_status": self.bridge.get_integration_status() if self._initialized else None
		}