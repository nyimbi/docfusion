...
from ..automation.workflow_engine import WorkflowEngine, WorkflowConfiguration
from ..automation.task_scheduler import TaskScheduler, SchedulerConfiguration  
from ..coordination.task_coordinator import TaskCoordinator, CoordinatorConfiguration
from ..coordination.deadline_manager import DeadlineManager, DeadlineConfiguration
from ..monitoring.workflow_monitor import WorkflowMonitor, MonitoringConfiguration

# Import integration components
from .workflow_document_bridge import (
	WorkflowDocumentBridge, WorkflowDocumentConfiguration,
	create_workflow_document_bridge
)
from .workflow_storage_integration import (
	WorkflowStorageIntegration, create_workflow_storage_integration
)
...
from ...document_engine.document_engine import DocumentEngine, DocumentGenerationConfiguration
...
class WorkflowIntegrationLayer:
	...
	def __init__(
		...
		logger.info(f"WorkflowIntegrationLayer created in {self.config.integration_mode.value} mode")
	...
	async def _initialize_core_components(self):
		...
		coordinator_config = self.config.coordinator_config or CoordinatorConfiguration()
	...
	async def _initialize_comprehensive_integrations(self):
		...
		try:
			...
			if self.config.integration_mode in [IntegrationMode.COMPREHENSIVE, IntegrationMode.DEVELOPMENT]:
			...
			if self.config.integration_mode in [IntegrationMode.COMPREHENSIVE, IntegrationMode.PRODUCTION]:
				try:
			...
			if self.config.integration_mode == IntegrationMode.COMPREHENSIVE:
				from .unified_docufusion_integration import create_unified_docufusion_integration, IntegrationConfiguration, IntegrationScope
				...
				self.unified_integration.security_integration = self.security_integration
				...
				if unified_result['success']:
					logger.info("Unified DocuFusion integration initialized successfully")
				else:
					logger.warning(f"Unified integration initialization had issues: {unified_result.get('error', 'Unknown error')}")
	...
	async def _validate_system_health(self):
		...
		for component_name, component in [
			...
			if component and hasattr(component, 'health_check'):
				try:
					...
					health_checks.append((component_name, health.get('status') == 'healthy'))
				except Exception as e:
		...
		if failed_components:
			raise RuntimeError(f"Health check failed for components: {failed_components}")
		
		logger.info(f"System health validated - {len(health_checks)} components healthy")
		...
		try:
			...
			if self.document_bridge:
				workflow_instance = await self.document_bridge.create_document_workflow(
					...
					deadline=extended_request.deadline if hasattr(extended_request, 'deadline') else None
				)
				...
				if self.storage_integration:
					storage_result = await self.storage_integration.store_workflow_document(
						document_id=extended_request.request_id,
						workflow_instance_id=workflow_instance.instance_id,
						content=self._extract_content_from_result(result),
						title=extended_request.generation_config.document_title or "Generated Document",
						phase="completed",
				...
				return {
					...
					'workflow_instance_id': workflow_instance.instance_id,
					'generation_result': result,
					'storage_result': storage_result if self.storage_integration else None,
	...
	async def get_workflow_document_recommendations(
		...
		try:
			...
			if workflow_instance_id:
				...
				recommendations = await self.storage_integration.search_workflow_documents(
					query=query,
					user_id=user_id,
					limit=limit
				)
	...
	async def get_comprehensive_workflow_analytics(
		...
		try:
			...
			if self.workflow_monitor:
				analytics['workflow_metrics'] = await self.workflow_monitor.get_comprehensive_workflow_metrics()
			
			if self.storage_integration:
				analytics['storage_insights'] = await self.storage_integration.get_workflow_performance_insights(
			...
			if include_predictions:
				analytics['predictions'] = await self._generate_predictive_analytics(analytics)
	...
	async def create_agent_enabled_workflow(
		...
		try:
			...
			agent_assignments = await self.agents_integration.assign_agents_to_workflow(
				...
				workflow_requirements=agent_requirements or {}
			)
			...
			if len(agent_assignments) > 1:
				crew = await self.agents_integration.create_agent_crew_for_workflow(
					workflow_id=workflow_id,
					crew_configuration=agent_requirements or {}
				)
				workflow_request['agent_crew_id'] = crew.crew_id if crew else None
			
			# Execute workflow with agent coordination
			execution_result = await self.agents_integration.execute_workflow_with_agents(
				workflow_id=workflow_id,
			...
			return {
				'success': True,
				'workflow_id': workflow_id,
				'agent_assignments': {role: assignment.agent_id for role, assignment in agent_assignments.items()},
	...
	async def execute_secure_workflow(
		...
		try:
			# Authenticate user
			auth_success, auth_data = await self.security_integration.authenticate_workflow_user(
			...
			if not auth_success:
				return {'success': False, 'error': 'Authentication failed', 'auth_data': auth_data}
			
			# Create security context
			from .security_workflow_integration import WorkflowSecurityContext, WorkflowSecurityLevel
			security_context = WorkflowSecurityContext(
				user_id=auth_data['user_id'],
				security_level=WorkflowSecurityLevel(security_requirements.get('security_level', 'internal')),
				ip_address=user_credentials.get('ip_address'),
				user_agent=user_credentials.get('user_agent')
			)
	...
	async def process_workflow_with_nlp(
		...
		try:
			from .nlp_workflow_integration import NLPWorkflowContext, ContentType, NLPProcessingMode
			
			# Create NLP context
			nlp_context = NLPWorkflowContext(
				workflow_id=workflow_request.get('workflow_id', uuid7str()),
				user_id=workflow_request.get('user_id', 'system'),
				content_type=ContentType(nlp_requirements.get('content_type', 'plain_text')),
				processing_mode=NLPProcessingMode(nlp_requirements.get('processing_mode', 'batch')),
			...
			if 'pipeline_stages' in nlp_requirements:
				pipeline_results = await self.nlp_integration.execute_nlp_workflow_pipeline(
					workflow_id=nlp_context.workflow_id,
					pipeline_definition={'stages': nlp_requirements['pipeline_stages']},
					context=nlp_context
				)
				...
				return {
					'success': True,
					'workflow_id': nlp_context.workflow_id,
					'nlp_pipeline_results': [result.dict() for result in pipeline_results],
			...
			# Execute parallel NLP tasks if specified
				...
				from .nlp_workflow_integration import NLPTaskConfiguration, NLPWorkflowTaskType
				...
				parallel_results = await self.nlp_integration.coordinate_parallel_nlp_tasks(
					workflow_id=nlp_context.workflow_id,
				...
				return {
					'success': True,
					'workflow_id': nlp_context.workflow_id,
					'nlp_parallel_results': [result.dict() for result in parallel_results],
	...
	async def execute_unified_workflow(
		...
		try:
			from .unified_docufusion_integration import UnifiedWorkflowRequest, WorkflowExecutionMode
			...
			unified_workflow_request = UnifiedWorkflowRequest(
				...
				execution_mode=WorkflowExecutionMode(unified_request.get('execution_mode', 'intelligent')),
			...
			unified_result = await self.unified_integration.execute_unified_workflow(unified_workflow_request)
	...
	async def _on_workflow_started(self, event: Dict[str, Any]):
		...
		if workflow_id:
			self.performance_metrics[f'workflow_{workflow_id}_start_time'] = datetime.now()
	...
	async def _on_workflow_completed(self, event: Dict[str, Any]):
		...
		workflow_id = event.get('workflow_instance_id')
		if workflow_id:
			start_time = self.performance_metrics.get(f'workflow_{workflow_id}_start_time')
			if start_time:
	...
	async def _on_task_completed(self, event: Dict[str, Any]):
		...
		task_id = event.get('task_id')
		if task_id:
			scheduled_time = self.performance_metrics.get(f'task_{task_id}_scheduled_time')
			if scheduled_time:
	...
	async def _on_document_generated(self, event: Dict[str, Any]):
		...
		logger.info(f"Document {document_id} generated with quality score {quality_score}")
		...
		while not self._shutdown_event.is_set():
			try:
				...
				unhealthy_components = [
					name for name, status in health_summary.items()
		...
		while not self._shutdown_event.is_set():
			try:
				...
				if len(event_queue) >= self.config.event_batch_size:
					await self._process_event_batch(event_queue[:self.config.event_batch_size])
	...
	async def _collect_performance_metrics(self):
		...
		try:
			...
			if self.workflow_engine:
				engine_metrics = await self.workflow_engine.get_workflow_engine_metrics()
				self.performance_metrics[f'workflow_engine_{current_time}'] = engine_metrics
			
			# Collect task scheduler metrics
			if self.task_scheduler:
				scheduler_metrics = await self.task_scheduler.get_task_scheduler_metrics()
				self.performance_metrics[f'task_scheduler_{current_time}'] = scheduler_metrics
			
			# Collect task coordinator metrics
			if self.task_coordinator:
				coordinator_metrics = await self.task_coordinator.get_task_coordinator_metrics()
				self.performance_metrics[f'task_coordinator_{current_time}'] = coordinator_metrics
			
			# Collect deadline manager metrics
			if self.deadline_manager:
				deadline_metrics = await self.deadline_manager.get_deadline_manager_metrics()
				self.performance_metrics[f'deadline_manager_{current_time}'] = deadline_metrics
			
			# Collect workflow monitor metrics
			if self.workflow_monitor:
				monitor_metrics = await self.workflow_monitor.get_workflow_monitor_metrics()
				self.performance_metrics[f'workflow_monitor_{current_time}'] = monitor_metrics
			...
			if component:
				...
				try:
					if hasattr(component, 'health_check'):
						...
						health_summary[name] = {'status': 'healthy', 'note': 'No health check available'}
						health_summary['healthy_components'] += 1
				except Exception as e:
	...
	async def _generate_predictive_analytics(self, current_analytics: Dict[str, Any]) -> Dict[str, Any]:
	...
	async def _cleanup_old_workflow_data(self):
		...
		try:
			cutoff_date = datetime.now() - timedelta(days=self.config.workflow_data_retention_days)
			...
			if self.storage_integration and hasattr(self.storage_integration, 'cleanup_old_data'):
	...
	async def _cleanup_caches(self):
		...
		try:
			if self.storage_integration and hasattr(self.storage_integration, 'cached_recommendations'):
	...
	async def shutdown(self):
		...
		if not self.is_initialized:
			logger.warning("WorkflowIntegrationLayer not initialized, skipping shutdown")
			return
