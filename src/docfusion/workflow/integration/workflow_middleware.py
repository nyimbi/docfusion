...
from .workflow_document_bridge import (
	...
	DocumentWorkflowEvent, WorkflowProgress
)
from .workflow_request_extensions import (
	WorkflowEnabledDocumentRequest, WorkflowConfiguration,
	WorkflowIntegrationLevel, TeamRole, WorkflowTemplateManager,
	create_workflow_request_builder, template_manager
)
...
class WorkflowMiddlewareConfig(BaseModel):
	...
	default_integration_level: WorkflowIntegrationLevel = WorkflowIntegrationLevel.STANDARD
...
class WorkflowMiddleware(BaseHTTPMiddleware):
	...
	def __init__(
		...
		# State tracking
		self._active_workflows: Dict[str, str] = {}  # request_id -> workflow_instance_id
	...
	async def dispatch(self, request: Request, call_next: Callable) -> Response:
		...
		try:
			...
			if self._is_document_creation_endpoint(request):
				return await self._handle_document_creation(request, call_next, metadata)
	...
	async def _handle_document_creation(
		...
		try:
			...
			if metadata.workflow_enabled:
				...
				workflow_instance = await self._create_workflow_instance(enhanced_request, metadata)
				
				if workflow_instance:
					# Track workflow
					self._active_workflows[metadata.request_id] = workflow_instance.instance_id
					...
					return await self._execute_workflow_document_creation(
						enhanced_request, workflow_instance, request, call_next, metadata
					)
	...
	async def _create_workflow_enabled_request(
		...
		try:
			...
			try:
				priority = TaskPriority(priority_mapping.get(metadata.priority, 'MEDIUM'))
			except ValueError:
	...
	async def _create_workflow_instance(
		...
		try:
			...
			assigned_users = [assignment.user_id for assignment in workflow_request.team_assignments]
			...
			logger.info(f"Created workflow instance {workflow_instance.instance_id} in {creation_time:.3f}s")
	...
	async def _execute_workflow_document_creation(
		...
		try:
			if self.config.async_workflow_creation:
				...
				asyncio.create_task(
					self._async_workflow_execution(workflow_request, workflow_instance, metadata)
				)
				...
				return JSONResponse(
					...
					content={
						...
						"estimated_completion_time": metadata.deadline.isoformat() if metadata.deadline else None
					}
				)
			else:
			...
			logger.warning(f"Workflow execution timeout for {workflow_instance.instance_id}")
			return JSONResponse(
				status_code=202,
				content={
	...
	async def _handle_document_render(
		...
		try:
			...
			if document_id and metadata.workflow_enabled:
				...
				workflow_progress = await self.workflow_bridge.get_workflow_progress(document_id)
				...
				if workflow_progress:
					...
					if self.config.enhance_responses and hasattr(response, 'headers'):
						response.headers['X-Workflow-Instance-Id'] = workflow_progress.workflow_instance_id
						response.headers['X-Workflow-Progress'] = str(workflow_progress.completion_percentage)
						response.headers['X-Workflow-Phase'] = workflow_progress.current_phase.value
	...
	async def _extract_workflow_metadata(self, request: Request) -> WorkflowRequestMetadata:
		...
		if self.config.detect_workflow_headers:
			...
			for header_name, header_value in request.headers.items():
				if header_name.lower().startswith(self.config.workflow_header_prefix.lower()):
			...
			if f"{self.config.workflow_header_prefix}Enabled" in workflow_headers:
				metadata.workflow_enabled = workflow_headers[f"{self.config.workflow_header_prefix}Enabled"].lower() == "true"
			
			if f"{self.config.workflow_header_prefix}Template" in workflow_headers:
				metadata.workflow_template_id = workflow_headers[f"{self.config.workflow_header_prefix}Template"]
			
			if f"{self.config.workflow_header_prefix}Priority" in workflow_headers:
				metadata.priority = workflow_headers[f"{self.config.workflow_header_prefix}Priority"]
			
			if f"{self.config.workflow_header_prefix}Deadline" in workflow_headers:
				try:
					deadline_str = workflow_headers[f"{self.config.workflow_header_prefix}Deadline"]
		...
		if self.config.progress_callback_header in request.headers:
			metadata.progress_callback_url = request.headers[self.config.progress_callback_header]
		...
		if self.config.detect_workflow_params:
			...
			if 'workflow_enabled' in query_params:
				metadata.workflow_enabled = query_params['workflow_enabled'].lower() == 'true'
		...
		if (self.config.auto_workflow_integration and 
			not metadata.workflow_enabled and 
			self._is_document_creation_endpoint(request)):
		...
		if (metadata.workflow_enabled and 
			not metadata.workflow_template_id and 
			self.config.enable_template_suggestions):
			metadata.workflow_template_id = await self._suggest_workflow_template(request, metadata)
				...
				try:
					...
					if templates:
						if self.config.template_selection_strategy == "best_match":
							...
							return max(templates, key=lambda t: t.success_rate).template_id
						elif self.config.template_selection_strategy == "most_used":
							# Select most frequently used template
							return max(templates, key=lambda t: t.usage_count).template_id
						else:
							# Select latest template
							return max(templates, key=lambda t: t.created_at).template_id
	...
	async def _enhance_response(
		...
		try:
			...
			if metadata.workflow_enabled and hasattr(response, 'headers'):
				...
				response.headers['X-Workflow-Integration-Level'] = metadata.integration_level.value
				...
				if metadata.request_id in self._active_workflows:
					...
					if self.config.include_progress_links:
						response.headers['X-Progress-URL'] = f"/api/v1/workflows/{workflow_instance_id}/progress"
				
				if metadata.workflow_template_id:
					response.headers['X-Workflow-Template-Id'] = metadata.workflow_template_id
	...
	async def _send_progress_notification(self, callback_url: str, data: Dict[str, Any]):
			...
			async with aiohttp.ClientSession() as session:
				async with session.post(
					...
					if response.status != 200:
						logger.warning(f"Progress notification failed: {response.status}")
	...
	async def _handle_workflow_event(self, event: DocumentWorkflowEvent):
		...
		try:
			logger.debug(f"Received workflow event: {event.event_type} for document {event.document_id}")
			
			# Find associated request metadata
			request_metadata = None
			for req_id, metadata in self._request_metadata.items():
				if (metadata.user_id == event.user_id and 
					event.workflow_instance_id in self._active_workflows.values()):
...
def add_workflow_middleware_to_app(
	...
	app.add_middleware(WorkflowMiddleware, workflow_bridge=workflow_bridge, config=config)