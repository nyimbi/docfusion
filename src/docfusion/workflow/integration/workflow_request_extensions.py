...
from ..automation.task_scheduler import SchedulingStrategy, TaskPriority
...
class WorkflowEnabledDocumentResult(BaseModel):
    ...
    team_participation: Dict[str, Dict[str, Any]] = Field(
    ...
    task_completion_times: Dict[str, float] = Field(
        default_factory=dict
    )  # task_id -> duration
...
class WorkflowRequestBuilder:
    ...
    def with_generation_config(
        self, config: DocumentGenerationConfiguration
    ...
    def with_workflow_mode(
        self, mode: DocumentWorkflowMode
    ) -> "WorkflowRequestBuilder":
        """Set workflow mode"""
    ...
    def with_deadline(
        self, deadline: datetime, buffer_hours: float = 24.0
    ) -> "WorkflowRequestBuilder":
    ...
    def with_priority(
        self, priority: TaskPriority, urgency: str = "medium"
    ) -> "WorkflowRequestBuilder":
