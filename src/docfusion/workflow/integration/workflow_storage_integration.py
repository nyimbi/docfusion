...
from ..automation.workflow_engine import (
	WorkflowEngine, WorkflowInstance, ProcessState, WorkflowEvent
)
from ..coordination.task_coordinator import TaskAssignment
from ..monitoring.workflow_monitor import WorkflowMonitor, PerformanceMetrics
...
class WorkflowStorageMetadata(BaseModel):
	...
	phase_checkpoints: Dict[str, str] = Field(default_factory=dict)  # phase -> document_version
...
class WorkflowStorageIntegration:
	...
	def __init__(
		...
		# Internal state
		self.workflow_documents: Dict[str, List[str]] = {}  # workflow_id -> document_ids
	...
	async def store_workflow_document(
		...
		async with self._storage_lock:
			try:
				...
				workflow_instance = await self.workflow_engine.get_workflow_instance(workflow_instance_id)
				...
				if workflow_instance:
					...
					if hasattr(workflow_instance, 'completed_tasks') and hasattr(workflow_instance, 'total_tasks'):
						workflow_metadata.completion_percentage = (
							len(workflow_instance.completed_tasks) / max(workflow_instance.total_tasks, 1) * 100.0
						)
				
				# Add generation metrics if available
				if generation_result:
					workflow_metadata.processing_time_seconds = generation_result.processing_time
					workflow_metadata.quality_score = generation_result.overall_quality_score
					workflow_metadata.task_durations = generation_result.component_processing_times
				...
				logger.info(f"Stored workflow document {document_id} for workflow {workflow_instance_id}")
	...
	async def retrieve_workflow_document(
		...
		try:
			...
			logger.error(f"Failed to retrieve workflow document {document_id}: {str(e)}")
	...
	async def update_workflow_document_state(
		...
		async with self._storage_lock:
			try:
				...
				doc_result = await self.retrieve_workflow_document(document_id, include_workflow_context=True)
				...
				logger.info(f"Updated workflow state for document {document_id} to {new_state.value}")
		...
		try:
			...
			if workflow_filter:
				if 'workflow_state' in workflow_filter:
					filters['document_workflow_state'] = workflow_filter['workflow_state']
	...
	async def get_workflow_performance_insights(
		...
		try:
			...
			workflow_metrics = await self.workflow_monitor.get_comprehensive_workflow_metrics()
	...
	async def _get_document_workflow_context(self, document_id: str) -> Dict[str, Any]:
		...
		try:
			workflow_instance = await self.workflow_engine.get_workflow_instance(workflow_id)
			if not workflow_instance:
				return {}
			...
			context = {
				...
				'workflow_state': workflow_instance.state.value if workflow_instance.state else None,
				'workflow_type': getattr(workflow_instance, 'process_definition_id', None),
			...
			logger.warning(f"Failed to get workflow context for document {document_id}: {str(e)}")
			return {}
	...
	async def _enhance_search_result_with_workflow(
		...
		try:
			...
			if workflow_context:
				enhanced.workflow_state = ProcessState(workflow_context['workflow_state']) if workflow_context.get('workflow_state') else None
			...
			enhanced.completion_percentage = workflow_metadata.get('completion_percentage', 0.0)
			
			if workflow_metadata.get('deadline'):
				enhanced.deadline = datetime.fromisoformat(workflow_metadata['deadline'])
	...
	async def _rank_workflow_search_results(
		...
		try:
			...
			for result in results:
				workflow_score = await self._calculate_workflow_relevance(result, query, user_id)
	...
	async def _extract_workflow_context_factors(
		...
		return {
			...
			'assigned_users': getattr(workflow_instance, 'assigned_users', []),
			'workflow_age_days': (datetime.now() - getattr(workflow_instance, 'created_at', datetime.now())).days,
			...
			if workflow_id in self.workflow_documents:
				...
				for doc_id in document_ids:
					try:
						...
						logger.warning(f"Failed to retrieve candidate document {doc_id}: {str(e)}")
						continue
		...
		for candidate in candidates:
			try:
				...
				if score > 0.3:  # Minimum threshold
					...
					recommendation.workflow_state = ProcessState(workflow_context.get('workflow_state')) if workflow_context.get('workflow_state') else None
					recommendation.current_phase = workflow_metadata.get('current_phase')
					recommendation.completion_percentage = workflow_metadata.get('completion_percentage', 0.0)
					recommendation.assigned_users = workflow_metadata.get('assigned_users', [])
					
					# Add success probability based on historical data
					recommendation.success_probability = await self._estimate_success_probability(
	...
	async def _calculate_recommendation_score(
		...
		# Phase match boost
		if workflow_metadata.get('current_phase') == context_factors.get('current_phase'):
			score += 0.3
	...
	async def _analyze_workflow_document_performance(
		...
		# Calculate averages
		if quality_scores:
			performance_data['average_quality_score'] = sum(quality_scores) / len(quality_scores)
		
		if processing_times:
			performance_data['average_processing_time'] = sum(processing_times) / len(processing_times)
		
		performance_data['phase_performance'] = phase_counts
		performance_data['success_rate'] = successful_completions / max(len(document_ids), 1)
	...
	async def _generate_performance_recommendations(
		...
		if phase_performance:
			slowest_phase = max(phase_performance.items(), key=lambda x: x[1]['avg_time'])
	...
	async def _cleanup_cache_if_needed(self):
		...
		if len(self.cached_recommendations) > self.max_cache_entries:
			...
			keys_to_remove = list(self.cached_recommendations.keys())[:self.max_cache_entries // 5]
			for key in keys_to_remove:
				del self.cached_recommendations[key]
	
	
	async def _invalidate_related_cache(self, document_id: str, workflow_instance_id: str):
