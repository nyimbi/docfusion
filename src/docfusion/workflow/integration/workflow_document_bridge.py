...
from ..automation.workflow_engine import (
	WorkflowEngine, WorkflowInstance, ProcessState, TriggerType, 
	WorkflowTrigger, WorkflowEvent
)
from ..automation.task_scheduler import (
	TaskScheduler, ScheduledTask, TaskState, SchedulingStrategy, TaskPriority
)
from ..coordination.task_coordinator import (
	TaskCoordinator, TaskAssignment, AssignmentStrategy, TaskComplexity, 
	CollaborationType
)
from ..coordination.deadline_manager import (
	DeadlineManager, DeadlineAlert, AlertLevel, CriticalPath
)
from ..monitoring.workflow_monitor import (
	WorkflowMonitor, MonitoringAlert, PerformanceMetrics
)

# Import document engine components
from ...document_engine.document_engine import (
	DocumentEngine, DocumentGenerationRequest, DocumentGenerationResult,
	DocumentGenerationConfiguration, GenerationPhase
)
...
class WorkflowDocumentBridge:
		...
		async with self._lock:
			try:
				...
				workflow_context = {
					...
					"target_word_count": getattr(document_request.generation_config, 'target_word_count', 3000),
					"complexity_level": getattr(document_request, 'complexity_level', 'medium'),
				...
				await self._create_workflow_tasks(workflow_instance, document_request, assigned_users)
				...
				self.active_document_workflows[document_request.request_id] = workflow_instance
				...
				logger.info(f"Created document workflow {workflow_instance.instance_id} for document {document_request.request_id}")
	...
	async def execute_document_workflow(
		...
		try:
			...
			if self.config.workflow_mode == DocumentWorkflowMode.AUTOMATIC:
				result = await self._execute_automated_workflow(workflow_instance_id, document_request)
			elif self.config.workflow_mode == DocumentWorkflowMode.ASSISTED:
				result = await self._execute_assisted_workflow(workflow_instance_id, document_request)
			else:
				result = await self._execute_manual_workflow(workflow_instance_id, document_request)
			...
			logger.error(f"Failed to execute document workflow {workflow_instance_id}: {str(e)}")
	...
	async def get_workflow_progress(self, document_id: str) -> Optional[WorkflowProgress]:
	...
	async def update_task_progress(
		...
		async with self._lock:
			try:
				...
				# Emit progress event
				document_id = await self._get_document_id_for_workflow(workflow_instance_id)
				if document_id:
					await self._emit_workflow_event(
	...
	async def subscribe_to_workflow_events(self, callback: Callable[[DocumentWorkflowEvent], None]):
	...
	async def _create_workflow_tasks(
		...
		try:
			...
			logger.info(f"Created {len(created_tasks)} workflow tasks for {workflow_instance.instance_id}")
	...
	async def _assign_task_to_users(self, task: ScheduledTask, user_ids: List[str]):
		...
		try:
			...
			if assignment:
				logger.info(f"Assigned task {task.task_id} to user {assignment.assigned_user_id}")
			else:
	...
	async def _update_workflow_progress(self, workflow_instance_id: str):
		...
		try:
			...
			workflow_tasks = await self.task_scheduler.get_tasks_for_workflow(workflow_instance_id)
			
			# Calculate completion percentage
			if workflow_tasks:
				total_tasks = len(workflow_tasks)
				completed_tasks = sum(1 for task in workflow_tasks if task.state == TaskState.COMPLETED)
				progress.completion_percentage = (completed_tasks / total_tasks) * 100.0
				
				# Update active tasks
				progress.active_tasks = [
					task.task_id for task in workflow_tasks 
					if task.state in [TaskState.RUNNING, TaskState.QUEUED]
				]
				
				# Update current phase based on active tasks
				for task in workflow_tasks:
					if task.state == TaskState.RUNNING and "generation_phase" in task.metadata:
			...
			if progress.completion_percentage < 100.0:
				remaining_tasks = [t for t in workflow_tasks if t.state != TaskState.COMPLETED]
				if remaining_tasks:
					remaining_minutes = sum(t.estimated_duration_minutes for t in remaining_tasks)
					progress.estimated_completion_time = datetime.now() + timedelta(minutes=remaining_minutes)
	...
	def _estimate_completion_time(self, document_request: DocumentGenerationRequest) -> float:
		...
		doc_type = getattr(document_request.generation_config, 'document_type', 'standard_document')
		...
		target_words = getattr(document_request.generation_config, 'target_word_count', 3000)
		word_factor = max(1.0, target_words / 3000.0)  # Scale based on 3000 words baseline
	...
	async def _get_document_id_for_workflow(self, workflow_instance_id: str) -> Optional[str]:
		...
		while not self._shutdown_event.is_set():
			try:
				...
				for doc_id in list(self.document_progress.keys()):
					...
					if progress.completion_percentage < 100.0:
						await self._update_workflow_progress(progress.workflow_instance_id)
			...
			try:
				...
				for doc_id, progress in self.document_progress.items():
					if (progress.completion_percentage < 100.0 and 
						progress.estimated_completion_time and
						progress.estimated_completion_time < datetime.now() + timedelta(hours=2)):
						...
						await self._emit_workflow_event(
							...
							None,
							{
								...
								"hours_remaining": (progress.estimated_completion_time - datetime.now()).total_seconds() / 3600
							}
						)
