...
from .engines.text_search_engine import TextSearchEngine, SearchQuery, SearchResult
from .engines.document_retrieval import DocumentRetrieval, DocumentMetadata
from .indexes.document_indexer import DocumentIndexer, IndexConfiguration, IndexedDocument
from .retrievers.document_retriever import DocumentRetriever, ContentRecommendation
...
class StorageService:
			...
			try:
				...
				if self.config.enable_indexing:
					...
					self._document_indexer = DocumentIndexer(
						self.config.storage_root_path / "indexes",
						index_config
					)
	...
	async def retrieve_document(
		...
		try:
			...
			if result and include_metadata and self._document_indexer:
				...
				indexed_doc = await self._document_indexer.get_document_by_id(document_id)
				if indexed_doc:
		...
		try:
			...
			if self._document_retriever:
				...
				self._stats.total_content_blocks = len(
					[block for block in (await self._document_retriever.get_popular_content("block", 10000))]
				)
				self._stats.total_templates = len(
					[template for template in (await self._document_retriever.get_popular_content("template", 10000))]
				)
	...
	async def optimize_storage(self) -> Dict[str, Any]:
		...
		try:
			...
			if self._document_retrieval:
				...
				optimization_results['components_optimized'].append('document_retrieval')
	...
	async def health_check(self, include_details: bool = True) -> StorageHealthCheck:
		...
		try:
			...
			if self._search_engine:
				...
				performance_metrics['search_response_time_ms'] = search_health.response_time_ms
				
				if search_health.status == HealthStatus.UNHEALTHY:
					errors.append(f"Search engine unhealthy: {search_health.error_message}")
				elif search_health.status == HealthStatus.DEGRADED:
					warnings.append(f"Search engine degraded: {search_health.error_message}")
			else:
			...
			if self._document_retrieval:
				retrieval_health = await self._check_document_retrieval_health(include_details)
				component_health['document_retrieval'] = retrieval_health
				performance_metrics['retrieval_response_time_ms'] = retrieval_health.response_time_ms
				
				if retrieval_health.status == HealthStatus.UNHEALTHY:
					errors.append(f"Document retrieval unhealthy: {retrieval_health.error_message}")
				elif retrieval_health.status == HealthStatus.DEGRADED:
					warnings.append(f"Document retrieval degraded: {retrieval_health.error_message}")
			else:
			...
			if self._document_indexer:
				indexer_health = await self._check_document_indexer_health(include_details)
				component_health['document_indexer'] = indexer_health
				performance_metrics['indexer_response_time_ms'] = indexer_health.response_time_ms
				
				if indexer_health.status == HealthStatus.UNHEALTHY:
					errors.append(f"Document indexer unhealthy: {indexer_health.error_message}")
				elif indexer_health.status == HealthStatus.DEGRADED:
					warnings.append(f"Document indexer degraded: {indexer_health.error_message}")
			else:
			...
			if self._document_retriever:
				retriever_health = await self._check_document_retriever_health(include_details)
				component_health['document_retriever'] = retriever_health
				performance_metrics['retriever_response_time_ms'] = retriever_health.response_time_ms
				
				if retriever_health.status == HealthStatus.UNHEALTHY:
					errors.append(f"Document retriever unhealthy: {retriever_health.error_message}")
				elif retriever_health.status == HealthStatus.DEGRADED:
					warnings.append(f"Document retriever degraded: {retriever_health.error_message}")
			else:
			...
			if filesystem_health.status == HealthStatus.UNHEALTHY:
				errors.append(f"Filesystem unhealthy: {filesystem_health.error_message}")
			elif filesystem_health.status == HealthStatus.DEGRADED:
				warnings.append(f"Filesystem degraded: {filesystem_health.error_message}")
			
			# Determine overall status
			overall_status = self._determine_overall_health_status(component_health.values())
			
			# Add performance metrics
			performance_metrics.update({
				'uptime_seconds': uptime,
				'total_components': len(component_health),
				'healthy_components': len([c for c in component_health.values() if c.status == HealthStatus.HEALTHY]),
				'degraded_components': len([c for c in component_health.values() if c.status == HealthStatus.DEGRADED]),
				'unhealthy_components': len([c for c in component_health.values() if c.status == HealthStatus.UNHEALTHY])
			})
	...
	async def _check_search_engine_health(self, include_details: bool) -> ComponentHealth:
	...
	async def _check_document_retrieval_health(self, include_details: bool) -> ComponentHealth:
		...
		try:
			...
			if response_time > 3000:  # 3 seconds
				status = HealthStatus.UNHEALTHY
				error_message = f"Document retrieval response time too high: {response_time:.2f}ms"
			elif response_time > 1000:  # 1 second
				status = HealthStatus.DEGRADED
				error_message = f"Document retrieval response time degraded: {response_time:.2f}ms"
			else:
				status = HealthStatus.HEALTHY
				error_message = None
	...
	async def _check_document_indexer_health(self, include_details: bool) -> ComponentHealth:
		...
		try:
			...
			if response_time > 2000:  # 2 seconds
				status = HealthStatus.UNHEALTHY
				error_message = f"Document indexer response time too high: {response_time:.2f}ms"
			elif response_time > 500:  # 500ms
				status = HealthStatus.DEGRADED
				error_message = f"Document indexer response time degraded: {response_time:.2f}ms"
			else:
				status = HealthStatus.HEALTHY
				error_message = None
	...
	async def _check_document_retriever_health(self, include_details: bool) -> ComponentHealth:
		...
		try:
			...
			if include_details:
				...
				details = {
					...
					'average_recommendation_time': stats.get('average_recommendation_time', 0)
				}
			
			# Determine status based on response time
			if response_time > 2000:  # 2 seconds
				status = HealthStatus.UNHEALTHY
				error_message = f"Document retriever response time too high: {response_time:.2f}ms"
			elif response_time > 500:  # 500ms
				status = HealthStatus.DEGRADED
				error_message = f"Document retriever response time degraded: {response_time:.2f}ms"
			else:
				status = HealthStatus.HEALTHY
				error_message = None
	...
	def _determine_overall_health_status(self, component_healths) -> HealthStatus:
		...
		degraded_ratio = (health_counts[HealthStatus.DEGRADED] + health_counts[HealthStatus.UNKNOWN]) / total_components
