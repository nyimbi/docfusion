...
class TextSearchEngine:
    ...
    def __init__(self, storage_path: Optional[Path] = None):
        ...
        self.inverted_index: Dict[str, Set[str]] = defaultdict(
            set
        )  # word -> document_ids
        self.tf_idf_scores: Dict[str, Dict[str, float]] = defaultdict(
            dict
        )  # doc_id -> {word: tf_idf}
        self.document_frequencies: Dict[str, int] = defaultdict(
            int
        )  # word -> num_docs_containing
    ...
    def _calculate_tf_idf(
        self, document_id: str, tokens: List[str]
    ) -> Dict[str, float]:
    ...
    def _update_inverted_index(self, document_id: str, tokens: List[str]) -> None:
        ...
        for token in unique_tokens:
            ...
            if document_id not in [
                doc for doc in self.document_indexes.values() if token in doc.stems
            ]:
    ...
    def _fuzzy_match(
        self, query_token: str, indexed_token: str, max_distance: int = 2
    ) -> float:
    ...
    def _highlight_text(
        self, text: str, query_tokens: List[str], max_fragments: int = 3
    ) -> List[str]:
    ...
    async def remove_document(self, document_id: str) -> None:
        ...
        for token in set(doc_index.stems):
            ...
            if not self.inverted_index[token]:
                ...
                self.document_frequencies[token] = max(
                    0, self.document_frequencies[token] - 1
                )
    ...
    async def search(self, query: SearchQuery) -> List[SearchResult]:
        ...
        try:
            ...
            self.stats.most_common_queries[query_lower] = (
                self.stats.most_common_queries.get(query_lower, 0) + 1
            )

            # Parse query based on settings
            if query.boolean_operators and any(
                op in query.query.upper() for op in ["AND", "OR", "NOT"]
            ):
            ...
            results = [
                r for r in results if r.relevance_score >= query.min_relevance_threshold
            ][: query.result_limit]

            # Generate highlights
            query_tokens = self._tokenize_text(query.query)
            for result in results:
                result.highlights = self._highlight_text(
                    result.content, query_tokens, query.highlight_fragments
                )
        ...
        for query_stem in query_stems:
            for indexed_stem in self.inverted_index.keys():
                similarity = self._fuzzy_match(
                    query_stem, indexed_stem, query.max_fuzzy_distance
                )
    ...
    async def suggest_queries(
        ...
        if not partial_query.strip():
            ...
            common_queries = sorted(
                self.stats.most_common_queries.items(), key=lambda x: x[1], reverse=True
            )[:max_suggestions]
        ...
        matching_tokens.sort(
            key=lambda token: len(self.inverted_index[token]), reverse=True
        )
        ...
        try:
            ...
            if stats_file.exists():
                with open(stats_file, "r") as f:
                    ...
                    self.stats.average_search_time = data.get(
