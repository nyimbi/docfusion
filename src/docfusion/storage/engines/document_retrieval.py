...
class DocumentRetrieval:
    ...
    async def list_documents(
        ...
        if tags:
            documents = [
                doc for doc in documents if any(tag in doc.tags for tag in tags)
            ]
    ...
    async def search_documents(
        ...
        if content_types:
            matching_docs = [
                doc for doc in matching_docs if doc.content_type in content_types
            ]
        ...
        matching_docs.sort(key=lambda x: (x.access_count, x.updated_at), reverse=True)
    ...
    async def delete_document(
        self, document_id: str, delete_all_versions: bool = False
    ) -> bool:
    ...
    async def get_storage_stats(self) -> Dict[str, Any]:
        ...
        total_size = sum(
            metadata.file_size for metadata in self.document_index.values()
        )
        ...
        return {
            ...
            "cache_hit_rate": (
                self.stats.cache_hits / max(self.stats.total_retrievals, 1)
            )
            * 100,
            ...
            for file_path in all_files:
                if file_path not in referenced_files:
                    try:
                        ...
                        optimization_stats["errors"].append(
...
async def store_text_document(
    ...
    return await retrieval.store_document(
        content=content, title=title, content_type="text", **kwargs
    )
