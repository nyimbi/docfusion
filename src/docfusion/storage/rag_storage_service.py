...
from .storage_service import StorageService, StorageConfiguration, StorageStats
from .rag import (
	RAGService, RAGConfiguration, RAGSearchResult, RAGQueryResult,
	create_rag_service, get_default_rag_config
)
...
class RAGStorageService:
		...
		if self._rag_available and enable_rag_indexing and self.rag_service:
			try:
				...
				if rag_doc_id:
					...
					self.logger.warning(f"Failed to index document {document_id} in RAG system")
			
			except Exception as e:
				self.logger.error(f"RAG indexing failed for document {document_id}: {e}")
	...
	async def search_documents(
		...
		if search_type == "semantic" and not self._rag_available:
			self.logger.warning("Semantic search requested but RAG not available, falling back to full_text")
		...
		try:
			...
			if search_type in ["semantic", "hybrid"] and self._rag_available and self.rag_service:
	...
	async def get_content_recommendations(
		...
		if use_semantic_search and self._rag_available and self.rag_service:
			try:
				...
				for rag_search_result in rag_result.results:
					...
					recommendation = type('ContentRecommendation', (), {
						'title': rag_search_result.document_title,
		...
		recommendations.sort(key=lambda x: getattr(x, 'relevance_score', 0), reverse=True)
	...
	async def optimize_storage(self) -> Dict[str, Any]:
		...
		try:
			optimization_results['base_storage_optimization'] = await self.base_storage.optimize_storage()
		except Exception as e:
		...
		if self._rag_available and self.rag_service:
			try:
				optimization_results['rag_optimization'] = await self.rag_service.optimize_system()
			except Exception as e:
