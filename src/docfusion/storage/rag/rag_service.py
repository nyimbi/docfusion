...
from .database import RAGDatabase, RAGDocument, DocumentChunk, DatabaseConfiguration
from .embedding_service import EmbeddingService, EmbeddingConfiguration, EmbeddingResult
...
class RAGService:
	...
	def _init_database_config(self):
		...
		embedding_model = self.config.ollama_model if self.config.embedding_provider == "ollama" else self.config.openai_model
	...
	async def add_document(
		...
		try:
			...
			if stored_chunks > 0:
				self.logger.info(f"Successfully added document {document_id} with {stored_chunks} chunks")
				return document_id
			else:
	...
	async def search(
		...
		try:
			...
			query_embedding_result = await self.embedding_service.generate_query_embedding(query)
	...
	async def get_document_context(
		...
		try:
			...
			raw_results = await self.database.semantic_search(
				query_embedding=[0.0] * self.config.embedding_dimensions,  # Dummy embedding
				limit=max_chunks,
	...
	async def get_related_documents(
		...
		try:
			...
			self.logger.error(f"Failed to find related documents for {document_id}: {e}")
			return []
	...
	async def get_statistics(self) -> RAGStats:
		...
		try:
			...
			if self.query_history:
				successful_queries = len([q for q in self.query_history if not q.get('error')])
				self._stats.cache_hit_rate = successful_queries / len(self.query_history)
	...
	async def optimize_system(self) -> Dict[str, Any]:
		...
		try:
			...
			optimization_results['operations_performed'].append('cleared_embedding_cache')
	...
	def _update_query_stats(self, query_result: RAGQueryResult):
		...
		if self._stats.avg_query_time == 0:
			...
			self._stats.avg_query_time = (
				(self._stats.avg_query_time * (self._stats.total_queries - 1) + query_result.search_time)
				/ self._stats.total_queries
			)
		
		# Update average similarity score
		if query_result.results:
			avg_similarity = sum(r.similarity_score for r in query_result.results) / len(query_result.results)
			if self._stats.avg_similarity_score == 0:
				self._stats.avg_similarity_score = avg_similarity
			else:
				self._stats.avg_similarity_score = (
					(self._stats.avg_similarity_score * (self._stats.total_queries - 1) + avg_similarity)
					/ self._stats.total_queries
				)
		...
		query_record = {
			...
			'avg_similarity': sum(r.similarity_score for r in query_result.results) / len(query_result.results) if query_result.results else 0,
