...
class ContentBlock:
	...
	dependencies: List[str] = field(default_factory=list)  # Dependencies on other blocks
...
class Template:
	...
	category: str = "general"
	structure: Dict[str, Any] = field(default_factory=dict)  # Template structure definition
...
class DocumentRetriever:
	...
	def _extract_keywords(self, text: str, max_keywords: int = 10) -> List[str]:
		...
		stop_words = {
			...
			'said', 'each', 'which', 'them', 'would', 'there', 'could'
		}
	...
	async def retrieve_content_block(self, block_id: str, track_usage: bool = True) -> Optional[ContentBlock]:
	...
	async def retrieve_template(self, template_id: str, track_usage: bool = True) -> Optional[Template]:
		...
		try:
			...
			# Update performance metrics
			retrieval_time = asyncio.get_event_loop().time() - start_time
			self.stats.performance_metrics['last_template_retrieval_time'] = retrieval_time
			self.stats.average_retrieval_time = (
				(self.stats.average_retrieval_time * (self.stats.total_retrievals - 1) + retrieval_time) / 
				self.stats.total_retrievals
			)
	...
	async def search_content_blocks(
		...
		matching_blocks.sort(
			key=lambda x: (x[1], x[0].usage_count, x[0].quality_score), 
			reverse=True
		)
	...
	async def search_templates(
		...
		for template_id in candidates:
			...
			if query:
				...
				# Description match
				desc_similarity = self._calculate_similarity(query, template.description)
	...
	async def recommend_content(
		...
		if cache_key in self.recommendation_cache:
			...
			cache_time = cached_result[0].metadata.get('cached_at') if cached_result else None
			if cache_time and datetime.now() - datetime.fromisoformat(cache_time) < self.cache_ttl:
				...
				if relevance_score >= min_relevance:
					recommendation = ContentRecommendation(
						...
						recommendation_reason=self._generate_recommendation_reason(
							relevance_score, content_similarity, keyword_overlap, usage_bonus
						),
						usage_frequency=block.usage_count,
						quality_score=block.quality_score,
						last_used=block.last_used,
						metadata={'block_type': block.block_type, 'category': block.category}
					)
		...
		if content_type in ["template", "both"]:
			for template_id, template in self.templates.items():
				...
				desc_similarity = self._calculate_similarity(context, template.description)
				relevance_score += desc_similarity * 2.0
	...
	async def get_content_block_by_category(self, category: str) -> List[ContentBlock]:
		...
		return [self.content_blocks[block_id] for block_id in block_ids if block_id in self.content_blocks]
	...
	async def get_popular_content(self, content_type: str = "both", limit: int = 20) -> List[Union[ContentBlock, Template]]:
				...
				for block_file in self.blocks_path.glob("*.json"):
					try:
						async with aiofiles.open(block_file, 'r') as f:
							...
							for field in ['created_at', 'updated_at', 'last_used']:
								if field in block_dict and isinstance(block_dict[field], str):
									block_dict[field] = datetime.fromisoformat(block_dict[field])
				...
				for template_file in self.templates_path.glob("*.json"):
					try:
						async with aiofiles.open(template_file, 'r') as f:
							...
							# Convert datetime strings back
							for field in ['created_at', 'updated_at', 'last_used']:
								if field in template_dict and isinstance(template_dict[field], str):
									template_dict[field] = datetime.fromisoformat(template_dict[field])
...
async def create_document_retriever(storage_path: Optional[Path] = None) -> DocumentRetriever:
