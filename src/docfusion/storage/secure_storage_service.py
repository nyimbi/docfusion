...
from .rag_storage_service import RAGStorageService, RAGStorageConfiguration, EnhancedSearchResult
...
class SecureStorageService:
	...
	def __init__(self, config: SecureStorageConfiguration):
		...
		self.encrypt_sensitive_fields = config.encrypt_sensitive_fields or [
			"content", "description", "notes", "comments"
		]
	...
	async def create_document(
		...
		try:
			...
			if self.config.enable_document_encryption and (encrypt_content is True or encrypt_content is None):
				...
				if encryption_result.success:
					...
					self.logger.warning(f"Failed to encrypt document content: {encryption_result.error_message}")
			...
			secure_metadata = {
				**(metadata or {}),
				...
				'security_level': 'encrypted' if encryption_metadata.get('encrypted') else 'standard',
				**encryption_metadata
			}
			...
			if self.config.audit_all_operations:
				await self.security.log_document_activity(
					...
					f"Created document: {title}",
					{
						...
						'tags': tags or [],
						'encrypted': encryption_metadata.get('encrypted', False),
						'sharing_permissions': list(sharing_permissions.keys()) if sharing_permissions else []
					}
				)
	...
	async def get_document(
		...
		try:
			...
			if (decrypt_content and 
				document['metadata'].get('encrypted') and
				'encryption_key_id' in document['metadata']):
				...
				if decryption_result.success:
					...
					self.logger.warning(f"Failed to decrypt document {document_id}: {decryption_result.error_message}")
	...
	async def update_document(
		...
		try:
			...
			after_state = {
				...
				'content_length': len(content) if content else before_state['content_length'],
			...
			if self.config.audit_all_operations:
				await self.security.log_document_activity(
					"modified", document_id, user_id,
					f"Updated document: {title or before_state['title']}",
					{
						'fields_updated': [k for k, v in {
		...
		try:
			...
			if current_doc['success']:
				doc_info = {
					...
					'encrypted': current_doc['document']['metadata'].get('encrypted', False)
				}
			else:
	...
	async def search_documents(
		...
		try:
			...
			accessible_docs = await self.security.doc_permissions.get_user_accessible_documents(
			...
			search_results = [
				...
				for i in range(min(3, limit)) if f"doc_{i}" in accessible_doc_ids[:3]
			]
	...
	async def share_document(
		...
		if not permission_result['has_permission']:
			return {'success': False, 'error': 'Permission denied - cannot share document'}
...
# Factory function
def create_secure_storage_service(config: SecureStorageConfiguration) -> SecureStorageService:
